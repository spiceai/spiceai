...
- [ ] **Concurrency limiting**: The connector limits the number of concurrent in-flight requests to the upstream source (e.g. via a semaphore). HTTP-based connectors must share this limit per upstream origin across datasets, and the limit must be configurable by both a runtime-level default (e.g. `runtime.params.http_max_concurrent_requests`) and a dataset override (e.g. `max_concurrent_requests`).
- [ ] **Request-rate limiting**: HTTP-based connectors that can issue multiple upstream requests support configurable request budgets (e.g. `requests_per_second_limit` or `requests_per_minute_limit`) and jitter controls (e.g. `rate_control_jitter_min` and `rate_control_jitter_max`) to reduce synchronized request bursts. Runtime-level defaults should be available for HTTP connector families, with dataset parameters overriding them when needed.
