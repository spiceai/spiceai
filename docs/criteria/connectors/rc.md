...
- [ ] **In-flight request observability**: The connector emits a component-level metric (e.g. `connector_inflight_requests` gauge) that tracks the number of concurrent in-flight requests, with `service` and `operation` dimensions. This metric must be observable via the runtime metrics endpoint.
