...
**Which passes may overlap.** `compaction_lock` is a read/write lock. Every pass that repoints or re-reads the *whole* protected set or the current snapshot takes it exclusively and skips if anything else holds it: the current-snapshot rewrite (which folds in and clears every protected snapshot), the seq-prefix bake, the post-write manifest rebuild, cold promotion, and the reopen drain barrier. The protected-snapshot subset merge on a key-delete table takes it *shared*, because it rewrites only the runs it selected and commits them with the catalog CAS in `swap_protected_snapshots_in_txn`. Without that, one large-tier merge — minutes for a few hundred MB on one encoder — blocked every other merge while inline checkpoints kept adding tier-0 runs that each scan unions, so files per query climbed until it committed (#14291). Concurrent merges are kept apart by `protected_merge_claims`: Phase 1 hides runs another merge has claimed, and once a pass has chosen its inputs it registers them in one check-and-insert that refuses a run already claimed or no longer published, and any tier not strictly below every running merge's tier. That tier rule makes the concurrent lane a short small-tier lane — two large merges never run at once, and a merge never feeds the tier a running merge is producing — and a pass selects against the pass budget *minus* the running merges' inputs, so together they stay inside the single-pass memory ceiling below. The claim is released only after the in-memory publish, on every exit. Each merge still captures its own deletion snapshot and fence and tags its output with it, so its result does not depend on which other runs exist or are being merged. A decline while another merge runs is recorded as `declined_lock_busy`, as it was when the pass stopped at the lock. Position-delete tables keep the fully serialized shape: their subset merge takes the lock exclusively as well as the write lock.
...
| `declined_lock_busy` | another pass holds the per-table compaction lock, or (subset merge) the runs or tier it could merge are held by another in-flight merge |
