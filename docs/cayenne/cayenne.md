...
The worker then clears only the markers whose stored `sequence_number` is still at or below what it claimed (a newer commit that re-dirtied a key leaves its marker in place, so the next pass delivers it). Delivery failure never blocks accelerator commits; the dirty set simply grows until the next successful pass. v1 delivers single-column primary keys.
...
| `cayenne_pending_write_back` | durable-write-back dirty-key markers: one row per committed-but-undelivered primary key (`WITHOUT ROWID`; raw-UUID-byte `table_id` + `RowConverter`-encoded `pk_bytes`; `sequence_number` of the commit that dirtied it; `op` = whether that commit upserted or deleted the key, moving with the winning sequence); the delivery worker claims, reconciles to the federated source per the recorded `op`, and clears |
