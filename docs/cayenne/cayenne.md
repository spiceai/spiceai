...
The scan-view maintainer's rebuild floor is a **process-wide environment variable**, not a per-dataset parameter (it is operational tuning, not data configuration, so it stays out of `configuration_matches`): `CAYENNE_SCAN_VIEW_REFRESH_FLOOR_MS` (default `250`; `0` = rebuild as fast as the input changes) bounds how often the background maintainer recomputes a table's published scan view. It exposes `cayenne_scan_view_builds_total` / `cayenne_scan_view_build_duration_ms` (one off-query-path build per table version — expect the count ≪ the scan count) and `cayenne_scan_view_waits_total` / `cayenne_scan_view_wait_duration_ms` (scans that waited on the linearizable freshness gate for a fresh-enough bundle — ~0 in steady state) for operator observability.
