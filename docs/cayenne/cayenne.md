...
**Superseded snapshot directories are reclaimed against live state, not on a timer.** Every commit that advances the snapshot pointer schedules one debounced sweep, coalesced so a burst of commits produces a single pass, and the pass reads the *current* pointer itself rather than an id captured when it was queued — an id captured at schedule time goes stale, and the sweep keeps every directory at or newer than the id it is given. What protects a directory from a scan still reading it is the per-snapshot in-flight-scan ref-count (`SnapshotScanRef`), which a scan takes under the listing fence alongside the snapshot ids it captures, so no new scan can reference a superseded directory after the flip and a directory with a live ref is never unlinked however long its query runs. A short static grace (5 s, measured from a directory's own UUIDv7 creation time and from the last scan that listed it) covers only the residual window between a plan resolving a listing and opening the files it resolved; a sweep that defers a directory on that grace re-arms itself once, so a table that stops committing still reaps its last window. Because reclamation tracks the live pointer, a table's steady-state warm footprint tracks its live size rather than its commit rate — a rapidly refreshing table used to retain a full grace-window of dead snapshot directories, which at a few commits a second was orders of magnitude more than the table itself (#13657).
