...
| `cayenne_datalake_promotion_interval_ms` | datalake promoter cadence (`0` rejected at registration — it would disable promotion AND GC) | `60_000` |
| `cayenne_datalake_gc_interval_ms` | physical-GC cadence AND orphan grace for superseded datalake objects (`0` rejected at registration) | `300_000` (5 min) |
...
|------|-----------------|---------|
| 2026-07-12 | branch `sgrebnov/0710-cold-pk-bloom-split` (pre-merge) | **Fix: subset-merge/bake in-memory publish guarded against a mid-pass overwrite.** Both passes commit their catalog swap first and only then update the in-memory protected-snapshot map under the listing fence; an overwrite or datalake promotion committing in between wipes the map and the catalog rows, so the un-guarded re-insert resurrected the whole pre-overwrite row set next to its new/cold copies. The passes now capture `current_snapshot_id` at Phase 1 and revalidate under the Phase-3 fence, discarding the merged output on mismatch. |
| 2026-07-12 | branch `sgrebnov/0710-cold-pk-bloom-split` (pre-merge) | **Fix: seq-prefix bake no longer resurrects superseded datalake (cold) rows.** The bake physically applies tombstones to the warm snapshots it rewrites and then prunes them from the deletion index — but it never rewrites cold objects, so pruning a tombstone that masked a superseded cold-resident key silently un-hid the stale cold row (CH-benCH `-cold` runs over-counted promoted tables by ≈ their update count). Both prune sites (the bake and the full-rewrite prune) now cap the cutoff at the cold manifest's max sequence: tombstones above it survive until a promotion physically applies them, at which point the promotion commit clears the index as before. Retained tombstones cost memory bounded by the promotion cadence, never correctness. |
| 2026-07-11 | branch `sgrebnov/0711-cold-tier-hardening` (pre-merge; doc otherwise baselined at `4685a3dd`) | Datalake (cold) tier hardening. **Atomic promotion visibility**: a promotion's two publication points — the metastore cold-manifest commit (cold scans list files from `cayenne_cold_tier_file`) and the in-memory warm snapshot flip — now publish under ONE held `listing_fence.write()` acquisition, closing a transient window where a concurrent scan could pair the old warm snapshot with the new cold manifest and double-count the promoted rows. **Registration-time validation**: configurations that previously left the tier silently inert now surface at dataset registration — structured errors for an explicit `cayenne_deletion_mode: position` (previously warn + silent override to `key`; unset/`auto` still resolves to `key`), `cayenne_datalake_promotion_interval_ms: 0`, and `cayenne_datalake_gc_interval_ms: 0`; a PK-less table registers with a WARNING that the tier stays inactive; unknown `cayenne_datalake_clustering_columns` entries now warn instead of silently falling back. Clarified in-code that table drop removes cold manifest **rows** only — physical datalake objects are intentionally not deleted on drop (operator-managed, possibly shared bucket). |
| 2026-07-11 | PR #11812 (pre-merge; doc otherwise baselined at `4685a3dd`) | Cold-tier per-file PK bloom is now bounded by a per-file byte cap `COLD_PK_BLOOM_PER_FILE_MAX_BYTES` (raised 16→32 MiB); a file over the cap carries no bloom and the upsert keyset rebuild falls back to an exact cold scan. Promotion now **row-caps** each written cold file: the Z-order-sorted stream is split into contiguous row-bounded chunks (`RowChunkedSource` / `RowChunkStream`) so every cold file stays under the cap and remains bloom-backed, preserving global order (a split emits one WARN). Applies to bloom-eligible (upsert) tables only; `DoNothing`/non-upsert tables keep the exact-scan keyset path. |
