...
|------|-----------------|---------|
| 2026-07-14 | `c564ce76` + working-tree fixes (pre-merge; doc otherwise baselined at `4685a3dd`) | Exposed Spicepod `MIN`/`MAX` maintained aggregates with resolved-primary-key validation; bounded direct no-PK extremum multisets; added N>1 mem-tier UPDATE/DELETE maintenance as one pre-assigned, ordered IVM delta; and bound scan shard snapshots to the IVM epoch with an even/odd seqlock so optimizer substitution cannot cross visibility generations. The N>1 delete path now projects the original PK batch once instead of repeatedly concatenating shard prefixes. |
