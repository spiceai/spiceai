...
**The stored types are not always the source's types.** Vortex cannot represent every Arrow type, so `transform_schema_for_vortex` rewrites two of them at table creation and the table stores the rewritten form for its lifetime: `Float16` → `Float32`, and any non-microsecond `Timestamp` → `Timestamp(Microsecond, tz)`, preserving the timezone. The timestamp rule is the one operators meet: a `PostgreSQL` `timestamptz` is inferred as `Timestamp(ns, "UTC")` and stored at microsecond precision (as it is in the source, and as `DuckDB` stores its own `TIMESTAMPTZ`). This is a property of the storage format, not a stale acceleration — `on_schema_change`, `mode: file_create`, and dropping `partition_by` all leave it as it is, because a freshly created table applies the same rewrite. The same list is published as `CAYENNE_TYPE_REWRITE_RULES` so the acceleration write path can tell an engine-imposed type from a schema that has genuinely drifted. Types Vortex supports at all (`Interval`, `Duration`, `FixedSizeBinary` are the exceptions) are stored as-is, subject to `unsupported_type_action`.
...
|------|-----------------|---------|
| 2026-08-14 | PR for #13051 (pre-merge; doc otherwise baselined at `44d7538e`) | **A CDC widening bound for a partitioned acceleration is refused instead of applied lossily and acknowledged.** The apply loop resolves a Cayenne write target by walking to the concrete provider, and cannot reach one through the partition fan-out. It read that as "this engine cannot evolve mid-stream" and took the fallback written for engines a restart repairs — warn, cast the batch to the current accelerator schema, carry on. For a partitioned target a restart does not repair it: the accelerator pins the partitioned provider to `SchemaEvolutionMode::Disabled` when it builds it, so a restart re-classifies and refuses again, matching what registration already decided. Because the cast reported success, the source committers advanced past those changes and nothing would replay them — the reported defect. The apply now resolves the write target before the cast (reusing `extract_cayenne_write_target`, which crosses the poly and upsert-dedup wrappers and answers from the creator's own `accepts_direct_partition_writes` opt-in rather than a schema-metadata string) and returns a typed apply error for a partitioned Cayenne one, so the committers are dropped un-acked and the source redelivers. **User-visible behavior change**: such a dataset now errors and stops where it previously kept running while silently dropping widened columns; refusing is recoverable, the cast was not. Unpartitioned and non-Cayenne accelerations are unaffected. The change is in `crates/runtime-table`, not `crates/cayenne`; no schema or config-key change. *The corpus outlives a schema change* extended with the partitioned exclusion and its CDC consequence. |
| 2026-08-13 | PR for issue #12846 (pre-merge; doc otherwise baselined at `44d7538e`) | **A folded delete tombstone no longer restores `Exact` over a stale row count.** Datalake promotion applied every tombstone and cleared the deletion index — dropping the `has_pending_deletions()` mask — without re-baselining `num_rows`, so a table reported `Exact(110)` where the cold manifest held 109 live rows, and an `Exact` count may be substituted into a distributed `COUNT(*)` answer. Promotion now `Set`s the count from the manifest it commits (tainting instead when any written file's footer row count was unreadable), and a standalone user `DELETE` taints the persisted `num_rows_exact` — before the irreversible delete, so a cancellation or crash cannot strand the claim — so no fold path, including the seq-prefix bake, can un-mask a stale count as exact. *Table statistics* updated. |
| 2026-08-13 | PR for #13013 (pre-merge; doc otherwise baselined at `44d7538e`) | **The host-memory classification now sees accelerated catalogs.** Both axes above folded over the pod's datasets and views only, and a catalog was excluded on the grounds that it carries `CatalogAcceleration`, a separate type — a statement about representation, not about whether it demands memory. A catalog accelerates every table it discovers through the same write path as a dataset's, and `changes` is the only refresh mode it admits, so a pod whose only Cayenne acceleration was a catalog kept the standard 90% query pool while the in-memory CDC tier grew beside it, and took no compaction carve however its tables were stored — precisely the over-commit this accounting exists to prevent. A catalog's acceleration is now **converted** into the dataset acceleration its tables are configured with (`CatalogAcceleration::to_dataset_acceleration`) and classified through the same `RefreshWriteProfile` mapping, so the budget cannot disagree with the tables it is budgeting for. The conversion is a config-shape mapping only — it discovers nothing and yields one value per catalog — and it is the same one the catalog connector configures each synthesized table from, rather than a second copy of the classification rules kept in step by hand. Should a third component kind ever need this analysis, that conversion is the seam to replace with a trait both acceleration types implement, the pre-init counterpart of `AccelerationSource`. The off-pool per-table cache reservation counts a catalog once — how many tables it discovers is unknowable before the connector connects — which is a floor rather than an estimate, and a floor is what the previous zero was not. The dedicated CDC-apply runtime had the same blind spot and is fixed with it: a catalog-accelerated pod ran the changes-apply loop on the low-priority refresh runtime, which is the deprioritization that pool exists to prevent. No schema or config-key change. |
| 2026-08-12 | PR #12940 (pre-merge; doc otherwise baselined at `44d7538e`) | **The scan charge now counts the splits Vortex decodes concurrently, and the count finally means what it says.** The 2026-08-08 entry below describes a charge of one batch per output partition. A Vortex file scan does not decode one split at a time — it holds `scan_concurrency` of them in flight — so the pool was charged a fraction of what a scan materializes. Fixing that exposed a deeper problem: `ScanBuilder::with_concurrency` is a **per-worker** lookahead that Vortex multiplied by `available_parallelism()`, the machine's core count rather than the cgroup share, so the resolved number was never the number in flight. `off` still decoded one split per worker instead of serially, and a CPU ceiling placed on it bounded the multiplier rather than the width. spiceai/vortex#87 adds `with_absolute_concurrency`; the opener now requests it, so the resolved count is the count in flight and the charge, the `off` mode, and the entitlement cap all describe reality. `plan_decode_concurrency` sums each file-backed source's `scan partitions x concurrency` across the subtree, and the wrapper divides that total across its own output partitions with the remainder distributed — Cayenne inserts a round-robin `RepartitionExec` *beneath* the accounting wrapper, so a single-partition file scan commonly sits under `target_partitions` accounted streams, and charging each of them the whole total over-reserved by up to the core count (~4 GiB at first poll on a 64-core host for a table occupying one file, refusing queries that trivially fit). Memory-backed branches contribute nothing to the multiplier, which keeps a wholly in-memory plan out of the accounting as before and stops a mixed plan multiplying mem-tier batches — already reserved by `cayenne:mem_tier` — by a file branch's fan-out. That same walk now gates the accounting, absorbing the separate `plan_materializes_files` predicate the entry below introduced (a file-backed source with no files to read decodes nothing, so it contributes nothing): sizing the charge and deciding whether to charge cannot disagree about which branches decode when one walk answers both. **New config parameter:** `cayenne_scan_concurrency` (`auto` | `off` | `<n>`), the per-table lever for trading resident decode memory against scan throughput without shrinking query parallelism everywhere via `runtime.query.target_partitions`. `auto` derives from query fan-out and planned file count, capped at the CPU entitlement (`cpu_budget::scan_split_concurrency`) — the cap bounds only the derived value, so an explicit setting still outranks it. An invalid value **fails the dataset** with a structured error rather than falling back to `auto`, the widest mode, which would have turned a typo meant to reduce memory into an increase surfacing as pool exhaustion under load. Because the provider runs with the STORED `VortexConfig`, the field is reconciled on reopen (`reconcile_runtime_only_config`, extended from the cold-tier reconciliation) — without which the knob would be inert on every table that already exists, which is exactly when an operator reaches for it. Known limits, documented in the code rather than claimed away: the charge is spread evenly, so a lopsided plan charges each stream the average rather than its own branch; a mixed plan's mem-tier batches still flow through and are charged at the prevailing rate on top of their existing reservation; and the pre-poll charge remains one batch behind on the growing edge. Each wants charging at the materializing leaf. |
...
| 2026-08-08 | PR #12829 (pre-merge; doc otherwise baselined at `8a7f4645a5`) | **The maintained row count is withheld from the `Exact` fast path while a commit's delta is still queued.** `cached_table_statistics_for_optimizer` decided exactness from three in-memory conditions standing in for "rows the persisted count does not describe yet" — `has_pending_deletions`, a non-zero `inlined_row_count`, mem-tier tombstones — none of which is the delta itself, which `schedule_post_write_maintenance` hands to a debounced task. `clear_inlined_state_after_checkpoint` clears all three together and drains no maintenance, so a reader landing after a checkpoint on a pure-append table folded a count short by exactly the undrained delta into a distributed `COUNT(*)` answer: `prop_concurrent_mixed_dense_key_sqlite` caught `Exact(9)` over 11 live rows (#12824). `PostWriteMaintenance` now counts outstanding deltas directly (`outstanding_live_rows_deltas`), incremented under the state lock beside the delta it accounts for and retired only by a drain whose persist reports that it landed — `persist_table_stats`/`persist_table_stats_locked` return `bool` for that, because both of their bail-outs (an accumulator yielding no blob; a failed `upsert_table_statistics`) abandon `num_rows_update` while leaving `num_rows_exact` untouched. A count rather than a high-water mark, so an abandoned delta stays outstanding and a later drain's success cannot cover the gap. No schema or config change. Costs a real scan for a distributed `COUNT(*)` issued between a commit and the following maintenance debounce; unchanged outside that window. *Table statistics* section updated. |
| 2026-08-08 | PR for #12820 (pre-merge; doc otherwise baselined at `44d7538e`) | **The Turso metastore pool now guarantees autocommit at acquire time.** A connection returns to the K = 16 pool still inside `BEGIN CONCURRENT` whenever its holder could not finish the transaction — `TursoTransaction`'s `Drop` hands the `ROLLBACK` to a spawned task that never runs if the runtime is shutting down, and `commit`/`rollback` release the slot even when the statement they issue fails. Under `BEGIN CONCURRENT` that open transaction still holds its MVCC snapshot, so the *next* statement on the slot reads what the abandoned transaction saw rather than the committed database; the reads that matter are the non-transactional borrowers (`execute`, `execute_batch`, `query_row`, `query`), which serve the cold-tier GC root, where a stale live-file list becomes a deleted file the manifest still references. `begin_transaction` had defended only itself, with an unconditional `ROLLBACK` before its `BEGIN`. That defence moves into `TursoConnectionPool::acquire` and covers every borrower in one place: it clears an open transaction and then **confirms** autocommit, refusing the slot with an error if it cannot (a failed acquire is recoverable; a silent stale read is not). Net one fewer round trip on the hot path — `is_autocommit` is connection-local, so a clean slot pays nothing, while `begin_transaction` previously paid a real `ROLLBACK` every time. `SqliteMetastore` is unaffected (`BEGIN IMMEDIATE` takes no snapshot); this divergence is Turso-only. No schema or config change. *Metastore backends* updated. |
...
| 2026-08-07 | PR for DDL-partition interval compaction (pre-merge; doc otherwise baselined at `44d7538e`) | **A `CREATE TABLE … PARTITIONED BY` partition now runs an interval background compactor, on the process-wide budget.** Previously the DDL path built its `CayennePartitionCreator` with no background compaction, so a partition consolidated only while it kept being appended to — post-write compaction is write-triggered, so a partition that was written and then went quiet kept its small files for the table's lifetime and every later scan paid for them. `build_partitioned_provider` now attaches `with_background_compaction(provider::compaction_budget())`, the same process-global semaphore every accelerator-backed table draws on, so DDL-created partitions queue against the same ceiling instead of adding an unbounded second pool. Because those partitions belong to no accelerator, the `cayenne_compaction_permits_available` / `_total` gauges no longer read a handle published during accelerator registration (which left them silent in a DDL-only process) and instead read the global budget directly — removing the published `Weak` handle and its accessor. No schema or config-key change. *Flow F* updated accordingly, and its duplicated, stale semaphore description (which still said `available_parallelism()` and "per-accelerator") collapsed into one accurate paragraph. |
...
| 2026-08-13 | PR for #13014 (pre-merge; doc otherwise baselined at `44d7538e`) | **Documented the Vortex type rewrites, and stopped the acceleration write path reporting one as schema drift.** Cayenne stores `Float16` as `Float32` and every non-microsecond `Timestamp` at microsecond precision; the refresh sink compared the incoming schema against the accelerated one with no way to know that, so each refresh of a `PostgreSQL` `timestamptz` column warned that the acceleration was behind the source and told operators to set `on_schema_change` — a setting that cannot change what Vortex is able to store. The always-applied half of `transform_schema_for_vortex` is now also published as `CAYENNE_TYPE_REWRITE_RULES` — which the transform itself now applies, so the two cannot describe different rewrites — reached from the sink through a new `DataAccelerator::type_rewrite_rules` capability. Cayenne behavior, config, and schema are unchanged; *The on-disk layout* gains a paragraph on the stored types. |
