...
| `cayenne_datalake_warm_max_bytes` / `_max_files` / `_max_age_ms` | warm→datalake promotion triggers (size / file count / residency age) | 16 × `cayenne_datalake_target_file_size_mb` / (unset) / (unset) |
...
|------|-----------------|---------|
| 2026-08-09 | PR for the datalake residency-age trigger (pre-merge; doc otherwise baselined at `44d7538e`) | **A third warm→datalake promotion trigger, on how long warm data has been resident.** Promotion was gated only on warm bytes and warm file count, so a table whose volume never reached either threshold stayed in the warm tier indefinitely — correct for a capacity or storage-cost motive, but a silent functional gap when the datalake is the intended landing place for the data, and the only bound on how long a row exists solely on local disk. `cayenne_datalake_warm_max_age_ms` adds a residency bound as a third disjunct alongside the existing two, defaulting to `0` (off) so an existing configuration keeps promoting on size alone. Residency is timed from the first tiering tick that observes a warm generation, held in `warm_generation_first_seen` and cleared on a successful promotion, on an observed-empty warm tier, and on a no-op promotion; it is therefore accurate to within one `cayenne_datalake_tiering_check_interval_ms` and resets across a restart — the same process-local trade the RAM-tier age cap (`cdc_mem_tier_max_age_ms`) makes, and for the same reason: it keeps per-write timestamp bookkeeping off the CDC path. A snapshot-derived clock was rejected because compaction mints a fresh snapshot id for data that never left the warm tier, which would rejuvenate it on every compaction. Setting the age trigger alone deliberately leaves the default byte trigger in place, so a table that does reach the size threshold still publishes well-sized files. Caveat carried forward, shared with the existing `cayenne_datalake_warm_max_files` trigger rather than introduced here: clean cold files are never consolidated, so any trigger that fires often accumulates small datalake files, and `commit_overwrite_to_cold_in_txn` re-inserts every carried-forward file's statistics and bloom blobs on each commit, making per-promotion metastore work proportional to total file count. Folding undersized clean files into the rewrite set, and diffing the manifest instead of replacing it wholesale, are the follow-ups. Covered end-to-end through the runtime against real S3 (`test_cayenne_datalake_warm_max_age_e2e`, byte trigger set beyond reach so only age can promote) alongside the crate-level trigger tests. Registration now also warns that the datalake tier is in preview. No schema change. *The cold object-store tier*, *Flow: warm→cold promotion*, and *Appendix B* updated. |
