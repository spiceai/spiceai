...
At execution, a published **one-column** index can also probe a dynamic join filter after its build side has materialized. Equality, equal inclusive bounds, and a literal `IN` set are eligible; an `IN` set probes every member and unions the resulting row addresses. Composite index keys use this path only when every component resolves to one point, because independent value sets do not identify the tuples the join produced. This is automatic for an already-declared index and falls back to the ordinary scan when the filter is not an exact point or the index does not cover the scan's snapshot files.
...
**`mode: file`** (`lookup_index.rs`) indexes one snapshot's Vortex files. The serving structure is a sorted array of row-key hashes and packed `(file, row position)` postings; a hash collision only admits an extra candidate because the original SQL predicate is evaluated before the row is returned. BtrBlocks-compressed Vortex arrays of the exact keys and postings are retained for index verification. A write-time full refresh also retains the snapshot's Arrow batches when the query memory pool can admit them, so a point hit slices the retained row without opening the Vortex file. If the row store cannot be admitted, the postings still narrow an ordinary Vortex read to the candidate positions.
...
**Memory.** Both kinds are admitted against the query memory pool (`runtime.query.memory_limit`) through `CayenneMemoryAccount::try_reserve_lookup_index`, and show up as `cayenne_memory_account_bytes{kind="lookup_index"}`. The file-mode reservation includes its hash/posting arrays, compressed verification arrays, and optional retained Arrow rows; its build also reserves working memory as the `cayenne_index_build:<table>` consumer. If only the retained rows do not fit, point reads fall back to the indexed Vortex-file path. When the pool cannot fit the index itself, file mode does not publish it and memory mode reads that batch whole; results never change. An index's reservation is released when the index is dropped — replaced, stale, or with its table.
