...
|------|-----------------|---------|
| 2026-08-17 | PR #13126 | The seq-prefix bake now reports compaction telemetry under its own `kind="bake"` label, so bake passes are countable where they were previously invisible; it emits no `cayenne_compaction_merged_bytes`. *Maintenance: seq-prefix bake* updated. |
