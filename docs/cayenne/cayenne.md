...
An accelerator-only table serves its writes locally; a table configured for durable write-back (`write_mode: write_back`, which additionally requires dataset-level `replication.enabled: true`) also reconciles committed writes back to its federated source. A committed write records the primary keys it touched in the `cayenne_pending_write_back` marker table **inside the same commit `MetastoreTransaction`** — never on the CDC apply path, so an echo of Cayenne's own write cannot spawn a delivery. Each marker also records **which operation dirtied the key** (`op`: upsert or delete), and that recorded intent is what delivery acts on. Markers are key-addressed, so durable write-back requires key-based deletes: registration rejects an explicit `cayenne_deletion_mode: position` (whose file-position tombstones cannot express them); the CDC-with-primary-key `auto` resolution already lands on `key`.

A per-table delivery worker drains the markers: it claims a batch, point-scans those keys' *current* committed values from the accelerator, and reconciles each key to the source idempotently. A key the scan returned whose marker records an upsert is upserted with that value. A key the scan did not return is deleted at the source **only if its marker records a delete**; one carrying an upsert marker is not delivered at all. A key the scan *did* return whose marker records a delete is **deferred** — nothing is delivered and the marker is withheld from the clear — because that combination is ambiguous: either a later commit re-created the key (whose own marker bumped the sequence, so the next pass delivers the new value), or the delete is committed but not yet scan-visible (a commit publishes its marker before the caches and mem tier reflect the delete, and delivery takes no lock against a writer). Guessing "re-created" would upsert the row back and clear the marker, losing the delete permanently; deferring converges in either case on the next pass. Absence is deliberately never read as a deletion: a row can be missing simply because the read could not see it, and issuing a source `DELETE` on that evidence would destroy rows nobody deleted. Only a user `DELETE` marks its keys for deletion — a CDC-applied delete is the source replaying a delete it already performed, and a retention prune bounds the accelerator without making any claim about the source.

**Nothing but delivery retires a marker.** A marker is the only durable record that an acknowledged write has not reached the source, so dropping one because a pass could not read its key silently loses that write — the source keeps its previous value and nothing remains to reconcile it. No retry bound makes that safe either, since a miss that has recurred a hundred times is no more evidence the row is gone than the first was. An unreadable key is therefore a condition to wait out rather than evidence to act on, and the accelerator is expected to hold every write it acknowledged. Retention is the one mechanism that legitimately removes a live row, so a durable-write-back dataset **refuses to configure it** — `retention_period` or `retention_sql` on such a dataset is rejected at registration rather than degraded, because ignoring retention would grow the accelerator without bound and ignoring write-back would lose writes. Time-based retention is not even a deletion the worker could observe: Cayenne applies it as a read-time keep filter, so an expired row stops being readable while it is still physically present.

Because markers persist indefinitely, an undeliverable one must not monopolize the claim. Each marker carries its own delivery schedule (`delivery_attempts`, `last_delivery_attempt`, both reset when a newer commit re-marks the key, since fresh data must not inherit failure history), and a pass draws from **two queues on separate budgets**: a *fresh* queue of markers never attempted plus the one immediate retry a first miss is owed (`delivery_attempts <= 1`, oldest commit first), and a *deferred* rotation of everything above that, ordered least-recently-attempted first. The rotation ordering is by timestamp rather than attempt count so that waiting longer monotonically improves a marker's position — a count would let a trickle of newly-deferred keys jump ahead of long-suffering ones indefinitely. Both queues are delivered as one batch: the split governs what is claimed, not how it is sent. A non-empty rotation is surfaced as a rate-limited warning naming the dataset and the worst attempt count, since it means acknowledged writes are not landing.

The worker is the **sole owner** of that delivery. A write-back accelerated table otherwise forwards a non-transactional `DELETE` to the source itself, fire-and-forget, in the background; on a durable-write-back table that forward stands down, because the same mutation arriving through two unordered paths can have the late one delete a row the other had already re-created — after its marker was cleared, so nothing remains to reconcile them. The marker path is also the durable one: the fire-and-forget forward is lost outright if the process dies.

The worker then clears only the markers it actually delivered, and only those whose stored `sequence_number` is still at or below what it claimed (a newer commit that re-dirtied a key leaves its marker in place, so the next pass delivers it). Everything undelivered keeps its marker and is charged one attempt, under the same sequence bound — an in-flight pass must not charge a commit that superseded the one it was working on. Delivery failure never blocks accelerator commits; the dirty set simply grows until the next successful pass. v1 delivers single-column primary keys.
...
| `cayenne_pending_write_back` | durable-write-back dirty-key markers: one row per committed-but-undelivered primary key (`WITHOUT ROWID`; raw-UUID-byte `table_id` + `RowConverter`-encoded `pk_bytes`; `sequence_number` of the commit that dirtied it; `op` = whether that commit upserted or deleted the key, moving with the winning sequence; `delivery_attempts` + `last_delivery_attempt` = the two-queue delivery schedule, reset by a winning re-mark, indexed partially over the deferred rotation so the commit path pays for no index a healthy table does not use); the delivery worker claims, reconciles to the federated source per the recorded `op`, and clears only what it delivered |
