...
An unset `refresh_mode` is filled in by the *connector* (`DataConnector::resolve_refresh_mode`), not by a fixed default — `debezium` and `cdc` resolve it to `changes`, `sink` to `disabled`, everything else to `full`. The runtime builder's host-memory sizing applies those same connector defaults, so an unannotated `debezium:` dataset is budgeted as the CDC stream it is. The per-table profile in the table above is read from the *declared* `refresh_mode`, which the connector-resolved value is never written back into, so a dataset that relies on the connector default is configured as `full`; spell `refresh_mode` explicitly to pin the profile you want.
