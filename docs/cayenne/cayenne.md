...
- **Freshness is per-scan, derived from the dataset's `access` mode** (`default_scan_freshness`). A **read-write** dataset uses `0` = **read-your-writes**: the scan captures the CURRENT state, keys it, and serves an exact-key completed view or builds one — so a scan always sees its own writes (fixing the maintainer's `INSERT OVERWRITE` read-your-writes regression, where a scan served a bounded-stale published bundle that predated the overwrite). A **read-only** dataset (no user DML reaches it — writes require both a ReadWrite API key and `access: read_write`; its data moves only via refresh/CDC, eventually-consistent within the freshness SLO) uses a bounded lag (`CAYENNE_SCAN_VIEW_FRESHNESS_MS`, default 1000 ms): a fast path serves `latest_complete` when it is within the lag AND its structural generation still matches, letting concurrent analytical scans share one build without a capture or fence — the cross-query reuse that relieves the per-version rebuild CPU competing with the CDC apply. On a miss (or read-your-writes) it captures the current state and builds. The read-current fast path is gated on the structural generation so a stale-tolerant scan is **never** served a pre-schema-evolution bundle. `scan_input_version` remains as the cache's **ordering axis** (which completed build wins the `latest_complete` slot) and a cheap freshness signal — not a correctness gate.
...
- A background **evictor** (`run_scan_view_evictor`, a 5 s tick holding only a `Weak<Self>`) drops `latest_complete` once it ages past the dataset's freshness bound — i.e. once it is already non-servable, so eviction never costs a reusable view. This releases the `SnapshotScanRef` a cached bundle holds so compaction GC can reclaim its snapshot dirs even on a table that was queried once and then went idle (the cache only refreshes `latest_complete` on a scan miss, so with no scans nothing else would ever release it). The provider records a `Weak<Self>` (`init_scan_view_cache`) so a build closure — which is `'static` and cannot borrow `&self` — can obtain an owned `Arc<Self>` for its `spawn_blocking`; a provider never `Arc`-wrapped (unit-test path) builds inline instead.
...
|------|-----------------|---------|
| 2026-07-23 | `b461cb3f5` (pre-merge, branch `ben/scan-view-cache`; supersedes the #11948 maintainer approach) | **Demand-driven scan-view cache** replaces the #11948 background maintainer. A `ScanView` is now built **only for a scan-visible state a scan actually queries** (`ScanViewCache`, two slots — `latest_complete` + a `Shared`-future `in_flight` dedup list — behind a `parking_lot::Mutex`; build in `spawn_blocking` with an independently-spawned driver; decide-under-lock, await-outside), not eagerly per version. This fixes the maintainer's **read-your-writes regression** on `INSERT OVERWRITE` (`test_overwrite_operations`): freshness is now per the dataset's `access` mode via `default_scan_freshness` — read-write → `0` (read-your-writes: capture the current state, so a scan sees its own writes); read-only → a bounded lag (`CAYENNE_SCAN_VIEW_FRESHNESS_MS`, default 1000 ms) within which concurrent analytical scans share one cached view (cross-query reuse relieving the per-version rebuild CPU that competed with the CDC apply). The `StructuralVersion` seqlock is **retained** (the schema-evolve flush re-enters `listing_fence.write()` in `checkpoint_*`, so it cannot be fence-serialized): the read-your-writes capture runs under `read_validated_async` so a capture straddling a live schema-evolution is retried rather than built torn, and the generation folds into `RawScanInput::key`; the read-current fast path is gated on it so a stale-tolerant scan is never served a pre-evolution bundle. A background **evictor** (`run_scan_view_evictor`, 5 s tick, `Weak<Self>`) drops an idle `latest_complete` past its freshness bound, releasing the `SnapshotScanRef` so compaction GC can reclaim snapshot dirs on an idle table. Retires the maintainer loop, the published `ArcSwapOption<ScanView>`, `RawScanInput::changed`, `CAYENNE_SCAN_VIEW_REFRESH_FLOOR_MS`, and the `cayenne_scan_view_builds_total` / `_waits_total` metrics; `scan_input_version` remains only as the cache's ordering axis. SF1000 CH-benCHmark (`postgres-cayenne[file]-adaptive.yaml`) vs the last green trunk run: QPH +9.4%, tpmC and replication lag flat, all gates passed. Updated *Other managed state*, the seal memo-transparency note, the scan overview, and the cheat-sheet. |
