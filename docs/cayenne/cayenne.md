...
This document is a point-in-time snapshot of a fast-moving crate. Each entry records the date, the merged pull request (or `trunk` commit) reviewed, and what changed here — so a future revision can be tied to the specific repository changes that prompted it. When updating, add a new row (newest first); when the revision re-checks the document as a whole against a commit, also refresh the baseline commit reference in *Scope and sourcing* at the top.
...
**One row per merged pull request, not one per revision of it.** A change that is revised while still in review updates its existing row rather than adding another, so the table reads as a list of changes rather than a list of review rounds.

**A row earns its place by recording something a reader would act on**: a new or renamed parameter, a schema change, a behavior or correctness change, or a structural revision to this document. Leave out what the document does not describe — an internal refactor, perf work with identical behavior, or a fix to unreleased work already covered by its own row.
...
| 2026-08-30 | PR #13574 | A `retention_sql` predicate is an engine-level delete filter that runs again after every replace, unlike `retention_period`, which Cayenne injects as a scan-time keep filter; the distinction is what the predicate-`DELETE` fix on upsert tables turns on. |
| 2026-08-30 | PR #13720 | A commit claims its live-row delta (`OutstandingLiveRowsDelta`) before it publishes rows and marks the claim published once they are visible, so the row-count exactness gate covers the publish instead of starting after it. |
| 2026-08-30 | PR #13133 | A partitioned acceleration pins `SchemaEvolutionMode::Disabled`, so a widened CDC batch it cannot apply is refused rather than routed through the fallback meant for engines a restart repairs. |
| 2026-08-30 | PR #13695 | A `Map` is stored under the entries declaration the Arrow layout requires (non-nullable `entries`), not the one its producer sent, so the stored and compared schemas agree. |
| 2026-08-28 | PR #13651 | The checkpoint also reclaims inline tombstones — an upsert that cannot locate the row it supersedes leaves one — and returns the freed metastore pages. |
| 2026-08-28 | PR #13663 | The orphaned deletion-vector sweep is scheduled from every publication that can orphan one, keyed on the surviving-sequence floor; it proves eligibility under `listing_fence.read()` and releases the fence before unlinking anything. |
| 2026-08-27 | PR #13593 | A catalog carries its own `cayenne_data_dir` / `cayenne_metadata_dir` pair, defaulting to structurally disjoint paths, and is refused at load when the data directory would hold its metastore, on the same terms as an acceleration. |
| 2026-08-26 | PR #13567 | `Map` columns store in a file-mode accelerator (Vortex aliases the type to `List<Struct<keys, values>>` on write); the frozen list of types Vortex cannot encode is `Interval`, `Duration`, `FixedSizeBinary`, `Union` and `RunEndEncoded`, and a test probes Vortex per Arrow type family so the list cannot drift again. |
| 2026-08-26 | PR #13300 | Segment-cache retirement bounds both of its waits at ten seconds, so a stuck put cannot hold a delete or overwrite open; a put that lands after the enumeration removes its own entry once it sees the path retired. |
| 2026-08-26 | PR #13350 | `PostgreSQL` write-back delivers through a connector-owned `WriteBackDeliverer` that stamps each delivery with its transaction id and registers it for the CDC pump's echo filter (`XidRegistry`). Known limitation: a source-side trigger can rewrite an untracked row out from under that filter. |
| 2026-08-25 | PR #13471 | `cayenne_metadata_dir` is listed among the parameters: it must not sit inside any acceleration's data directory, and a teardown that would delete a metastore no parameter names is refused. |
| 2026-08-25 | PR #13502 | Write-back delivery uses a native upsert (`InsertOp::Replace`) for the keys the read returned plus a delete for those it no longer returns, falling back to delete-by-PK-then-insert only against a source that cannot upsert; the `dataset_acceleration_write_back_pending_keys` gauge reports the per-dataset backlog. |
| 2026-08-25 | PR #13373 | A rewrite's shard bounds come from the per-partition bands its inputs report, cut at equal cumulative row mass, *when* those bands describe the key distribution (`range_bounds_from_histogram`). Without that evidence a single-column shard key falls back to equi-width bounds over the merged range (`range_bounds_from_statistics`), while a composite key takes range bounds only when histogram-derived cuts fill every shard and otherwise hashes (`range_split_is_worth_taking`); a key with no usable range statistics hashes as before. |
| 2026-08-23 | PR #13391 | The Cayenne accelerator moved into its own crate; the reference to `apply_memory_mode_overrides` now points at `crates/accelerators/accelerator-cayenne/src/lib.rs`. |
| 2026-08-21 | PR #13356 | Encode fan-out is decided by the write, not the table's declared `cayenne_sort_columns`: a rewrite that actually sorted its stream stays serial (`rewrite_write_policy`), while CDC delta writes and protected-snapshot merges fan out. A fanned-out write shards by `cayenne_shard_key_columns` (else the primary key): a rewrite range-partitions on what its inputs report, a streaming CDC write hashes. |
| 2026-08-20 | PR #13343 | While a pending mem-tier delete exists, both protected-snapshot consolidation passes fold only snapshots whose merge-on-read threshold is at or below the pass's delete fence — the size-tier subset merge filters its candidates, and the seq-prefix bake stops its prefix at the first above-fence input — so a fold can no longer mask rows newer than the fence (previously a key's live version could vanish from scans after a pass, which durable write-back escalated into a source-row delete). |
| 2026-08-20 | PR #13325 | A write that must not fan out now says so explicitly (`EncodeFanOut::Serial`) instead of asking for a single partition, so a configured `cayenne_write_concurrency` can no longer override it — the protected-snapshot merge relies on this for position-delete tables, and the Z-order-clustered cold promotion for its clustering. The partition count remains a hint, so a low `runtime.query.target_partitions` (or a small cluster executor-slot count) does not serialize ordinary CDC, DML, staged or overwrite writes. |
| 2026-08-20 | PR #13200 | Append overlap de-duplication now reconciles declared engine type rewrites with the accelerated table's stored representation, so an engine-rewritten type does not fail a valid append refresh as a schema mismatch for either legacy or current tables. *The on-disk layout* updated. |
| 2026-08-18 | PR #13057 (fixes #12999) | A partitioned dataset now resolves `schema_evolution` to `Disabled` (alongside the existing `refresh_mode: caching` exclusion), so the open path can no longer widen the parent catalog entry's stored schema while every partition child keeps its own — a mismatch that read back as a silent narrowing cast. `drop_table` also removes a partitioned table's child rows and parent row in one metastore transaction, and both teardown paths drop catalog metadata before deleting files, so a failed drop can no longer leave rows describing deleted files. A partitioned dataset also no longer publishes snapshots: the exported metastore slice does not yet include partition child tables, so an uploaded archive could not be restored once the cascade has removed the live child rows. |
| 2026-08-18 | PR #13219 (closes #13211) | The PK existence bloom is now split-block — one hash selects a 256-bit block and sets one bit in each of its eight lanes, so a probe costs one cache line instead of up to seven — and `with_expected_keys` rounds its bit count up rather than down, delivering the ~10 bits/key it asks for instead of as little as 5.24. Measured at SF1000: +48.8% QPH across two pairs, with replication lag and tpmC unchanged. |
| 2026-08-16 | PR #13180 (fixes #13018) | A table now stores the timestamp unit its source reports — Vortex represents all four Arrow units, so the creation-time rewrite to microseconds is gone and `Float16` to `Float32` is the only unconditional one. A `PostgreSQL` `timestamptz` accelerates as nanoseconds instead of being silently narrowed on every refresh. `CAYENNE_TYPE_REWRITE_RULES` keeps the old normalization for the write path only, so a table created earlier still replicates instead of failing as a schema change. *The on-disk layout* updated. |
| 2026-08-15 | PR #13153 (fixes #13137) | Every persisted PK bloom — the per-cold-file `pk_bloom_blob` and the `cayenne_pk_index` sidecar alike — now carries a magic, a format version, and a fingerprint of the probe function that filled it, so bits placed by a different hash, seed, probe count or layout are rejected into the exact-scan fallback instead of probed as if they were this build's. Blooms written before the frame stay readable, so no upgrade pays a rebuild. |
| 2026-08-13 | PR #12998 (fixes #12846) | A datalake promotion now re-baselines the maintained row count from the manifest it commits, and a standalone `DELETE` taints the persisted `num_rows_exact` before it tombstones, so no tombstone fold can restore `Exact` over a count a delete has staled. *Table statistics* updated. |
| 2026-08-13 | PR #13074 (fixes #13014) | The two type rewrites a table applies at creation — `Float16` to `Float32`, and any non-microsecond `Timestamp` to microseconds — are now published as `CAYENNE_TYPE_REWRITE_RULES` and read by the acceleration write path through the new `DataAccelerator::type_rewrite_rules`, so a type the storage format imposes no longer reports as a schema the acceleration is behind on. New paragraph in *The on-disk layout*. |
| 2026-08-12 | PR #12940 | New parameter `cayenne_scan_concurrency` (`auto` / `off` / `<n>`), the per-table lever trading resident decode memory against scan throughput. The resolved value is now the number of splits actually in flight, and the query pool is charged for all of them rather than one batch per output partition. |
| 2026-08-12 | PR #12983 | The Vortex segment cache is now one process-wide budget shared by every Cayenne table, rather than a per-table cache sized from host memory, so adding a table divides a fixed pool instead of reserving another full-sized cache and shrinking every query's budget. Retired files are evicted from it by exact path once cleanup proves them dead. |
| 2026-08-12 | PR #12949 | `auto` is now the tuning mode unless a dataset sets `cayenne_tuning: adaptive`; a `cayenne_goal_*` SLO no longer implies the closed loop. The mem-tier knobs also accept the literal `auto` as "unset". |
| 2026-08-08 | PR #12829 (fixes #12824) | The maintained row count is served `Inexact` while a commit's delta is still queued, so the optimizer cannot substitute a count that does not yet describe the table. |
| 2026-08-08 | PR #12821 (fixes #12820) | The Turso metastore pool now clears any open transaction and confirms autocommit at acquire time, so a borrower cannot inherit an abandoned `BEGIN CONCURRENT` snapshot and read stale rows. |
| 2026-08-08 | PR #12759 | Scan materialization — the largest unaccounted allocator at scale — is now charged to the query pool, gated on a plan that actually reads files, and refused with a `ResourcesExhausted` error naming the dataset and the remedy. |
| 2026-08-08 | PR #12833 (fixes #12766) | A Cayenne catalog now applies its `include`/`exclude` patterns at both points a table name can enter it; DDL registration is deliberately not filtered. New *Catalog visibility* section. |
| 2026-08-07 | PR #12757 (fixes #12617) | A `CREATE TABLE … PARTITIONED BY` partition now runs an interval background compactor on the process-wide compaction budget, instead of consolidating only while it was being written to. |
| 2026-08-07 | PR #12711 (fixes #12708) | A carry-forward rewrite now reads the manifest rows it classified, rather than intersecting them with the scan's later capture, closing a silent row-loss window (#12708/#12709). |
| 2026-08-05 | PR #12577 | A cross-tier read now captures the cold manifest in the same fenced instant as the warm snapshot, so a scan straddling a promotion can no longer count promoted rows twice. |
| 2026-08-04 | PR #12541 (fixes #12013) | Both protected-snapshot merge paths now cap selected input bytes at 4x the memory pool the pass accounts against, so an arbitrarily large run set can no longer OOM the host mid-merge. |
| 2026-08-02 | PR #12367 | A small whole-table replace now lands in the metastore inline tier instead of writing Vortex files nothing would merge, together with the visibility rules that keep the overwrite gap serving a complete table. Partitioned datasets are refused, and the inline corpus is adapted to the live schema on decode. |
...
| 2026-07-12 | PR #11803 | Atomic cross-partition appends: one shared catalog transaction for every participant's state, WAL recovery classified solely from durable catalog pointers, cancellation-safe deletion-vector ownership, and full state rehydration after an ambiguous commit. Recovery also stopped classifying a protected-snapshot commit from the current-snapshot pointer alone, which dropped durably-committed rows. |
| 2026-07-10 | PR #11806 | Lazy NDV is now unconditional and the `SPICE_CAYENNE_EAGER_NDV` escape hatch is removed, so *Table statistics* describes the lazy split and its bounded one-memtable lag rather than a per-row fold on the inline write. |
| 2026-07-09 | PR #11745 | Cold-tier promotion rewritten from whole-table re-materialization to incremental carry-forward, with per-file PK blooms serving keyset rebuilds, mark-and-sweep GC of orphaned objects, and the `cayenne_datalake_*` parameter rename. |
| 2026-07-08 | `4685a3dd` (committed 2026-07-08, `trunk`) | Moved the doc in-tree to `docs/cayenne/` and re-baselined. Folds in metastore schema versioning (#11651), opt-in integrity checksums (#11646), bounded append freshness (#11620), the durable CDC delete fast path (#11642), and the durable-path count taint (#11643). |
| 2026-07-05 | `1f8833ba` (committed 2026-07-05, `trunk`) | Folded in the five cayenne PRs merged since `5f61351`. The headline is the mem-tier seal (#11622): a periodic seal durably shadows the active piece and advances the replication slot, decoupling the slot ack from reads and the checkpoint cadence. |
| 2026-07-04 | `5f61351` (committed 2026-07-04, `trunk`) | Folded in the five cayenne PRs merged since `7aaebc4`: freshness-SLO-driven adaptive mem-tier shrink, cold PK-keyset rebuilds that fold in un-checkpointed mem-tier keys, and always-on orphaned-DV cleanup (parameter removed, breaking). |
| 2026-07-02 | `7aaebc4` (committed 2026-07-02, `trunk`) | Re-baselined to trunk from `64451e0`, folding in the optional cold object-store tier with Z-order clustering (#11543), the off-`write_lock` mem-tier checkpoint and adaptive query-admission throttle (#11538), and storage-aware adaptive CDC tuning (#11463). |
| 2026-06-30 | `64451e0` (committed 2026-06-29, `trunk`) | Initial version. Covers the three-tier architecture, the concurrency and lock model, the fused deletion subsystem, the key flows, the comparison to Iceberg / Delta Lake / Hudi / DuckLake, and the 11-table metastore schema. |
