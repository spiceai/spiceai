...
`SqliteMetastore` (the default) runs `tokio-rusqlite` in WAL journal mode with `synchronous = NORMAL`, foreign keys on, a 30-second `busy_timeout`, and a round-robin **connection pool** of `K = min(cpu_budget().cores(), 32)` independent connections (floor 2). SQLite WAL serializes writers at the engine level but allows concurrent readers; the pool primarily lifts read-side concurrency for metadata-heavy scans, where each scan pays several metastore reads. Within a process, every writer of a metastore file first queues on that file's **writer gate** — a FIFO `tokio::sync::Mutex` shared by every `SqliteMetastore` open on the file — and holds it exactly as long as the write lock: transactions (`BEGIN IMMEDIATE` through `COMMIT`/`ROLLBACK`), autocommit writes, an `UPDATE … RETURNING` issued through the query path, the TRUNCATE checkpoint, and the incremental vacuum. SQLite's own busy handler grants the lock in no particular order, retrying on a sleep backoff of up to 100 ms, so a writer that re-takes the lock back to back could starve the others until they failed with `database is locked`; the gate grants it in arrival order and wakes the next writer as soon as the previous one releases, leaving the busy handler to arbitrate only between processes. A writer takes the gate before a pool connection, so writers waiting their turn hold no connection: a writer parked in SQLite's busy handler would hold one, and enough of them leave no connection for a read. A wait on the gate is bounded by the same `busy_timeout` and ends in exactly the error the busy handler would have returned on that path, so retry handling is unchanged. Reads never take the gate. `begin_transaction` holds an `OwnedMutexGuard` on one pool slot for the transaction's lifetime. `TursoMetastore` (optional, `turso` feature) uses a fixed `K = 16` pool and libSQL's MVCC journal mode. Because `BEGIN CONCURRENT` pins an MVCC snapshot for the life of a transaction, that pool guarantees **autocommit at acquire time**: a slot whose previous holder returned it mid-transaction is rolled back before it is handed over, and is refused outright — `acquire` returns an error rather than a connection — if it cannot be confirmed back in autocommit. Without that, the next statement on the slot reads the abandoned transaction's snapshot instead of the committed database, so a reader can miss rows another connection has already committed. `SqliteMetastore` needs no equivalent: `BEGIN IMMEDIATE` takes no snapshot.
