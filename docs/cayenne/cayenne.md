...
**A retention predicate runs again after every replace, and it costs the inline tier.** `retention_sql` and `retention_period` are engine-level delete filters, and an overwrite reloads every source row — so whatever the predicate deleted last time returns with the refresh unless it runs again over the new snapshot. The overwrite therefore arms the same debounced post-write maintenance an append does, and the delete lands once the overwrite's `write_lock` is released; applying it inline would deadlock against the deletion sink, which takes that same lock. That sink reads the table's Vortex files, and an inlined overwrite has none, so retention flushes the corpus to Vortex before it scans. A table carrying retention filters consequently gives up the inline tier's cheap whole-table replace — one file per refresh instead — which is the same trade the append path already makes by refusing inlining outright for these tables.
