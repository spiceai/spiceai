...
An index answers only a lookup whose filters pin **every** column of one key to a literal, compared with the bare column (`WHERE tenant_id = 7 AND service_id = 'a'`). A cast on the column side, a range, an `IN` list or an `OR` scans as before. Floating-point columns (`Float16`, `Float32`, and `Float64`) cannot be index columns: their equality semantics include values such as signed zero that do not have a single byte representation, so Cayenne rejects the index definition at registration. Use an integer, decimal, string, or another exact-equality column instead.

The index only narrows what is read — every original predicate still runs on the candidate rows — so it can never add a row, and a lookup it cannot serve simply scans. `EXPLAIN` reports the decision on `CayenneAccelerationExec` as `lookup_index`, `lookup_index_outcome`, `candidate_files` (file mode), and `candidate_rows`; an indexed table whose predicate does not pin a complete key reports `lookup_index=none, lookup_index_outcome=not_applicable`. Each actual probe is also counted on `cayenne_lookup_index_probe_total{table, shape, outcome}`, with `outcome` one of `selected`, `empty` (the indexed snapshot has no candidate for the key), `unbuilt` (no index covers the rows read) or `snapshot_mismatch` (the index no longer matches the table's files). In file mode, newly appended or still-inline rows are scanned separately until they are included in a rebuilt index, so candidate counts describe the index-pruned part of the plan rather than the whole table.
...
- **Composition.** A row selection is intersected with the table's position-delete vectors (`LookupAccessPlanProvider`), so a deleted row is never selected, and a scan carrying a selection reports inexact statistics.
