...
**A `Map` is stored under the declaration the Arrow layout requires, not the one its producer sent.** Arrow forbids a map's `entries` field from being nullable and `MapArray::try_new` refuses one, but nothing enforces it on the way in — so a source that declares `entries` nullable yields a column that decodes cleanly and then fails in whichever kernel first rebuilds it, reporting `MapArray entries cannot contain nulls` whether or not a null is involved. Since the stored declaration is what every read is planned against and what the write sink casts each incoming batch to, a table persisted under the illegal form could never be read and would not recover on its own: the rule that keeps a stored schema canonical across a nullability difference is exactly what holds it in place. `create_table` therefore brings the requested schema into line before it persists or compares anything, `get_table` repairs a stored declaration on the way out — which heals such a table on its next load — and `evolve_schema_live` conforms a widening plan's evolved schema before it writes the metastore or swaps the live one, so a column added mid-stream cannot reintroduce the declaration on a table that was readable a moment earlier. Every one of these is metadata-only — nullability lives in the type rather than in any buffer — so no file is rewritten. The CDC comparison is conformed on the incoming side for the same reason: `widen_type` has no `Map` arm, so a source that keeps sending the forbidden declaration would otherwise differ from the stored one on every batch and classify `Incompatible`, stopping replication under `on_schema_change: fail` over a schema that never changed.
