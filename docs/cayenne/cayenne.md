...
The cgroup-aware total behind these process-wide ceilings is sampled on the blocking pool while the runtime builds and again during every app-reload memory preflight, including an otherwise-identical apply. Reload planning charges the fixed query/compaction pools, the identity-retained ceilings of active or cached DuckDB database pools, and the Cayenne cache reservation. Only the DuckDB aggregate is a high-water; the Cayenne reservation is republished each reload. The Cayenne reservation is tracked per component, each at the ceiling its provider was created with, so a component a reload replaces charges both its old and its replacement provider while the components beside it are charged once. Charging both is required where a reload builds an accelerated dataset's replacement before dropping what it replaces, and is deliberately conservative on the paths that drop first — a replaced view, and the non-hot-reload dataset path — where the two never coexist. The overlap charge stands only while the apply runs: once the dataset and view diffs have run, the reload republishes the figure the applied app *declares*, so a replacement or removal is released rather than throttling the in-memory tier until some later reload happens to recompute it. Releasing it is conditional on the diffs having realized what the app declares. A rejected snapshot-consistency check, a component whose spec no longer builds, an accelerator that fails to initialize, and a removal that cannot be parsed each leave a provider the new app does not describe still holding its caches; the dataset and view diffs report whether every declared component was reconciled, and the reload settles only when they both did. Otherwise the higher figure charged for the swap stands, with a warning, until a later reload succeeds — reapplying the same app is a no-op, so an under-reservation taken on that assumption would never be corrected. The live tier sampler subtracts the part of the resulting reservation above the standard host/10 headroom before it lets a query-light tier float upward. A newly introduced DuckDB identity receives only what remains after cached pools, down to the 128 MiB floor, instead of splitting memory those pools still own. The dynamic-tuning and fleet-wide PK-keyset ceilings are refreshed from the same total before it is release-published to the sampler. The query and compaction pools themselves remain fixed until restart; when a pod retaining no DuckDB reservation no longer fits its fixed pool beside the current limit and Cayenne reservations, the runtime warns that a restart or smaller limits are required — unless the pool was pinned by an explicit `runtime.query.memory_limit`, which a restart would reproduce verbatim. Lowering the PK-keyset ceiling preserves the bytes already charged by live table accounts, so the new limit bounds subsequent growth without hiding resident memory.
...
|------|-----------------|---------|
| 2026-08-13 | PR #12881 (pre-merge; doc otherwise baselined at `44d7538e`) | **Cgroup-aware accelerator ceilings now refresh coherently during app reloads.** The runtime carries one shared current-total value from startup through reload planning instead of leaving a build-time scalar behind, and probes host/cgroup memory off Tokio workers. Every apply preflight — even an otherwise-identical app — retains active/cached DuckDB ceilings by typed database identity, charges them before sizing a newly introduced pool, and holds the new pool to the remaining room or its 128 MiB floor. Cayenne reservations are tracked per component at the ceiling each provider was created with, charging old plus new only for the components a reload actually replaces — judged by the same equality the reload diff uses, so an edit that recreates nothing cannot double-charge the fleet — and the old-plus-new overlap is charged only for the duration of the apply, with the figure the applied app declares republished once the diffs have run, so a replacement or removal is released rather than held until restart — and released only when the dataset and view diffs report that every declared component was reconciled, so a failed diff leaves the higher figure standing rather than freeing caches a live provider still holds; the live in-memory-tier sampler subtracts reservation excess above the standard headroom so query-light floating cannot reclaim cache memory. The dynamic-tuning and fleet-wide PK-keyset ceilings refresh before the same total is release-published to the sampler. Query and compaction pools remain fixed until restart, so any cgroup/app change that leaves fixed pools and off-pool reservations outside the coordinated region produces explicit OOM/restart guidance rather than claiming a live resize. The PK-keyset budget preserves already charged usage when its ceiling changes. No schema or config change. *Appendix B* updated. |
