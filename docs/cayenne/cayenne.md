...
|------|-----------------|---------|
| 2026-08-20 | PR #13329 | `cayenne_force_view_types` now maps `LargeUtf8` to `Utf8View` as well as `Utf8`. It covered only `Utf8` before, which left the flag inert on the string type most sources deliver — a 100M-row `ClickBench` load arrives entirely as `LargeUtf8`, so setting the flag changed nothing. Measured on that load: 1.14x across the 35 runnable queries with no regression, concentrated in the string group-bys (`GROUP BY "URL"` 1.36x, `AVG(length("URL"))` 1.51x). Mapping `LargeUtf8` narrows the per-value ceiling to just under 4 GiB (an Arrow view's `length`/`offset` are `u32`), which is why it stays behind the opt-in flag rather than becoming the default. |
