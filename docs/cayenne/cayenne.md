...
Because the buffered batches and the serialized blob stay resident until the overwrite publishes, admission is claimed through a single non-blocking slot on the shared `CayenneContext`. A partitioned dataset's children share that context, so N concurrent partition overwrites hold at most one buffer between them — which is what makes the runtime's host-memory reservation of one buffer plus one entry per acceleration true rather than merely generous. The slot is never awaited: partition children are coupled writers fed by one demux, and parking there would stall the router (spiceai/spiceai#11818). A child that cannot claim it writes Vortex files instead. Inlining is also skipped outright when the table holds a mem-tier seal shadow — a durable inline row deliberately kept above the watermark while its rows are still live in RAM — since raising the watermark past it would serve those rows twice; only `cdc_durability: memory` can produce one, which the bulk-overwrite profile never selects.
...
An unset `refresh_mode` is filled in by the *connector* (`DataConnector::resolve_refresh_mode`), not by a fixed default — `debezium` and `cdc` resolve it to `changes`, `sink` to `disabled`, everything else to `full` — and the classification honours that on both the pre-init and post-init paths.

The profile also drives host-memory sizing outside the accelerator: only a pod containing a small-write table reserves room for the in-memory CDC tier (`cdc_durability` is forced to `file` on the other profiles, so the tier is unreachable), and only a pod containing a compacting table carves the dedicated compaction memory pool and thread pool.
