...
|------|-----------------|---------|
| 2026-07-16 | PR #11907 (pre-merge) | **Cold-promotion checkpoint serialization** (duplicate-row over-count fix). A cold promotion now serializes its pre-graduation mem-tier checkpoint on `mem_checkpoint_lock`, so it can no longer re-capture and re-publish a background checkpoint's captured-but-not-yet-cleared segments — the former durable over-count on cold-promoted tables. Capture-lock acquisition is split into a **guaranteed** fair path (fast `mem → try(write)`, then a `write → mem` queued fallback on contention — deadlock-safe vs the write-path spill/promotion, non-starving) and a **best-effort** `try`-both path returning `Busy` (retry next tick). Periodic callers map by urgency: age-due / critical-pressure / flush-every-tick checkpoints and due seals use the guaranteed path; a size-triggered checkpoint with age headroom is best-effort. `checkpoint_mem_tier` keeps its guaranteed `Result<u64>` contract; adds `try_checkpoint_mem_tier -> CheckpointAttempt{Completed, Busy}`. Updated the seal/checkpoint capture lock-hierarchy description accordingly (it is no longer always `mem → write`). |
