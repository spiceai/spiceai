...
| 2026-08-20 | PR #13343 | While a pending mem-tier delete exists, the fast protected-snapshot subset compaction folds only snapshots whose merge-on-read threshold is at or below the pass's delete fence, so a fold can no longer mask rows newer than the fence (previously a hot key's live version could vanish from scans after a pass, which durable write-back escalated into a source-row delete). |
