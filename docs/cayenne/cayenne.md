...
|------|-----------------|---------|
| 2026-08-21 | PR #13351 | Durable write-back delivery now goes through a connector-owned `WriteBackDeliverer` when the source needs to observe its own delivery transaction — `PostgreSQL`, so it can stamp each delivery with its transaction id for CDC echo suppression — delivering present-key upserts and absent-key deletes as two source transactions instead of the `TableProvider` path. Every other source is unaffected: it still upserts natively where supported, falling back to delete-then-insert only on `NotImplemented`. *Durable federated write-back* updated. |
