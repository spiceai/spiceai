...
| 2026-08-25 | PR #13373 | A rewrite's shard bounds come from the per-partition bands its inputs report, cut at equal cumulative row mass, *when* those bands describe the key distribution (`range_bounds_from_histogram`); a flat or uninformative histogram falls back to equi-width bounds over the merged range (`range_bounds_from_statistics`), and a key with no usable range statistics hashes as before. |
