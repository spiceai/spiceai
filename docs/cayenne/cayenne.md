...
|------|-----------------|---------|
| 2026-08-20 | PR #13325 | `cayenne_write_concurrency` is now clamped by the caller's `target_partitions` instead of overriding it, so a merge that pins one encoder — the protected-snapshot path does this for position-delete tables — actually gets one; previously the configured value won and the pinned write sharded, while its encode-permit request stayed clamped to the pin. |
