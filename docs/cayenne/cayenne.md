...
Validation is **per-key**, not per-table. Every committed write stamps its commit sequence onto the `sequence` field of the primary keys it touches in the PK index (`PkKeysetEntry`). On the two current-snapshot append paths — the pipelined CDC arm that finds no conflict, and the synchronous plain append — that stamp is a sequence the write **draws**, not the table's ambient high water: neither has a snapshot sequence of its own, so it draws one before its rows are published, or its stamp would equal the begin token a concurrent transaction is still holding and both readings of the check below would read "committed before you began" (#13685). On those two paths an append that publishes no rows draws none: it gives no transaction anything to race, and moving the high water for it would abort one on the per-table fallback over a write nobody can observe — the same rule the mem-tier checkpoint applies with `any_nonempty`.

> **Not yet a table-wide invariant.** The remaining stamp sites still read `sequence_high_water()` rather than a sequence the write drew, which is fail-open for any write path added after them — tracked as #14359. `stage_on_conflict` and `begin_deferred_snapshot_append` likewise still reserve for an append that publishes no rows. Read both rules above as scoped to the two paths named, not as properties of every committed write, until #14359 closes.

A transaction records the digests of the keys it *read* — captured from the scan's pushed-down PK-equality predicate and digested through the same PK `RowConverter` as the keyset, so the footprint is projection-independent and phantom-safe. At `COMMIT`, `transaction_has_conflict` aborts with a retryable `Error::WriteConflict` (surfaced as HTTP 409 / Flight `Aborted`) iff a pipelined staging append was in flight at `BEGIN` (the staging-clean bit) or any key in the read footprint or write set carries a stored `sequence` newer than the begin token — otherwise disjoint-key transactions commit without contending. The per-key path applies only to a read expressed as a bounded full-PK equality point; an unbounded or multi-key read (a range, an `IN` list, an `OR`) marks the footprint incomplete and falls back to the conservative per-table sequence gate. Reading a Cayenne table that was never registered as a participant fail-closes the transaction at commit.
