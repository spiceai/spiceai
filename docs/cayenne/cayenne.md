...
An index answers only a lookup whose filters pin **every** column of one key to a literal, compared with the bare column (`WHERE tenant_id = 7 AND service_id = 'a'`). A cast on the column side (`CAST(score AS BIGINT) = 5` also holds for 5.2), a range, an `IN` list or an `OR` scans as before. The index only narrows what is read — every original predicate still runs on the candidate rows — so it can never add a row, and a lookup it cannot serve simply scans. Each probe is counted on `cayenne_lookup_index_probe_total{table, shape, outcome}`, with `outcome` one of `selected`, `empty` (no row holds the key), `unbuilt` (no index covers the rows read) or `snapshot_mismatch` (the index no longer matches the table's files).
