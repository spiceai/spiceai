...
|------|-----------------|---------|
| 2026-07-31 | (pre-merge, branch `ben/12211-cpu-limits`; issue #12270) | **CPU sizing now follows the runtime's CPU *entitlement*, not the host's core count.** A new foundation crate (`crates/cpu-budget`) resolves the entitlement — explicit `runtime.cpu.cores` / `SPICE_CPU_CORES` / `--cpu-cores`, then a cgroup CPU quota, then a Kubernetes cgroup CPU share (the kubelet writes it from `requests.cpu`), then `sched_getaffinity` — and owns every quantity derived from it. Four Cayenne-visible sites now read it instead of `available_parallelism()`: the per-accelerator compaction semaphore, the SQLite metastore pool (`K = min(cores, 32)`, floor 2), the `cayenne_upload_concurrency` / write-concurrency-ceiling defaults, and — the behavioral one — the **denominator of the adaptive tuner's CPU busy-fraction** (`sample_cpu_pressure`). That last one was a wrong-decision bug: on a 4-core entitlement misread as 18 cores a fully saturated process reported ~0.22, so the controller believed CPU was idle and kept making CPU-stealing moves. The fraction is now clamped to 1.0, since an operator who configures `cores` below the true host count can legitimately exceed it. Every derivation is value-identical to the previous one — only the input is corrected — so a host where detection was already right (a cgroup quota, or bare metal) sees no change. No schema change. |
