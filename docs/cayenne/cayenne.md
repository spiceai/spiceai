...
|------|-----------------|---------|
| 2026-07-21 | PR #11973 (pre-merge; doc otherwise baselined at `44d7538e`) | **Default-on adaptive cold layout (F4)**: when both `cayenne_datalake_clustering_columns` and `cayenne_sort_columns` are unset, cold-tier promotion now clusters by the hottest columns observed in scan pushdown filters (`resolve_cold_clustering_indices` → `FilterColumnObservations::top_columns`, top-2), falling back to the primary key when nothing useful has been observed. The datalake-tier clustering-key precedence prose and the `cayenne_datalake_clustering_columns` cheat-sheet default are updated to match. In-memory histogram only — relearned per boot, not persisted. No schema change. |
