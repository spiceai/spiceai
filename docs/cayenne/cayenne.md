...
- **Source slot / LSN** — the source's change-log offset (e.g. a Postgres LSN), acked only once the corresponding burst is durable (under `mode: memory`, right after each in-RAM write instead).
