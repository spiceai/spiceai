...
The index only narrows what is read — every original predicate, residual equality, and join filter still runs on the candidate rows — so it can never add a row, and a lookup it cannot serve simply scans. `EXPLAIN` exposes the selected path as `CayenneIndexScanExec` and `CayenneIndexJoinExec`; otherwise it retains DataFusion's ordinary scan and join operators. Each actual static or join index probe is counted on `cayenne_lookup_index_probe_total{table, shape, outcome}`, with `outcome` one of `selected`, `empty` (the indexed snapshot has no candidate for the key), `unbuilt` (no index covers the rows read) or `snapshot_mismatch` (the index no longer matches the table's files). In file mode, newly appended or still-inline rows are scanned separately until they are included in a rebuilt index, so candidate counts describe the index-pruned part of the plan rather than the whole table.
