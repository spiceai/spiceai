...
Each declaration also builds a transient, resident covering catalog for the complete captured source. It stores the declared key plus every stored table column in immutable Arrow pages; tables with several index definitions share those payload pages rather than copying them per index. A catalog is admitted against the existing query memory pool and is not persisted. On restart, refresh, or any change that makes coverage incomplete, Cayenne keeps the ordinary scan path until a complete catalog is published. This is a performance optimization only: a catalog that cannot be built, is stale, or does not cover every source always falls back to the ordinary query plan and never changes query results.

At planning time, a covering index answers only a lookup whose filters pin **every** column of one key to a literal, compared with the bare column (`WHERE tenant_id = 7 AND service_id = 'a'`). A cast on the column side, a range, a SQL `IN` list or an `OR` scans as before. A physical-plan rule also replaces an eligible inner or logical-left equality hash join with a covering-index join when its outer input is such a bounded covering lookup and the inner side has a complete index. It matches configured composite keys in their declared order even when the SQL conjuncts are reordered, preserves additional equality conditions as residual checks, and leaves every unsupported or unbounded join as its normal plan. The rule admits at most 2,048 raw outer candidates and 65,536 candidate pairs, and requires the candidate-pair bound to be no greater than `max(2,048, I / 8)`, where `I` is the indexed side's physical row count; it performs no I/O or payload gathering while deciding. Floating-point columns (`Float16`, `Float32`, and `Float64`) cannot be index columns: their equality semantics include values such as signed zero that do not have a single byte representation, so Cayenne rejects the index definition at registration. Use an integer, decimal, string, or another exact-equality column instead.

Every original predicate, residual equality, and join filter still runs on the candidate rows, so the covering path can never add a row. `EXPLAIN` exposes a selected covering path as `CayenneIndexScanExec` and `CayenneIndexJoinExec`; an accepted covering plan reads resident key/payload pages rather than Vortex data files. A multi-way selective join can therefore contain one index scan and an index join for each admitted join edge. `HashJoinExec` and Vortex scans remain in the plan for every declined edge. The existing address index remains the fallback for a literal lookup while coverage is unavailable and reports `lookup_index=...` on `CayenneAccelerationExec`; that is not a covering scan. Each actual address-index probe is counted on `cayenne_lookup_index_probe_total{table, shape, outcome}`, with `outcome` one of `selected`, `empty` (the indexed snapshot has no candidate for the key), `unbuilt` (no index covers the rows read) or `snapshot_mismatch` (the index no longer matches the table's files). A full refresh stored entirely in the inline tier builds its covering catalog at the snapshot publication boundary and can use the covering path. Newly appended inline rows are scanned separately until a later full refresh covers them, so candidate counts describe the address-index-pruned part of the plan rather than the whole table.
...
- **Composition.** A row selection is intersected with the table's position-delete vectors (`LookupAccessPlanProvider`), so a deleted row is never selected, and a scan carrying a selection reports inexact statistics. A covering-index join selection is intersected with the file's planning-time plan in the Vortex opener, so it can only narrow what that plan reads.
