...
Bake passes are observable on the same series as the other compactions, under their own `kind="bake"` label: `cayenne_compaction_duration_ms` counts and times every pass that committed a merge or attempted one and failed, skipping the no-op early-outs, so its count is the bake rate. A bake records `cayenne_compaction_merged_bytes` on a committed merge, as the size-tier merge does, so its volume — and therefore its throughput — is computable rather than duration-only. The bake shares the subset machinery but reports separately from it because the two are triggered independently and shrink different things: a subset merge answers small-file accumulation, a bake answers a deletion index past `cayenne_bake_deletion_index_trigger`. A single `kind` for both would leave neither rate readable.
