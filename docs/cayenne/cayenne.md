...
|------|-----------------|---------|
| 2026-08-16 | PR #13180 (fixes #13018) | A table now stores the timestamp unit its source reports — Vortex represents all four Arrow units, so the creation-time rewrite to microseconds is gone and `Float16` to `Float32` is the only unconditional one. A `PostgreSQL` `timestamptz` accelerates as nanoseconds instead of being silently narrowed on every refresh. *The on-disk layout* updated. |
