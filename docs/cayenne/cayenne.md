...
|------|-----------------|---------|
| 2026-08-14 | PR #13057 (pre-merge; doc otherwise baselined at `44d7538e`) | **A partitioned dataset no longer takes the open-time widening schema evolution.** The evolution is gated where the Cayenne config is built, and that same config opens the **parent** catalog entry, which is created before any partition. Leaving evolution on for that open let the catalog widen the parent's stored schema while every child kept its own, so the accelerated table advertised a width its data did not have and read back a silent narrowing cast (#12999) — reached through the open path rather than `evolve_table_schema`. `schema_evolution` now resolves to `Disabled` whenever `partition_by` is non-empty, alongside the existing `refresh_mode: caching` exclusion; both are recorded in *The corpus outlives a schema change* above, which previously described the open-time commit as unconditional. Also in this change: `drop_table` deletes a partitioned table's child tables and its parent row in **one** transaction (a half-applied cascade is not self-healing — a partition row whose child is gone makes `infer_existing_partitions` propagate `TableNotFound` and the table stops opening at all), and `file_create` teardown takes the same metadata-before-files ordering with fatal catalog failures, so a failed drop can no longer leave rows describing deleted files. Partition child-table naming moves to a shared `partition_naming` module so the creator and the catalog derive the same names. Known limits, documented rather than claimed away: child discovery is read outside the drop transaction, so a partition created concurrently with a drop can still be missed; and both teardown paths still `remove_dir_all` the data directory without proving it disjoint from the metastore directory (pre-existing — see #13068). No schema or config-key change. |
