...
|------|-----------------|---------|
| 2026-08-20 | PR #13200 | Append overlap de-duplication now applies the Cayenne type rewrites before comparing source rows with stored rows, so a `PostgreSQL` `timestamptz` precision rewrite does not fail a valid append refresh as a schema mismatch. *The on-disk layout* updated. |
