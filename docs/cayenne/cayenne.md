...
A tier whose runs are individually too large to *pair* within the budget is thereby **settled** — the same "not a compaction candidate" outcome the current-snapshot picker gives files at or above the target size, but derived from the memory budget rather than configured. That is a deliberate trade: the pass declines instead of risking the host, but the protected set — and every scan's read amplification — then stops shrinking. Because a silent stall here is the failure mode operators can least afford to miss, an episode sustained past `PROTECTED_MERGE_BUDGET_STALL_WARN_AFTER` (10 min) is escalated to a one-shot WARN naming the tier's run count, the bytes its two smallest runs need, and the budget they exceeded.
