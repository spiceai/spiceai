...
`cayenne_metadata_dir` is **not** in the table above: it is a `runtime.params` key, because the accelerator opens exactly one metastore for the whole instance and hands it to every Cayenne table. A per-dataset directory could therefore be honoured only for whichever dataset opened the catalog first — which one that is is decided by a startup race between concurrent dataset loads — leaving every other dataset recording its manifests, snapshot pointers and partition rows in a catalog under a directory it never named. Set `runtime.params.cayenne_metadata_dir`; it defaults to `{cayenne_file_path}/metadata` when that path is local, else `{spice_data}/metadata`. An `acceleration.params.cayenne_metadata_dir` is ignored, with a warning naming the directory the dataset's rows actually go to.

Wherever it resolves, it must not sit inside any acceleration's data directory: that directory is deleted wholesale on a `file_create` bootstrap and on a schema recreate, and one metastore is the catalog for every Cayenne dataset in the instance. A layout the resolved metastore places inside a dataset's data directory is refused at load; a metastore found on disk under the directory about to be deleted — whoever configured it — refuses the teardown. The Cayenne *catalog* connector carries its own pair under `catalog.params` (`cayenne_data_dir` / `cayenne_metadata_dir`) and is refused at load on the same terms — see *Catalog visibility: which tables the catalog exposes*.
