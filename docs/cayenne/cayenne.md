...
The index only narrows what is read — every original predicate and join still runs on the candidate rows — so it can never add a row, and a lookup it cannot serve simply scans. `EXPLAIN` reports planning-time decisions on `CayenneAccelerationExec` as `lookup_index`, `lookup_index_outcome`, `candidate_files` (file mode), and `candidate_rows`; a runtime join lookup still reports the planning-time `lookup_index=none, lookup_index_outcome=not_applicable`. Each actual static or runtime index probe is counted on `cayenne_lookup_index_probe_total{table, shape, outcome}`, with `outcome` one of `selected`, `empty` (the indexed snapshot has no candidate for the key), `unbuilt` (no index covers the rows read) or `snapshot_mismatch` (the index no longer matches the table's files). In file mode, newly appended or still-inline rows are scanned separately until they are included in a rebuilt index, so candidate counts describe the index-pruned part of the plan rather than the whole table.
...
| `cayenne_cold_tier_file` | cold object-store tier file manifest (curve-clustered Vortex); inline stats blob for zero-round-trip listing-time pruning |
| `cayenne_pending_write_back` | durable-write-back dirty-key markers: one row per committed-but-undelivered primary key (`WITHOUT ROWID`; raw-UUID-byte `table_id` + `RowConverter`-encoded `pk_bytes`; `sequence_number` of the commit that dirtied it); the delivery worker claims, reconciles to the federated source, and clears. Claims page in commit order on a `(sequence_number, pk_bytes)` keyset cursor, which the table's own `(table_id, pk_bytes)` primary key cannot serve — a covering index on `(table_id, sequence_number, pk_bytes)` is what makes a claim a seek rather than a sort of every marker the table holds |
