...
The cgroup-aware total behind these process-wide ceilings is sampled on the blocking pool while the runtime builds and again during every app-reload memory preflight, including an otherwise-identical apply. Reload planning charges the fixed query/compaction pools, the identity-retained ceilings of active or cached DuckDB database pools, and a high-water Cayenne cache reservation. A changed Cayenne component temporarily charges both its old and replacement providers because reload initializes replacements before removing the old providers; the live tier sampler subtracts the part of that reservation above the standard host/10 headroom before it lets a query-light tier float upward. A newly introduced DuckDB identity receives only what remains after cached pools, down to the 128 MiB floor, instead of splitting memory those pools still own. The dynamic-tuning and fleet-wide PK-keyset ceilings are refreshed from the same total before it is release-published to the sampler. The query and compaction pools themselves remain fixed until restart; when any fixed pool and off-pool reservations no longer fit after a cgroup resize or app change, the runtime warns that a restart or smaller limits are required. Lowering the PK-keyset ceiling preserves the bytes already charged by live table accounts, so the new limit bounds subsequent growth without hiding resident memory.
