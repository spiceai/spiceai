...
|------|-----------------|---------|
| 2026-08-13 | PR #13027 | Cayenne's host-memory classification now counts an accelerated catalog, which was invisible to both the CDC-tier query-pool reduction and the compaction carve. A catalog's acceleration is converted into the dataset acceleration its tables are configured with, then classified through the same write-profile mapping. |
| 2026-08-12 | PR #12940 | The scan memory charge now counts the splits Vortex decodes concurrently, rather than one batch per output partition. New parameter `cayenne_scan_concurrency` (`auto` / `off` / `<n>`). |
...
| 2026-08-08 | PR #12802 | PK keyset budgets are now bounded process-wide (`pk_keyset_budget`) rather than per table, which let a multi-table pod exceed host memory while every table stayed inside its own limit. Also corrects this section: only the commit mirror is bounded during insert. |
| 2026-08-08 | PRs #12759 + #12790 | Scan materialization — the largest unaccounted allocator at scale — is now charged to the query pool, gated on a plan that actually reads files, and refused with a `ResourcesExhausted` error naming the dataset and the remedy. The sharded PK keyset budget is enforced during the insert rather than after it. |
...
| 2026-08-01 | PR #12192 | The sharded PK keyset byte budget is now enforced, degrading an over-budget upsert index to per-shard blooms, and per-entry accounting charges what the map actually stores (96 B, not 56 B, for an 8-byte key). |
...
| 2026-07-31 | PR #12262 (fixes #12208) | `write_to_snapshot` now creates the snapshot directory itself, making "a snapshot the catalog can reference has a directory" a property of the write rather than a per-call-site guard, and the fused commit takes the directory barrier before publishing. |
| 2026-07-31 | PR #12276 (issue #12270) | CPU-derived sizing moved behind one overridable budget (`crates/cpu-budget`), read by five Cayenne sites including the denominator of the adaptive tuner's CPU busy-fraction. Detection is unchanged, so existing deployments size identically. |
...
| 2026-07-16 | PR #11920 | Datalake cadence parameter renamed to `cayenne_datalake_tiering_check_interval_ms` (hard rename, no alias). Tiering logs reworded to plain language, and routine per-operation chatter demoted INFO to DEBUG. |
...
| 2026-07-10 | PR #11795 | The datalake per-table object prefix changed from the bare `<table_id>` to `<sanitized_table_name>-<table_id>`, so a shared bucket is navigable while staying collision-free. Write and GC prefixes only; reads follow the manifest. |
