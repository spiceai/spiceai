...
The index only narrows what is read — every original predicate and join still runs on the candidate rows — so it can never add a row, and a lookup it cannot serve simply scans. `EXPLAIN` reports planning-time decisions on `CayenneAccelerationExec` as `lookup_index`, `lookup_index_outcome`, `candidate_files` (file mode), and `candidate_rows`; a runtime join lookup still reports the planning-time `lookup_index=none, lookup_index_outcome=not_applicable`. Each lookup that pins an index's columns, and each runtime join probe, is counted once on `cayenne_lookup_index_probe_total{table, shape, outcome}`, with `outcome` one of:

- `selected`: the index returned candidate rows;
- `empty`: no indexed snapshot has a candidate for the key;
- `unbuilt`: no index covers some of the rows read;
- `snapshot_mismatch`: the index no longer matches the table's files.

A lookup reads the current snapshot and every protected snapshot, and reports the most telling of their outcomes: `unbuilt` or `snapshot_mismatch` if any snapshot was read without its index, otherwise `selected` if any snapshot had a candidate, otherwise `empty`. In file mode, newly appended or still-inline rows are scanned separately until they are included in a rebuilt index, so candidate counts describe the index-pruned part of the plan rather than the whole table.
...
- **Checkpoints, upserts and compaction.** Every write that creates a new snapshot directory builds that snapshot's index the same way while it writes. That covers an inline or mem-tier checkpoint, an upsert published as a protected snapshot, a protected-snapshot merge or bake, and a full or sorted rewrite. The index is registered under the snapshot id before the snapshot becomes visible.
  - A lookup probes each protected snapshot's own index, so a snapshot that cannot hold the key is not read at all.
  - A rewrite's index is promoted to the current snapshot's index inside its flip.
  - A protected snapshot's index is dropped once a merge, bake or rewrite folds the snapshot away.
  - A write that fails before publishing discards its index.
  - A snapshot whose build the pool refuses, or that was written before a restart, is read in full until compaction rewrites it. Nothing rebuilds it in the background.
- **Anything else that changes the current snapshot's files** — an append into its directory, the small-file subset rewrite, a restart — leaves the index stale. A probe drops an index built for another snapshot; a scan that lists a file the index lacks, after the table's directory generation or listing epoch has moved since the index was listed, drops it too. The next literal lookup or supported dynamic join lookup then claims the table's single background build, which reads the files back with Vortex `row_idx()` positions on the blocking pool. That first query scans normally; later queries use the index after the build publishes.
