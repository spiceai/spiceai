...
|------|-----------------|---------|
| 2026-07-15 | PR #11893 (pre-merge; doc otherwise baselined at `44d7538e`) | **Freshness mem-tier shrink eligibility gates**: `decide_goal` only shrinks `cayenne_cdc_mem_tier_max_bytes` when freshness is violated **and** apply is behind offered load (`apply_vs_arrival > BEHIND_RATIO`) **and** the stream is not mutation-heavy. Adaptive CDC tuning prose updated so the doc no longer claims an unconditional freshness shrink (ladder A scar: healthy-apply shrink starved absorb and raised order_line p99). |
