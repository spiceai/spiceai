...
| **bulk-overwrite** | `full` | **off** — each refresh replaces the whole table, so nothing accumulates to consolidate | off |
...
|------|-----------------|---------|
| 2026-08-11 | PR #12790 (pre-merge; doc otherwise baselined at `44d7538e`) | **The sharded PK keyset budget is enforced during the insert rather than after it.** `record_keys_in_shard` recorded a whole batch with `max_bytes: usize::MAX` — so `OverBudget` could never fire — and reconciled once afterwards via `degrade_to_blooms`, making the budget a **trim rather than an admission control**: within one batch the exact keysets grew to `batch_keys × entry_bytes` with no ceiling, each entry retaining a cloned `OwnedRow` beside its digest, `RowLocation` and sequence. An SF-1000 jemalloc heap profile attributed **~14.5 GiB** to `CachedPkKeyset::try_insert_with_digest` against the 256 MiB default — **58× the budget**, and 90.9% of the CDC-apply subtree. `record_keys_bounded` replaces it: the table-global tally is re-read every **512 keys** and the insert stops there, bounding overshoot to one chunk instead of one batch. Chunked rather than per-key because `approx_bytes` is O(shards) over cached per-keyset totals — cheap but not free, and a bound whose job is to stop unbounded growth need not be exact. It reports over-budget rather than converting in place because the `Exact`/`Bloom` variant is table-global — one shard cannot convert while its siblings still append under their own publish locks — so the policy stays with the caller: an upsert table degrades to per-shard blooms (a false positive is a harmless redundant delete), while `DoNothing` needs exactness and drops the index to rebuild lazily. Because a bounded insert stops mid-batch, the caller **backfills the whole batch into the blooms after degrading** (`record_keys_after_degrade`): degrading converts only what the keysets already hold, so keys after the stop would otherwise be absent — and an absent key is a false NEGATIVE, which under upsert reads as a new PK and writes a **duplicate live row**. The single-keyset path has always re-inserted the full batch; the sharded path now matches it. `degrade_to_blooms` also releases each shard's exact entries as its bloom is built, rather than holding exact + blooms together at the one moment memory is already exhausted. Measured at SF-1000 alongside the companion scan-side charge (#12759): bootstrap completes for the first time (`order_line` 300M rows) and survival goes ~10 min → ~24 min, but **peak RSS is unchanged at 95.8 GiB and the process is still OOM-killed** — this bounds the keyset path only, and the CDC apply (`mutation_writer`) remains unaccounted. No schema or config change. *Other managed state* updated with the during-insert enforcement and the post-degrade backfill. |
| 2026-08-10 | PR #12759 (pre-merge; doc otherwise baselined at `44d7538e`) | **Scan materialization is now charged to the query memory pool, and an over-budget scan errors instead of drifting toward an OOM kill.** Canonicalizing Vortex's compressed encodings into flat Arrow is the largest single allocator in the process at scale — ~50 GiB of an SF-1000 heap profile under `vortex_buffer::BufferMut::with_capacity_preferred_aligned` — and nothing reserved for it: `DataFusion` operators reserve for what they hold and `memory_account` covers long-lived resident state, so the pool could sit at its limit and spill while the scan beneath it kept allocating. `runtime.query.memory_limit` consequently did not bound the process (peak RSS tracked the cgroup cap: 95.8 GiB at 96G, 109.8 GiB at 110G, unmoved by partition count or tuning mode). The outermost `CayenneAccelerationExec` now wraps each output partition's stream in a `MemoryAccountedScanStream` that charges a running-max estimate **before** each poll, holds it across the decode, and settles to `get_array_memory_size()` once the batch exists — the ordering being load-bearing, since charging after the poll can only detect an overrun that has already been allocated. The reservation covers the batch **in flight** (yield to next poll), not its downstream lifetime, which would double-count against whichever operator now owns it. Two boundaries are explicit rather than implied: accounting attaches only to the outermost wrapper, so concurrently decoding base+delta children **under-count** against one estimate; and the install gate is whole-plan (`plan_materializes_files`), so a purely memory-backed scan is not charged at all — those bytes are already mirrored by the `cayenne:mem_tier` consumer — while a mixed union **over-counts** by one in-flight mem-tier batch per partition. Charging at each materializing leaf closes both and is the noted follow-up. **User-visible behavior change**: a scan that cannot fit returns `ResourcesExhausted`, so a workload that was quietly over-committing starts erroring. The CDC apply path (`mutation_writer`) is untouched and remains unaccounted. No schema or config-key change. *Flow A* updated accordingly. |
