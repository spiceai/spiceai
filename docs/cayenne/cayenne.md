...
One note on versioning: the **cold object-store tier** at the bottom is newer than the commit this document otherwise tracks — it was added in PR #11543 (`e972f80`), *after* `64451e0`. It is optional (enabled by `cayenne_cold_tier_location`, dormant and byte-identical otherwise) and shown here because it completes the tiering picture. When enabled, the tiers form a strict cascade — RAM mem-tier, then warm local-disk Vortex, then cold object store, with a row living in exactly one tier. A background `BackgroundColdTierPromoter` promotes warm to cold once size/file thresholds cross: it re-materializes the whole visible table (all deletes applied, one version per key), **Z-order-clusters** it for tight multi-column zone maps, writes read-optimized Vortex at a larger cold file size, and atomically registers the cold files (`cayenne_cold_tier_file`) while overwrite-clearing the warm tier — so cross-tier scans prune cold files from the metastore stats blob with no object-store round-trip, and a delete after promotion still hides a cold-resident row. Enabling it forces key-based deletes; partitioned and position-delete tables are unsupported in this first version.
...
| `cayenne_cold_tier_location` | object-store URI for the optional cold tier; presence enables it (dormant otherwise) | (unset) |
| `cayenne_cold_clustering_columns` | columns to Z-order-cluster cold files by | (derive from PK) |
| `cayenne_cold_tier_warm_max_bytes` / `_max_files` | warm→cold promotion triggers (size / file count) | (auto) |
| `cayenne_cold_tier_background_interval_ms` | cold-tier promoter cadence | (auto) |
