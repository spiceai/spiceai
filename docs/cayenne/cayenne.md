...
One row per merged PR that changed something a reader of this document would act on. The reasoning lives in the commit each row points at; the row exists to index it.

| Change | Reference |
|---|---|
| A relocation-only invalidation now preserves a bloomed sharded PK index instead of discarding it, bounded by two new drift thresholds (`SPICE_CAYENNE_BLOOM_MAX_STALE_PERMILLE`, `SPICE_CAYENNE_BLOOM_MIN_BITS_PER_KEY`). PK-index rebuilds and degrades size their blooms from live cardinality rather than the byte budget, build straight to blooms when the exact keyset provably cannot fit, and fan a bloomed rebuild's scan across its partitions. | _pending merge_ |
