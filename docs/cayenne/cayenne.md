...
- **Stage A / Stage B** — the two-phase write: Stage A is the durable half (encode + exact-file WAL); Stage B is the visibility half. In the default `rename_sync` mode Stage B moves staged files; in opt-in `manifest` mode Stage A pre-places immutable files and Stage B atomically publishes their manifest rows without data-file I/O.
...
- The heavy lifting (encoding Vortex files and checkpoints) happens *outside* the read/write barrier. In opt-in manifest publication, Stage B contains the atomic manifest-add transaction and in-memory cache publication but no data-file rename, copy, delete, or target-directory fsync; moving that remaining metastore commit off-fence is a later generation/pointer-publication milestone.
...
- **Scan-view maintainer** (`run_scan_view_builder_loop`, `scan_view`) — a per-table background task that computes the merge-on-read scan input ONCE per table version, off the query cores, and publishes it as an immutable `Arc<ScanView>` (the merged file∪mem-tier deletion snapshot + the per-segment deletion-filtered visible mem-tier batches + the tombstone union) for scans to borrow. This retires the two per-scan `ArcSwapOption` memos (`merged_scan_deletions`, `mem_tier_visible_memo`) and the N==1 append-lockstep it maintained: instead of every concurrent scan recomputing the merge on a cold memo (the thundering herd that hurt worst under a query burst on the CPU-starved box), one background build serves them all. The maintainer captures the live input under one `listing_fence.read()` (`capture_raw_scan_input`), decides whether to rebuild by a hash-free `RawScanInput::changed` (`Arc::ptr_eq` per shard + file-index pointer + structural epoch + snapshot id), runs the CPU-bound build in `spawn_blocking` (never stalling a tokio worker), and self-throttles to at most one build per `CAYENNE_SCAN_VIEW_REFRESH_FLOOR_MS` window (default 1000 ms, jittered ±25% per-table so the maintainers do not rebuild in lockstep and spike CPU contention against the apply). Mutations wake it with a cheap `Notify` (`scan_input_change`, wired at every scan-input-changing site) and advance a monotonic `scan_input_version` that paces the maintainer. **Scans are NON-BLOCKING**: a scan serves the LATEST published bundle and never waits on the maintainer to catch up to the newest mutation — freshness is bounded-stale by the refresh floor, well inside the freshness SLO. This keeps analytical scans off the maintainer's critical path; an earlier linearizable-on-input gate instead made scans wait through rebuild cycles (an SF100 HTAP measurement showed multi-second per-scan waits) and let the builder thrash cores away from the CDC apply. A scan WAITS in exactly one case: a forced structural event (truncate / full-delete / `INSERT OVERWRITE` / live schema-evolve / reopen) — bracketed by a `StructuralVersion` seqlock — has advanced the structural generation past the published bundle, so the scan waits for the post-event publish to see the new structure (`ScanView` carries the generation it was built at). Ordinary appends/deletes/upserts do not advance that generation, so they never make a scan wait; the seqlock also discards a capture that straddled a forced event rather than publishing a torn bundle. A scan sees a bundle GC-protected for its whole lifetime by the shared `SnapshotScanRef` the capture minted under the fence.
...
The scan resolves one consistent view, pins the snapshot so concurrent compaction/GC can't delete files out from under it, and applies deletions transparently. It no longer computes the merge itself: it borrows the **scan-view maintainer's** published `Arc<ScanView>` behind a NON-BLOCKING, bounded-stale gate (blocking only across a forced structural event — see *Other managed state* in Part 3), so the merged deletion snapshot + deletion-filtered visible mem-tier batches are computed once per version off the query cores rather than per-scan (or per-reference) on them. Nothing here blocks a concurrent writer except the microsecond window when a publisher takes the *write* side of the fences; and because the bundle is captured off the scan path, the scan holds no listing fence across plan-build.
...
| `cayenne_snapshot_file` | authoritative per-snapshot data-file manifest; `min/max_sequence` drive seq-prefix bake; nullable `digest` (`xxh3-128:<hex>`) for opt-in end-to-end file-integrity verification. Maintained incrementally at every snapshot-publishing commit (plain append, on-conflict protected-snapshot append, overwrite, and every compaction variant), not just derived from a directory listing. `scan_from_manifest` (default off) routes a scan's file resolution through this table instead of a directory `LIST`; once on, an empty manifest for a live snapshot is treated as authoritative (the snapshot genuinely has no files), not a "not yet populated" signal — a directory-listing table keeps the transparent dual-source fallback. |
...
| `cayenne_stage_b_publish_mode` | `rename_sync` writes under `_staging` and moves files during Stage B; `manifest` (unpartitioned tables only) pre-places immutable files during Stage A, forces authoritative manifest scans, and publishes exact WAL-recorded file rows atomically with no Stage-B data-file I/O | `rename_sync` |
...
| `cayenne_ingest_pipeline_depth` | CDC checkpoint-drain pipeline depth `D` (`cdc_durability: memory` only): `1` runs each RAM-tier checkpoint's freeze→encode→upload→clear serially under `mem_checkpoint_lock`; `>1` keeps up to `D` frozen checkpoint generations resident so their encode + object-store upload overlap off-lock (freeze *N+1* begins while drain *N* uploads), while publish still commits strictly oldest-first — durable results are identical to `1`. Resident RAM ≈ `D` × the checkpoint tier size, so it is capped at 8 (larger values clamp down) | `1` |
...
The scan-view maintainer's rebuild floor is a **process-wide environment variable**, not a per-dataset parameter (it is operational tuning, not data configuration, so it stays out of `configuration_matches`): `CAYENNE_SCAN_VIEW_REFRESH_FLOOR_MS` (default `1000`, jittered ±25% per-table; `0` = rebuild as fast as the input changes) bounds how often the background maintainer recomputes a table's published scan view — it sets the freshness/CPU trade, since scans are served non-blocking off the latest bundle (staleness ≈ floor + build, far inside the freshness SLO) and the floor also caps rebuild CPU so the maintainer yields cores to the apply. It exposes `cayenne_scan_view_builds_total` / `cayenne_scan_view_build_duration_ms` (one off-query-path build per table version — expect the count ≪ the scan count) and `cayenne_scan_view_waits_total` / `cayenne_scan_view_wait_duration_ms` (scans that waited for a post-structural-event publish — ~0 for ordinary churn, which is served non-blocking) for operator observability.
...
|------|-----------------|---------|
| 2026-07-17 | PR #TBD (assign the merged PR/commit) | **Scan-view maintainer**: scans now borrow a per-table background maintainer's published `Arc<ScanView>` through a **non-blocking, bounded-stale** gate instead of recomputing the merge-on-read view per-scan — retiring the two per-scan `ArcSwapOption` memos (`merged_scan_deletions`, `mem_tier_visible_memo`), the N==1 append-lockstep, and the `MergedScanDeletions` struct. The maintainer computes the merged file∪mem-tier deletion snapshot + deletion-filtered visible mem-tier segments ONCE per table version, off the query cores in `spawn_blocking`, throttled by `CAYENNE_SCAN_VIEW_REFRESH_FLOOR_MS` (env var, default 1000 ms, jittered ±25% per-table). A scan serves the latest published bundle without waiting on ordinary CDC churn (freshness bounded by the floor, inside the freshness SLO); it WAITS only across a forced structural event — the scan gate keys on the `StructuralVersion` generation the bundle was built at, so a post-truncate/-schema-change/-overwrite/-reopen scan sees the new structure. `RawScanInput` / `ScanView` / `build_scan_view` factor the capture + build; the `StructuralVersion` seqlock (`read_validated_async`, loom-modeled) brackets forced events so a straddling capture is discarded rather than published torn; a provider without a running maintainer (unit-test / pre-spawn) builds fresh inline. Adds `cayenne_scan_view_builds_total` / `_build_duration_ms` / `_waits_total` / `_wait_duration_ms` metrics and a `CancellationToken` shutdown (a parked scan wakes rather than hangs). Updated *Other managed state*, the seal memo-transparency note, the scan overview, and the cheat-sheet. |
...
| 2026-07-16 | (pre-merge; morsel pipeline Stage 2b) | **Pipelined CDC checkpoint drain + the `cayenne_ingest_pipeline_depth` parameter (`cdc_durability: memory`).** New parameter (default `1`) bounding how many frozen checkpoint generations `D` may be resident at once. At `D = 1` — the default — behavior is byte-identical to before: each RAM-tier checkpoint freezes the tier prefix, encodes it to Vortex, uploads, commits, publishes, and clears the tier, all under `mem_checkpoint_lock`, so the next freeze waits for the whole drain. At `D > 1` the freeze stays O(1) under the lock, then the encode + object-store PUT run **off-lock on a detached task**, so a later freeze overlaps an earlier generation's upload — lifting the serial `events_per_checkpoint / T_flush` checkpoint-drain ceiling toward `… / max(T_encode, T_put) × D` for encode/upload-bound memory-mode tables. Correctness is preserved by four drain-specific invariants: **relative-window encode** (a resident generation encodes only its own `[base, base+N_G)` segment window, since a later freeze's captured snapshot still includes an earlier generation's not-yet-cleared segments — the one tier-wide sequence-ordered deletion index resolves cross-window tombstones, so no Model-A resurrection); **ordered publish** (metastore commit + listing-fence swap + front-`retain_after` clear serialized strictly oldest-first, encode/PUT fanning out unordered ahead of it); **min-across-in-flight ack** (the slot advances to the max epoch over the contiguous terminal-published prefix, never past a still-encoding or failed earlier generation — never `fetch_max`); and **cascade-discard on failure** (a failed generation and all younger ones are discarded and their rows replay PK-idempotently, since a younger generation's window base counted the failed one's segments). The seal path stays inline + single-ordered: at `D > 1` it first **quiesces** the pipeline (acquires all `D` admission permits) so it captures over an empty ledger, never doubly-persisting a resident checkpoint's window. Resident RAM ≈ `D` × the checkpoint tier size (§4.5), so the depth is validated to `[1, 8]` (0 → 1; over-cap clamps, with a warning). Executor is a detached `tokio::spawn`, not the pinned ingest pool (the drain is async encode/PUT/commit-bound). The whole `D > 1` mechanism is gated behind the parameter; the default `1` leaves every drain serial and unchanged. |
| 2026-07-15 | (pre-merge; adapts PR #11907) | **Cold-tier promotion mem-tier checkpoint serialized on `mem_checkpoint_lock`.** A warm→cold promotion held `write_lock` but not `mem_checkpoint_lock`, so at the default `cdc_mem_tier_shards: 1` it could recapture a background checkpoint's not-yet-cleared segments and publish a second durable copy (a duplicate-row over-count baked into the cold tier). Cold promotion (and every checkpoint/seal path) now serializes the whole capture→encode→publish→clear on `mem_checkpoint_lock`. Adding that lock created a `write → mem` (spill/promotion) vs `mem → write` (background checkpoint) ordering inversion, resolved by acquiring capture locks in one consistent `write → mem` order: a **guaranteed** acquisition (`acquire_capture_locks_blocking`) takes `mem_checkpoint_lock`, fast `try_lock`s `write_lock`, and only on contention drops `mem_checkpoint_lock` and re-acquires fairly `write → mem` (never blocks on `write_lock` while holding `mem_checkpoint_lock` — deadlock-free, non-starving); a **best-effort** acquisition (`try_acquire_capture_locks`, size-triggered tick with age headroom) `try_lock`s both and skips (`CheckpointAttempt::Busy`, no slot advance) on contention. `checkpoint_mem_tier` self-acquires; `checkpoint_mem_tier_holding_locks` is for the write-path spill + `evolve_schema` (both locks already held). |
| 2026-07-15 | (pre-merge) | **Stage-A file pre-placement (pipelined-ingestion groundwork)**: added opt-in `cayenne_stage_b_publish_mode: manifest` for unpartitioned tables. The Vortex sink's exact file report is persisted in `_wal.json`; Stage A writes immutable files directly to the target snapshot, while Stage B atomically adds all manifest rows, updates the single-process manifest cache, and retires the WAL without rename/copy/delete/target-directory-fsync work. Manifest mode forces fail-closed manifest scans and never rebuilds visibility from a physical directory listing, since doing so could publish output from a process that died before durable intent. Recovery idempotently distinguishes staged versus pre-placed files; rollback removes only its exact unpublished generation; a bounded, age-gated, write-lock-serialized sweep reclaims old final-directory files absent from every live manifest after first completing any durable WAL. The default `rename_sync` path is unchanged. |
...
| 2026-07-14 | (pre-merge; doc otherwise baselined at `4685a3dd`) | **Manifest hardening (commit 1 of the pipelined-ingestion groundwork)**: `cayenne_snapshot_file` is now maintained incrementally (or, for paths with no per-file metadata to build an incremental row from, by a full relist) at every snapshot-publishing commit, instead of relying solely on a later best-effort re-listing pass. Fixed a systemic gap where several commit paths authored no manifest rows at all — harmless before this change (the directory-LIST fallback covered it) but a silent-data-loss-on-read risk once that fallback is removed for `scan_from_manifest` tables. Found and fixed via fuzzing plus a full sweep of every snapshot-publishing commit site: the plain-append Stage-B path (`insert_current_snapshot_manifest_rows`, local FS + S3), the on-conflict/pending-PK-deletion protected-snapshot append (`write_new_snapshot_after_validation` — the one the new fuzz coverage caught directly), the mem-tier/`cdc_durability: memory` checkpoint flush (`checkpoint_mem_tier_inner`, both its new-protected-snapshot and its direct-to-current-snapshot branches), the general inline-memtable checkpoint (`checkpoint_inlined_data`, both branches), the sort-column rewrite (`sort_and_rewrite_data`), and `insert_to_new_snapshot_with_sequence`. Compaction (full-rewrite, subset-merge, seq-prefix bake) and overwrite already authored a manifest; they now abort-and-retry (or hard-error for overwrite) instead of silently publishing when authoring fails and `scan_from_manifest` is on. Every new incremental-write call site is gated on `scan_from_manifest` so a directory-listing table's cost/behavior is unchanged (verified: this gating was itself a bug fixed mid-review, since the first pass added unconditional catalog round-trips to every table's hot append path). `scan_from_manifest` tables no longer silently fall back to a directory `LIST` on an empty manifest (now authoritative) or a failed open-time backfill (now a hard table-open error); crash recovery (`ensure_no_incomplete_write`) reconciles manifest rows for a resumed move whose files were already in place before the crash. Tests: `mutation_property_test.rs` gained `scan_from_manifest` fuzz dimensions for the append/on-conflict/compaction (File durability) and mem-tier-checkpoint (Memory durability) paths — each adversarially verified to actually fail without its corresponding fix; `staged_append_test.rs` gained a crash-injection test for the reconciliation path; `sort_rewrite_test.rs` gained a `scan_from_manifest` regression test. |
