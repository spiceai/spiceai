...
|------|-----------------|---------|
| 2026-08-24 | PR #13300 (fixes #12964) | Snapshot cleanup's two waits — for the in-flight segment-cache writes on the retiring paths, and for the enumeration that finds their cached keys — are now bounded at ten seconds each, so a saturated host no longer stalls the `DELETE` or overwrite that triggered the retirement behind whatever had stalled the blocking pool. Giving up emits a `vortex::segment_cache` warning naming the files, and costs a moment of residency for the writes or, for the enumeration, segments that stay cached until capacity evicts them. |
