...
**Pipelined protected-snapshot compaction** (key-delete tables only; position-delete tables always take the single-tier path above). `select_protected_snapshot_merge_tiers` partitions the protected set into *every* qualifying same-size tier (≥ `cayenne_compaction_trigger_protected_snapshots` runs each), not just the lowest one; `compact_protected_snapshots_pipelined` takes one shared Phase-1 fence read — `(candidates, fence_max_delete_seq, deletion_snapshot, snapshot_at_capture)` — and then runs up to `cayenne_compaction_max_concurrent_merges` of those disjoint tiers' Phase 2 (rewrite) + Phase 3 (CAS commit + RCU publish) concurrently via `futures::future::join_all`, instead of one tier per pass. Concurrency is cooperative, not `tokio::spawn`-based: `run_compaction_trigger` only has `&self`, so each tier merge's own async work (Vortex encode, catalog CAS) overlaps without a `'static` task. This is what lets one table draw more than one merge's worth of the process-global encode-permit budget per pass — the previous single-tier-per-pass path left that budget idle whenever a table's `write_concurrency` was smaller than the pool, even under a real multi-tier backlog. `compaction_lock` is still held for the whole pipelined pass (unchanged): this parallelizes tiers *within* one pass, not passes against each other. Because `swap_protected_snapshots`'s CAS is per-input-id rather than whole-manifest, disjoint tiers can commit independently with no coordination beyond the shared fence read; each merge also revalidates `snapshot_at_capture` against the live `current_snapshot_id` before its RCU publish (the same overwrite/promotion guard the single-tier path applies), so a concurrent full-snapshot overwrite mid-pass still can't resurrect rows. Partial progress on error: one tier's failure doesn't discard another's already-committed merge. Two metrics make in-flight concurrency directly observable — the `cayenne_compaction_subset_tier_total` counter and the `cayenne_compaction_subset_inflight` gauge (per `table`); if the gauge is ever seen above 1 for a table, tiers are genuinely merging concurrently. The whole feature is gated behind the `SPICE_CAYENNE_DISABLE_PIPELINED_COMPACTION` environment-variable kill switch (forces `max_concurrent_merges` to `1`, i.e. the old single-tier behavior) for a fast, config-free rollback.
...
| `cayenne_compaction_max_concurrent_merges` | key-delete tables: max disjoint protected-snapshot size tiers merged concurrently per pipelined pass (`1` = old single-tier-per-pass behavior); force-clamped to `1` by the `SPICE_CAYENNE_DISABLE_PIPELINED_COMPACTION` env var | `1` |
...
|------|-----------------|---------|
| 2026-07-13 | `worktree/pipeline-writing` @ `19ef0063` (pre-PR; doc otherwise baselined at `4685a3dd`) | **Pipelined protected-snapshot compaction** (key-delete tables only): `compact_protected_snapshots_pipelined` merges up to `cayenne_compaction_max_concurrent_merges` disjoint size tiers concurrently per pass from one shared Phase-1 fence read, instead of the single-lowest-tier-per-pass path — letting one table draw more of the process-global encode-permit budget when its `write_concurrency` is smaller than the pool. Each concurrent merge still revalidates `snapshot_at_capture` against `current_snapshot_id` before its RCU publish (same overwrite/promotion guard as the single-tier path). New `cayenne_compaction_subset_tier_total` counter and `cayenne_compaction_subset_inflight` gauge make concurrent tier-merges directly observable; new `SPICE_CAYENNE_DISABLE_PIPELINED_COMPACTION` env var force-disables it (clamps to `1`) as a config-free kill switch. Position-delete tables are unaffected (still route to the single-tier path). Added to Flow F and Appendix B. |
