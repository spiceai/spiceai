...
Restore is a metastore read, not a rescan. At open, `load_table_statistics` reads the aggregate blob (`get_table_statistics`) and deserializes it straight into a `Statistics`, and the per-file layer comes back from `cayenne_snapshot_file_statistics`. The raw Vortex data is never re-read to rebuild statistics, with one bounded exception on the warm path: a per-file blob that carries no per-column byte size *on any column* is re-inferred from that file's footer rather than served at a size the footer disagrees with. That is what a blob written before the byte sizes were persisted looks like, and what a file whose footer carries no size statistics produces; both re-infer on first touch after each cache clear (a pre-size blob is then rewritten with its sizes; the other recurs) and are then held by the in-memory statistics cache. The freshness test is per-column and deliberately not the restored `total_byte_size`, which is summed with an absorbing `Precision::add`: a file missing a column the table schema has can never restore a total, so testing the total would re-read and rewrite every pre-widening file of a widened table on every cold scan, for the life of the file. Such a file still carries sizes for the columns it does have and is served from its blob; the planner's own `total_byte_size` is re-derived from the per-column sizes either way, so the two sources agree on what a scan reports. The cold listing path has no footer to fall back to and serves such a blob as it is, until the next promotion rewrites it.
