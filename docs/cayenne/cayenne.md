...
Everything called "warm" so far means the local-disk Vortex files of the current and compacted snapshots. An optional third tier sits below them: a **cold object store**, enabled by setting `cayenne_cold_tier_location` (dormant and byte-identical to the warm-only behavior when unset). With it on, the tiers form a strict cascade — RAM mem-tier → warm local disk → cold object store — with every row resident in exactly one tier.

A dedicated background worker, `BackgroundColdTierPromoter`, runs on its own `cayenne_cold_tier_background_interval_ms` cadence and promotes the warm tier to cold once it crosses the `cayenne_cold_tier_warm_max_bytes` / `cayenne_cold_tier_warm_max_files` thresholds. Promotion reuses the existing overwrite machinery: it re-materializes the *whole visible table* (all deletes applied, one version per key), **Z-order (Morton) clusters** the rows so multi-column zone maps stay tight, writes read-optimized Vortex at a larger cold file size, and then — in one metastore transaction — registers the new cold files in `cayenne_cold_tier_file` and overwrite-clears the warm tier. A cold graduation is therefore just "an overwrite whose content lives on the cold store," correct by construction. The clustering key defaults to the primary key or is set with `cayenne_cold_clustering_columns`; the kernel builds order-preserving per-column keys and MSB-first bit-interleaves them, applied through the ordinary `SortExec` path without materializing a clustering column.
...
| `cayenne_cold_target_file_size_mb` | datalake Vortex file target size | `512` |
| `cayenne_cold_tier_warm_max_bytes` / `_max_files` | warm→datalake promotion triggers (size / file count) | 16 × cold target file size / (unset) |
| `cayenne_cold_tier_background_interval_ms` | datalake promoter cadence | `60_000` |
