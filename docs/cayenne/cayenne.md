...
A snapshot, then, is not a self-contained copy of the table; it is a *directory of immutable Vortex files plus a `deletions/` subdirectory*, whose membership is defined by the manifest rows in `cayenne.db`. Publishing a new snapshot is a metastore transaction that points at a new set of file paths — many of them shared, unchanged, with the previous snapshot — which is why a pointer flip or a compaction is cheap, and why the handful of retained protected snapshots do not each cost a full table's worth of disk. The per-table Vortex segment cache follows the same physical identity: its key is the exact object-store path plus the Vortex segment id. Snapshot cleanup invalidates only paths proven retired by the manifest and the current/protected/in-flight snapshot guards, after the catalog transition has committed; a shared path or a losing rewrite therefore keeps its cached segments. Cleanup explicitly drives Moka's pending maintenance before it completes, so an idle table releases the retired buffers without waiting for another cache access.
...
|------|-----------------|---------|
| 2026-08-12 | PR for #12936 (pre-merge; doc otherwise baselined at `44d7538e`) | **Retired Vortex files now leave the per-table segment cache when snapshot cleanup proves their paths dead.** The cache enables Moka predicate invalidation and the persistent-format layer exposes exact-path invalidation. Full-refresh and full-compaction cleanup, protected-snapshot retirement sweeps, and their S3 equivalents feed it only file paths that survive the same current/protected/in-flight and manifest-reference safety gates used for physical deletion; unrelated live paths remain cached, and failed or fenced-out rewrites never retire their live inputs. Cleanup also drives Moka's pending maintenance before returning, so idle tables physically release retired buffers without needing a later cache access. No TTL, whole-cache clear, schema, or config change. *On-disk layout and snapshot identity* updated. |
