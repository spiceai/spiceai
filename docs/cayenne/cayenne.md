...
Compaction is gated by a **shared per-accelerator semaphore** (sized at the runtime's CPU budget, `cpu_budget().cores()`), so a fleet of tables can't oversubscribe the writer pool. It `try_lock`s the write lock and skips if a writer is active; the `compaction_lock` serializes passes so write-driven and background-driven runs never overlap. Key-delete tables compact *concurrently* with their writers.
...
|------|-----------------|---------|
| 2026-08-06 | PR for issue #12531 (pre-merge; doc otherwise baselined at `44d7538e`) | **The tuner's memory-pressure signal now measures unreclaimable demand, not the total cgroup charge.** `sample_mem_pressure` divided `memory.current` (v1: `memory.usage_in_bytes`) by the budget, and that charge counts the file-backed page cache a table's own Vortex writes leave behind — cache the kernel drops on demand rather than OOM-killing for. On a CH-benCHmark SF-1000 runner that read 215.6 GiB of a 256 GiB limit (0.842) against 152.5 GiB of `anon` (0.596, below `MEM_PRESSURE_OK`), with 50 µs of reclaim stall over the container's entire life; sampled across a run, 19% of readings sat at or past `MEM_PRESSURE_CRITICAL` on MySQL (28% on Postgres) with PSI at zero. Each such reading collapsed `mem_tier_max_bytes` to its 64 MiB floor and `inline_flush_max_bytes` to 2 MiB, and a floor-sized tier spills continuously — 24 spills totalling 465 s of a 908 s window on `order_line`, long enough on MySQL for the shared binlog pump to stop reading its socket and the source to drop the connection past `net_write_timeout`. `current_memory_bytes` now subtracts the freely-reclaimable page cache read from `memory.stat`: `file` less `shmem` (tmpfs needs swap), `file_dirty` and `file_writeback` (writeback must finish first), leaving anon + kernel + socket as demand. It is built from those totals rather than the kubelet-style `current − inactive_file` because the LRU counters do not reconcile with their own totals in this environment (`inactive_file` 194.3 GiB against a `file` of 61.1 GiB that contains it). An unreadable or `file`-less `memory.stat` subtracts nothing, so the estimate degrades to today's raw charge rather than to an unknown signal. Separately, the query-latency goal's `target_vortex_file_size_bytes` raise — the one grow move that carried no memory gate — now takes the same `mem_ok` gate as the rest, so the memory rule can no longer drive the mem-tier cap to its floor while an unrelated rule grows the file target it feeds 16× to its ceiling. That gate is now an enforced invariant rather than a convention: `Actuator::consumes_memory` classifies every actuator by whether raising it costs resident bytes (an exhaustive match, so a new actuator cannot skip the question), and one sweep test drives both ladders across the violating snapshots at 0.95 pressure and asserts no memory-consuming actuator is ever raised — the check that would have caught this raise and the write-concurrency raise before either shipped. Also removes a duplicate memory sample: the background tick sampled pressure in `observe_environment` and again in `retune` microseconds later, which as of this change re-reads `memory.stat` for no new information and let the controller act on a different reading than the one the tick exported as a gauge. No schema or config change. |
