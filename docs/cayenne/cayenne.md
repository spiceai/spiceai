...
Both axes classify every component kind that creates a Cayenne table: datasets, views, and a catalog with an `acceleration` block. A catalog is configured by a different type (`CatalogAcceleration`) but expands into one dataset acceleration per table it discovers, and it is classified through exactly that expansion — so a catalog-accelerated pod is budgeted on the same terms as the equivalent set of datasets. `changes` is the only refresh mode a catalog admits, so it always reaches the CDC tier; whether it takes the compaction carve follows its storage `mode` like any other table's.
