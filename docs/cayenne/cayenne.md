...
**Pipelined apply** (opt-in `cdc_apply: pipelined`, memory-durability only; default `inline` is byte-identical). Under sustained CDC load a slow hot table's apply can back-pressure the *shared* source reader: the reader routes the combined WAL to bounded per-table channels, so if one table's apply can't drain its channel the reader blocks on send, stops reading the slot, and starves every co-located table. Pipelined mode breaks that coupling — the recv+coalesce loop (Stage A) hands each coalesced burst to a spawned Stage-B applier over a bounded handoff and returns immediately to draining its channel, so a slow apply runs off the recv path. Stage B applies bursts strictly in source order and defers the source-slot ack behind the covering durable checkpoint exactly as the inline path does (the committer is enqueued only after Stage B's apply produces the mem-tier epoch, never in Stage A). Back-pressure, when Stage B genuinely cannot keep up, is bounded to that one table's handoff — never the shared reader. The cayenne-side apply is correspondingly split into an off-`write_lock` prepare (stream drain + PK-shard split) and a lock-held apply (sharded PK-index build + per-shard validate + append), moving the PK-encode/shard off the critical section.
...
|------|-----------------|---------|
| 2026-07-21 | pipelined-mem-apply branch (pre-merge; on top of PR #11948) | **Pipelined memory-tier CDC apply** (opt-in `cdc_apply: pipelined`; default `inline` = byte-identical). For `cdc_durability: memory` tables, the recv+coalesce loop (Stage A) hands each coalesced burst to a spawned Stage-B applier over a bounded handoff, so a slow hot table's apply runs OFF the recv path and no longer back-pressures the SHARED CDC reader (which otherwise stops reading the slot and starves every co-located table). Stage B owns validate+append plus the deferred source-slot ack — the committer is enqueued only AFTER Stage B's apply produces the mem-tier epoch (never in Stage A), preserving the durable-fence deferral. Cayenne-side, the sharded in-memory apply is split into an off-`write_lock` **prepare** (stream drain + `split_batch_by_pk_shard` PK-encode/shard) and a lock-held **apply** (build sharded PK index + per-shard validate + append), moving the PK-encode/shard off the critical section (the `ShardedPkIndex` take-from-cache stays serial under the lock). New `cdc_apply` runtime param (`inline`\|`pipelined`), resolved global→env→per-dataset overlay. Default (`inline`) path and every non-memory table are untouched. See *The CDC mem-tier* (Level 0). |
...
| 2026-07-17 | PR #11948 | **Scan-view maintainer**: scans now borrow a per-table background maintainer's published `Arc<ScanView>` through a **non-blocking, bounded-stale** gate instead of recomputing the merge-on-read view per-scan — retiring the two per-scan `ArcSwapOption` memos (`merged_scan_deletions`, `mem_tier_visible_memo`), the N==1 append-lockstep, and the `MergedScanDeletions` struct. The maintainer computes the merged file∪mem-tier deletion snapshot + deletion-filtered visible mem-tier segments ONCE per table version, off the query cores in `spawn_blocking`, throttled by `CAYENNE_SCAN_VIEW_REFRESH_FLOOR_MS` (env var, default 1000 ms, jittered ±25% per-table). A scan serves the latest published bundle without waiting on ordinary CDC churn (freshness bounded by the floor, inside the freshness SLO); it WAITS only across a **live schema-evolution** — the sole event that bumps `StructuralVersion` (the generation the scan gate keys on), because its all-shards mem-tier flush runs OFF the listing fence and could otherwise tear a straddling capture. Every other change, including the forced snapshot events (truncate / full-delete / `INSERT OVERWRITE` / reopen), is fence-serialized and bumps only the additive `scan_input_version`, so it is served bounded-stale like ordinary churn (no wait). `RawScanInput` / `ScanView` / `build_scan_view` factor the capture + build; the `StructuralVersion` seqlock (`read_validated_async`, loom-modeled) brackets the schema-evolve widen so a straddling capture is discarded rather than published torn; a provider without a running maintainer (unit-test / pre-spawn) builds fresh inline. Adds `cayenne_scan_view_builds_total` / `_build_duration_ms` / `_waits_total` / `_wait_duration_ms` metrics and a `CancellationToken` shutdown (a parked scan wakes rather than hangs). Updated *Other managed state*, the seal memo-transparency note, the scan overview, and the cheat-sheet. |
