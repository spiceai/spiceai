...
|------|-----------------|---------|
| 2026-07-13 | (pre-merge; doc otherwise baselined at `4685a3dd`) | Targeted **datalake (cold) tier** correctness update: promotion now **drains in-flight pipelined Stage-B publishes** under the table `write_lock` before capturing the visible set (bounded wait shared with live schema evolution — `STAGED_WRITE_DRAIN_TIMEOUT`; on timeout the pass is skipped and the next tick retries). Closes a silent-loss window: a staged batch whose Stage-A sequence row was already durable (source slot acked) but whose publish was pending was invisible to the promotion capture while the overwrite-clear deleted its sequence row — the batch survived in memory but vanished at restart. Pinned regression tests in `cold_tier_test.rs` (`test_cold_tier_promotion_preserves_pending_stage_b`, `…_racing_stage_b_finalize`); the promotion paragraph in *the cold tier section* updated accordingly. |
