...
**Delete-all / TRUNCATE.** A `TRUNCATE` (or any CDC delete-all) propagates as `delete_from` with an always-true filter (`DELETE … WHERE TRUE`). That filter cannot enumerate keys, so the `InlineAwareDeletionSink` tombstones only the durable file rows and catalog-inlined data — the standard tombstone-across-tiers a *per-key* delete gets does not apply. Un-checkpointed rows still resident in the RAM mem-tier would therefore survive the truncate and keep appearing in scans (a `SELECT count(*)` after `TRUNCATE` returning non-zero — [#11987](https://github.com/spiceai/spiceai/issues/11987)). To close this, a delete-all additionally **purges every mem-tier shard wholesale** (`purge_mem_tier_all`, gated on the tautology filter and run under the sink's `write_lock` so no concurrent apply races it): it drops all shard segments, returns their bytes to the global budget, resets the resident-row count, and — because the rows are *discarded*, not flushed durable — fires the slot advancer for the discarded epoch so deferred source committers still release (the same `#11644` stall-avoidance the checkpoint path observes). This works identically in `memory` mode, where a checkpoint is a no-op and could never move the rows to a file for the file sink to reach. (Scope: this covers the delete-all path; a *filtered* user `delete_from` — `WHERE pk = …` or a range — still only reaches the file/inline tiers, so a matching row resident in the mem-tier is not yet removed on that path.)
...
|------|-----------------|---------|
| 2026-07-23 | PR for issue #11987 (pre-merge) | **TRUNCATE / delete-all now purges the CDC mem-tier.** A `DELETE … WHERE TRUE` (how a CDC `TRUNCATE` propagates) cannot enumerate keys, so the `InlineAwareDeletionSink` left un-checkpointed RAM mem-tier rows resident and scans kept unioning them in (`SELECT count(*)` after `TRUNCATE` returned non-zero). `InlineAwareDeletionSink::delete_from` now detects the tautology filter (`is_delete_all`) and, under its `write_lock`, calls the new `purge_mem_tier_all` — clearing every shard, releasing the bytes to the global budget, resetting the resident-row count, and firing the slot advancer for the discarded epoch so deferred source committers still release (works in `memory` mode too, where a checkpoint is a no-op). *Flow D — Delete* gains a **Delete-all / TRUNCATE** paragraph. Known follow-up: a *filtered* user `delete_from` (per-key / range) still does not reach mem-tier-resident rows. |
