...
Every original predicate, residual equality, and join filter still runs on the candidate rows, so the covering path can never add a row. `EXPLAIN` exposes a selected covering path as `CayenneIndexScanExec` and `CayenneIndexJoinExec`; an accepted covering plan reads resident key/payload pages rather than Vortex data files. A multi-way selective join can therefore contain one index scan and an index join for each admitted join edge. `HashJoinExec` and Vortex scans remain in the plan for every declined edge. The existing address index remains the fallback for a literal lookup while coverage is unavailable and reports `lookup_index=...` on `CayenneAccelerationExec`; that is not a covering scan. Each actual address-index probe is counted on `cayenne_lookup_index_probe_total{table, shape, outcome}`, with `outcome` one of `selected`, `empty` (the indexed snapshot has no candidate for the key), `unbuilt` (no index covers the rows read) or `snapshot_mismatch` (the index no longer matches the table's files). In file mode, newly appended or still-inline rows are scanned separately until they are included in a rebuilt index, so candidate counts describe the address-index-pruned part of the plan rather than the whole table.
