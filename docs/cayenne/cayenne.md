...
- **Scan-view maintainer** (`run_scan_view_builder_loop`, `scan_view`) — a per-table background task that computes the merge-on-read scan input ONCE per table version, off the query cores, and publishes it as an immutable `Arc<ScanView>` (the merged file∪mem-tier deletion snapshot + the per-segment deletion-filtered visible mem-tier batches + the tombstone union) for scans to borrow. This retires the two per-scan `ArcSwapOption` memos (`merged_scan_deletions`, `mem_tier_visible_memo`) and the N==1 append-lockstep it maintained: instead of every concurrent scan recomputing the merge on a cold memo (the thundering herd that hurt worst under a query burst on the CPU-starved box), one background build serves them all. The maintainer captures the live input under one `listing_fence.read()` (`capture_raw_scan_input`), decides whether to rebuild by a hash-free `RawScanInput::changed` (per-shard mem-tier `version` + file-index pointer + structural epoch + snapshot id; per-shard `version` — not `Arc::ptr_eq` — so a seal, which swaps the shard `Arc` but preserves `version`, does not trigger a rebuild), runs the CPU-bound build in `spawn_blocking` (never stalling a tokio worker), and self-throttles to at most one build per `CAYENNE_SCAN_VIEW_REFRESH_FLOOR_MS` window (default 1000 ms, jittered ±25% per-table so the maintainers do not rebuild in lockstep and spike CPU contention against the apply). Mutations wake it with a cheap `Notify` (`scan_input_change`, wired at every scan-input-changing site) and bump a monotonic `scan_input_version`; the `Notify` — not `scan_input_version`, which the builder loop does not read — is what wakes the builder, and it then rebuilds only when `RawScanInput::changed` detects a real change. **Scans are NON-BLOCKING**: a scan serves the LATEST published bundle and never waits on the maintainer to catch up to the newest mutation — freshness is bounded-stale by the refresh floor, well inside the freshness SLO. This keeps analytical scans off the maintainer's critical path; an earlier linearizable-on-input gate instead made scans wait through rebuild cycles (an SF100 HTAP measurement showed multi-second per-scan waits) and let the builder thrash cores away from the CDC apply. A scan WAITS in exactly one case: a **live schema-evolution** — the sole event wired to bump the `StructuralVersion` seqlock in this PR, because its off-fence all-shards mem-tier flush could otherwise tear a straddling capture — has advanced the structural generation past the published bundle, so the scan waits for the post-evolution publish to see the new schema (`ScanView` carries the generation it was built at). The other forced snapshot events (truncate / full-delete / `INSERT OVERWRITE` / reopen) are fence-serialized and bump only the additive scan-input version, so — like ordinary appends/deletes/upserts — they never make a scan wait and are served bounded-stale. The seqlock also discards a capture that straddled the schema-evolve rather than publishing a torn bundle. A scan sees a bundle GC-protected for its whole lifetime by the shared `SnapshotScanRef` the capture minted under the fence.
