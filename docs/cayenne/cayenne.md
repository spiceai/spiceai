...
**Which datasets share a metastore.** `cayenne_metadata_dir` resolves per dataset, in order: the explicit parameter, else `{cayenne_file_path}/metadata` when that path is local, else `{spice_data}/metadata`. Every dataset that resolves to the same directory shares one `cayenne.db` and appears as its own row in that metastore's `cayenne_table`; datasets resolving to different directories are fully isolated, each with its own metastore, snapshot pointers and cold-tier manifest. So sharing is the default for a pod whose datasets set no storage parameters (they all land on `{spice_data}/metadata`), while datasets given distinct local `cayenne_file_path`s — or distinct explicit `cayenne_metadata_dir`s — each get their own. Neither layout is preferred: what matters is that a dataset is always opened against the metastore its own resolved directory names, so its tables cannot be created in, or looked up from, a database its configuration does not point at.
...
Every metric below is emitted on the `cayenne` OpenTelemetry meter and reaches `spiced --metrics <addr>` at `/metrics`. Per-table metrics carry `table`; metastore-wide metrics carry `catalog` (the metastore path), because a metastore is shared by every dataset resolving to the same `cayenne_metadata_dir` and its file is always named `cayenne.db`, so only the path distinguishes one from another.
