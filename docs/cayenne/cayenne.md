...
| `cayenne_metadata_dir` | directory holding the metastore (`cayenne.db`). Must not sit inside *any* acceleration's data directory, nor be reached through a symlink inside one — that directory is deleted wholesale on a `file_create` bootstrap and on a schema recreate, and the metastore is shared by every Cayenne dataset in the instance, so the accelerator refuses such a layout at load | `{cayenne_file_path}/metadata` when it is local, else `{spice_data}/metadata` |
...
|------|-----------------|---------|
| 2026-08-14 | PR for #13068 (pre-merge; doc otherwise baselined at `44d7538e`) | **A data directory that contains the metastore is now refused at load, and no teardown can delete one.** Both Cayenne teardown paths — the `file_create` bootstrap and the `drop_table` behind a schema recreate — `remove_dir_all` the acceleration's data directory, and the metastore is one catalog shared by every Cayenne dataset in the instance, holding their manifests, snapshot pointers and partition rows. With stock defaults a dataset *named* `metadata` resolves its data directory onto `{spice_data}/metadata`, which is exactly where the shared catalog lives, so bringing that dataset up destroyed the metadata of every other one. Deletion now goes through a single helper that first proves no metastore lies at or below the directory, and `init` refuses the layout at load — while the operator can still change the configuration — with an error naming the dataset, the data directory and the metastore at risk. The check compares canonicalized paths, so a `file://` spelling, a trailing slash, a relative path or a `..` detour cannot walk around it, and a symlink followed by `..` resolves the way the kernel will when it performs the deletion. Three directories are treated as at-risk by name: the metastore this dataset's params resolve to, the instance-wide default, and — authoritatively — every directory that has actually opened a catalog, since whichever dataset gets there first decides where it lives for all of them. Naming cannot be complete on its own, though: a metastore configured by a dataset that has not initialized yet is in none of those sets, so under plain sequential startup a `file_create` dataset over `/x/orders` would delete a catalog another dataset had configured at `/x/orders/catalog`. The teardown therefore also **reads the directory** immediately before removing it and refuses when a `cayenne.db` (or a `-wal`/`-shm` sidecar) lies anywhere beneath, which requires no knowledge of who configured what and so cannot be defeated by load order. The disk read does not follow symlinks, matching `remove_dir_all`, which unlinks a link rather than descending it, so an unconfigured catalog reachable only *through* a link survives the teardown and does not block it; an unreadable entry refuses the deletion rather than being read as an empty directory. A *configured* metastore reached through a link inside the data directory is refused instead of skipped, and by name rather than on disk: unlinking that link costs the catalog even though the file survives, because the next `create_dir_all` puts a real, empty directory where the link was and opens a fresh catalog inside it, leaving every dataset's manifests on disk with nothing able to reach them. The by-name half therefore checks the symlinks along the way to each candidate metastore as well as where it resolves; plain directory components are not checked, since a teardown re-creates those where they were and the path still names the same directory. The on-disk read runs only at teardown, not at load, so no Cayenne dataset pays for a directory walk at startup. A metastore *beside* the data directory (the normal layout) and an object-store data path are both unaffected. See the `cayenne_metadata_dir` row in the parameter table above. |
