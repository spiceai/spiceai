...
|------|-----------------|---------|
| 2026-08-12 | PR #12940 (pre-merge; doc otherwise baselined at `44d7538e`) | The scan memory charge now counts the splits Vortex decodes concurrently, rather than one batch per output partition. New parameter `cayenne_scan_concurrency` (`auto` / `off` / `<n>`). |
| 2026-08-12 | tuning-mode default (pre-merge; doc otherwise baselined at `44d7538e`) | `auto` is now the tuning mode unless a dataset sets `cayenne_tuning: adaptive`; a `cayenne_goal_*` SLO no longer implies the closed loop. The mem-tier knobs also accept the literal `auto` as "unset". |
| 2026-08-10 | PR #12759 (pre-merge; doc otherwise baselined at `44d7538e`) | Completes the scan-materialization charge: the install gate now requires a file-backed source with files to read, and the `ResourcesExhausted` refusal names the dataset, the bytes needed, and which remedy applies. |
| 2026-08-10 | PR for inlined overwrites (pre-merge; doc otherwise baselined at `44d7538e`) | The inline corpus is now null-filled and cast to the live schema on decode, fixing a scan that failed outright after open-time schema widening. |
| 2026-08-08 | PR for #12820 (pre-merge; doc otherwise baselined at `44d7538e`) | The Turso metastore pool now clears any open transaction and confirms autocommit at acquire time, so a borrower cannot inherit an abandoned `BEGIN CONCURRENT` snapshot and read stale rows. |
| 2026-08-08 | PR #12802 (pre-merge; doc otherwise baselined at `44d7538e`) | PK keyset budgets are now bounded process-wide (`pk_keyset_budget`) rather than per table, which let a multi-table pod exceed host memory while every table stayed inside its own limit. Also corrects this section: only the commit mirror is bounded during insert. |
| 2026-08-08 | PRs #12759 + #12790 (pre-merge; doc otherwise baselined at `44d7538e`) | Scan materialization is charged to the query pool, and the sharded PK keyset budget is enforced during the insert rather than after it. Peak RSS was unchanged at SF-1000 — the CDC apply path remains unaccounted. |
| 2026-08-08 | PR for the catalog include/exclude fix (pre-merge; doc otherwise baselined at `44d7538e`) | A Cayenne catalog now applies its `include`/`exclude` patterns at both points a table name can enter it; DDL registration is deliberately not filtered. New *Catalog visibility* section. |
| 2026-08-07 | PR for DDL-partition interval compaction (pre-merge; doc otherwise baselined at `44d7538e`) | A `CREATE TABLE … PARTITIONED BY` partition now runs an interval background compactor on the process-wide compaction budget, instead of consolidating only while it was being written to. |
| 2026-08-07 | PR for the carry-forward rewrite input (pre-merge; doc otherwise baselined at `44d7538e`) | A carry-forward rewrite now reads the manifest rows it classified, rather than intersecting them with the scan's later capture, closing a silent row-loss window (#12708/#12709). |
| 2026-08-05 | PR for the cold-tier scan/promotion pairing (pre-merge; doc otherwise baselined at `44d7538e`) | A cross-tier read now captures the cold manifest in the same fenced instant as the warm snapshot, so a scan straddling a promotion can no longer count promoted rows twice. |
| 2026-08-04 | `b0b88e9b3d` + working-tree fix for issue #12013 (pre-merge) | Both protected-snapshot merge paths now cap selected input bytes at 4x the memory pool the pass accounts against, so an arbitrarily large run set can no longer OOM the host mid-merge. |
...
| 2026-08-01 | PR for the full-refresh write profile (pre-merge; doc otherwise baselined at `44d7538e`) | `refresh_mode: full` gets its own bulk-overwrite write profile with background compaction off, and host-memory sizing now follows the write profile. The per-table Vortex segment cache is counted in the off-pool reservation. |
| 2026-08-01 | PR #12192 (pre-merge; doc otherwise baselined at `44d7538e`) | The sharded PK keyset byte budget is now enforced, degrading an over-budget upsert index to per-shard blooms, and per-entry accounting charges what the map actually stores (96 B, not 56 B, for an 8-byte key). |
| 2026-08-01 | PR for the SF-1000 memory-brake gap (pre-merge; doc otherwise baselined at `44d7538e`) | Write-concurrency raises now respect the memory brake. Without it the tuner added encode shards at 0.98 memory pressure and was OOM-killed at SF-1000. |
| 2026-07-31 | PR for #12208 (pre-merge; doc otherwise baselined at `44d7538e`) | `write_to_snapshot` now creates the snapshot directory itself, making "a snapshot the catalog can reference has a directory" a property of the write rather than a per-call-site guard, and the fused commit takes the directory barrier before publishing. |
| 2026-07-31 | (pre-merge, branch `ben/12211-cpu-limits`; issue #12270) | CPU-derived sizing moved behind one overridable budget (`crates/cpu-budget`), read by five Cayenne sites including the denominator of the adaptive tuner's CPU busy-fraction. Detection is unchanged, so existing deployments size identically. |
| 2026-07-27 | (pre-merge, branch `ben/scan-view-cache`; merge-queue correctness fix) | Bounded-staleness scan views are now scoped to read-only CDC replicas (`access: read` *and* `refresh_mode: changes`); every other table reads its own writes. |
| 2026-07-26 | `8661dfc44` (pre-merge, branch `ben/scan-view-cache`; review fixes) | Closed two live-schema-evolution races in the scan-view cache: the read schema is captured in the same fenced window as the rest of the snapshot, and the read-current fast path checks `structural_version` under the cache mutex. |
| 2026-07-25 | PR #12049 (pre-merge; doc otherwise baselined at `44d7538e`) | `VortexConfig` now records sort-column provenance (`user` / `inferred`, via `cayenne_sort_columns_origin`), so an inference-derived sort order no longer shadows the adaptive layout. Layout precedence is configured > observed > inferred > primary key. |
| 2026-07-23 | `b461cb3f5` (pre-merge, branch `ben/scan-view-cache`; alternative to the #11948 maintainer approach) | A demand-driven scan-view cache replaces the per-scan merge-on-read memos, building a view only for a scan-visible state some scan actually queries. SF1000 CH-benCHmark: QPH +9.4%, replication lag flat. |
| 2026-07-22 | PR #11973 (pre-merge; doc otherwise baselined at `44d7538e`) | Adaptive cold layout is on by default: with clustering and sort columns unset, warm rewrites and cold promotion cluster by the hottest observed filter columns, type-guarded and falling back to the primary key. |
| 2026-07-22 | PR for issue #11933 (pre-merge) | Maintained `SUM`/`AVG` now support `Decimal128`, via exact `i128` accumulators that are invertible on the retract path. |
| 2026-07-16 | PR #11920 (pre-merge; doc otherwise baselined at `44d7538e`) | Datalake cadence parameter renamed to `cayenne_datalake_tiering_check_interval_ms` (hard rename, no alias). Tiering logs reworded to plain language, and routine per-operation chatter demoted INFO to DEBUG. |
...
| 2026-07-15 | PR #11893 (pre-merge; doc otherwise baselined at `44d7538e`) | A freshness violation now shrinks the mem-tier only when apply is also behind arrival and the stream is not mutation-heavy, rather than unconditionally. |
...
| 2026-07-14 | PR #11870 (pre-merge; doc otherwise baselined at `4685a3dd`) | Serializable gated transactions across one or more accelerator-only tables, with per-key OCC and a retryable `WriteConflict`, plus durable federated write-back through the new `cayenne_pending_write_back` table. Appendix A now lists 13 tables. |
| 2026-07-14 | `44d7538e` (committed 2026-07-14; doc otherwise baselined at `4685a3dd`) | Hardened maintained-aggregate retraction and bounds: `SUM` restores SQL `NULL` after its last non-null contribution is retracted, `AVG` validates empty state, and stale views detach without blocking the async visibility fence. |
| 2026-07-14 | `5c1316c7` (committed 2026-07-14; doc otherwise baselined at `4685a3dd`) | Exposed Spicepod `MIN`/`MAX` maintained aggregates with resolved-primary-key validation, and bound scan shard snapshots to the IVM epoch with a seqlock so optimizer substitution cannot cross visibility generations. |
| 2026-07-13 | PR #11720 (pre-merge; doc otherwise baselined at `4685a3dd`) | New acceleration `mode: memory`: fully in-RAM Cayenne with the mem-tier as permanent store and no Vortex, compaction, seal, or datalake. New *Storage modes* section; distinct from `cdc_durability: memory` on file-mode tables. |
| 2026-07-12 | `f059f9cf6e` (recovery fix) + merge of `trunk` | Recovery now rolls a protected-snapshot target back only when it has no durable `cayenne_snapshot_sequence`, instead of classifying from the current-snapshot pointer alone — which dropped durably-committed rows at reopen. |
| 2026-07-11 | `6ed94e7f` + final working-tree audit (pre-merge) | Finalized the atomic-append hardening: mixed or unreadable durable pointer states retain all WALs and files, deletion-vector ownership is cancellation-safe and exact-path, and recovery republishes catalog-backed state for committed protected snapshots. |
| 2026-07-10 | (pre-merge; doc otherwise baselined at `4685a3dd`) | The datalake per-table object prefix changed from the bare `<table_id>` to `<sanitized_table_name>-<table_id>`, so a shared bucket is navigable while staying collision-free. Write and GC prefixes only; reads follow the manifest. |
| 2026-07-10 | PR #11806 (pre-merge; doc otherwise still baselined at `4685a3dd`) | Lazy NDV is now unconditional and the `SPICE_CAYENNE_EAGER_NDV` escape hatch is removed, so *Table statistics* describes the lazy split and its bounded one-memtable lag rather than a per-row fold on the inline write. |
| 2026-07-09 | `2e4693c3` + working-tree audit (pre-merge) | Correctness hardening for atomic cross-partition appends: one shared catalog transaction for all participant state, WAL recovery classified solely from durable catalog pointers, and full same-process state rehydration after an ambiguous commit. |
| 2026-07-09 | PR #11745 (pre-merge; doc otherwise still baselined at `4685a3dd`) | Cold-tier promotion rewritten from whole-table re-materialization to incremental carry-forward, with per-file PK blooms serving keyset rebuilds, mark-and-sweep GC of orphaned objects, and the `cayenne_datalake_*` parameter rename. |
