...
Types Vortex supports at all (`Interval`, `Duration`, `FixedSizeBinary` are the exceptions) are stored as-is, subject to `unsupported_type_action`.
...
|------|-----------------|---------|
| 2026-08-21 | PR #13356 | The encode fan-out is now decided by what a write *is*, not by what its table declares. A table's inferred `cayenne_sort_columns` no longer forces a single Vortex encoder, so the CDC delta write and the protected-snapshot merge — neither of which sorts — encode across cores. The rewrites that do sort declare `EncodeFanOut::Serial` (`rewrite_write_policy`), which is what keeps a globally sorted stream on one writer; a low `target_partitions` cannot, since a configured `cayenne_write_concurrency` is honored above the hint. A sharded write without an explicitly configured shard key now fills each shard with a **contiguous run** of the arrival stream (`ShardSpec::ContiguousRuns`) rather than hash-partitioning on the primary key, so each output file still covers a contiguous slice and keeps its zone maps narrow — parallel encode no longer costs the file pruning a serial write had. |
