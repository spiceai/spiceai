...
| Patch | What breaks if it is lost | Loss | Guard |
|---|---|---|---|
| Inactive acceleration federation must preserve local subquery planning (federation PR #61; runtime boundary for [#14010](https://github.com/spiceai/spiceai/issues/14010)) | Publishing an inert source invokes federation optimization before decorrelation and strands subqueries inside fallback scans. A second, active federation provider can invoke the same optimization, so fallback scans must also decline unplanned subquery predicates | silent (query failure) | `crates/runtime/tests/acceleration/on_zero_results_subqueries.rs`: `duckdb_on_zero_results_subqueries` and `sqlite_on_zero_results_subqueries` execute correlated scalar, `IN`, and `EXISTS` queries with populated and empty acceleration, default-policy controls, and mixed federation. `cayenne_on_zero_results_subqueries` is the local-accelerator control. The guard reads storage directly before querying through the runtime. Requires runtime features `duckdb,sqlite` |
