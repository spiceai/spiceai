...
| [vortex](#vortex) | `3b996ab6cb7a4ea460567a6f2224bf335b04cc9a` | `spiceai-54` |
...
| A timestamp value never goes through a Jiff span (fork PR #93) | The row above converts a date into a count of the target unit; this is the range that count has to land inside, and the same fork PR carries both. A Jiff span's limits are not a timestamp's at either end, so validating or rendering a timestamp through one refuses `i64::MIN` nanoseconds — 1677-09-21, an instant a `timestamp[ns]` column holds and the conversion produces — and aborts on a count past the span range instead of reporting it | silent (wrong data), and abort | `crates/vortex/src/persistent/mod.rs::a_nanosecond_timestamp_scalar_spans_the_whole_i64_range`, which builds that scalar and renders it. The two cast guards above pass on either side of this half, so a re-cut that carried only the cast would not be caught without it |
