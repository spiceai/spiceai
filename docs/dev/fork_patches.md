...
silently disabled optimization rather than a build failure:
...
| Analyzer: a recursive CTE's work table is neutral, not a local table (spiceai/datafusion-federation#84) | The recursive term refers to the CTE by name, and that reference is planned as a `TableScan` over a `CteWorkTable`. Treated like any other unfederated table it takes the placeholder `NopFederationProvider`, and that placeholder is **not** neutral: `ScanResult::merge` leaves `None` alone but turns two different `Distinct` providers into `Ambiguous`. So a query joining a recursive CTE to a federated table merges `Distinct(remote).merge(Distinct(Nop))` into `Ambiguous`, the boundary falls below the join, and only the bare scan federates — even when the CTE reads no table at all and the remote could run the whole statement. Measured on a BigQuery-backed runtime, the analyzed plan came back as `Inner Join` with `Federated` wrapping one side only, the generator and the join both left local. A *non-recursive* literal CTE is unaffected, because `VALUES`/`EmptyRelation` is not a `TableScan` | silent (perf, badly — one statement becomes a local join over separate scans) | in the fork, `datafusion-federation/src/sql/mod.rs::tests::a_recursive_cte_does_not_block_the_join_from_federating`, which asserts the analyzed plan is federated at the root with no `RecursiveQuery` above it and fails on the previous behaviour. It deliberately continues into `create_physical_plan`/`final_sql`: the first version of this patch passed a logical-plan-only assertion and then **panicked the query driver**, because `gather_analyzers` walked every `TableScan` in a federated plan and `expect`ed a federated source from each — true until a work table travelled inside one. The symptom was `Query driver task ended unexpectedly`, a panic on a tokio worker rather than a reportable error. Pairs with the `supports_recursive_cte` row above: federating one is only useful if the dialect can render `WITH RECURSIVE` |
| Analyzer: a pinned expression name moves with the table it names (spiceai/datafusion-federation#84) | An optimizer that rewrites an expression pins its old display name with an alias so the node's schema does not change under it, and that pinned name spells out the *local* table. `rewrite_column` already moves a qualifier that appears inside an unqualified column's name — how a reference to an aggregate's output crosses the rewrite — but the alias underneath was given only its relation, so half the rename happened and the plan stopped resolving: `Schema error: No field named "sum(CASE WHEN <remote>.c …)". Did you mean 'sum(CASE WHEN <local>.c …)'?`. It fires on any federated aggregate whose expression an optimizer rewrote — a coerced literal in a `CASE` is enough — and hit 55 of a 271-statement BigQuery corpus. Note the rewrite is deliberately scoped to *dotted* references: a pinned name always spells `table.column`, while an alias a user wrote can be the table's own bare name and must be left alone | silent (query failure) | in the fork, `datafusion-federation/src/sql/analyzer/rewrite_table_scan.rs::tests::test_rewrite_table_scans_moves_a_pinned_name_with_its_table`, which optimizes a plan so the name is pinned and then rewrites the scan; `::test_rewrite_table_scans_preserve_existing_alias` and `::test_rewrite_multipart_table` are the controls that fail if the rewrite widens to a bare alias name |
| ADBC schema fetch leaves a query's own `WITH` at the top level (fork PR #71) | A driver that does not implement `execute_schema` — the BigQuery one does not — has its query's schema read by executing a row-free form of it, which wrapped the query in a `fetch_schema` CTE. BigQuery accepts `WITH RECURSIVE` only at the top level of a statement, so the wrapper pushed a correctly-federated recursive query one level down and it came back `WITH RECURSIVE is only allowed at the top level of a query`. Nothing upstream of the driver can see this: the plan, the `EXPLAIN` and the unparser output are all correct, and the wrapping happens on the way out | silent (query failure) | `test/scripts/bigquery_pushdown.py::recursive-cte-joined-to-a-table`, which runs a recursive CTE joined to a BigQuery table against the real engine. It has to run the query, not only `EXPLAIN` it: the pushed SQL this patch protects is the SQL the `EXPLAIN` already reports as correct |
