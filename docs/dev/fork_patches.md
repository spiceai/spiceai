...
| Stale `TaskStatus` rejection for reset tasks (fork PR #53) | A status update already in flight when its executor was lost arrives for a task the reset has since failed (`ResultLost`), whose partitions are back on the stage's pending queue for another executor. The scheduler's update path gates on this rejection — `if !update_task_info(..) { continue; }` — so without it a late `Successful` overwrites the reset record and registers shuffle output locations on the executor that is gone, for a partition a live executor is at that moment re-running | silent (wrong shuffle locations) | `crates/runtime/src/cluster/datafusion/mod.rs::stale_status_for_a_reset_task` — `a_status_for_a_task_that_was_reset_is_refused` and `a_status_for_a_task_that_is_still_running_is_accepted`, asserted against `RunningStage::update_task_info` on a stage that `RunningStage::reset_tasks` has reset for the lost executor while another executor's task stays running, so a patch that refused every status fails the second. Driving the real `reset_stages_on_lost_executor` would be closer still, but `ExecutionGraph::pop_next_task` is `#[cfg(test)]` on the fork and unreachable from here |
