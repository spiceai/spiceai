...
This copy is the ledger for **`release/2.2`**, and it says less than `trunk`'s. A
release branch pins the forks it shipped with, so four of them sit at a different
revision here, and a row is only true of the revision it names. Where a patch is not on this branch's pin
there is no row for it, and where its guard has not been carried back the row is a
**GAP**. Both differences are the ledger doing its job; do not reconcile this file
with `trunk`'s by copying rows across.
...
| [iceberg-rust](#iceberg-rust) | `c86f2f7ae9b37a8a795cc4c25574df436aa1d392` | `spiceai-0.10.0-df-54` |
...
| ICU extension statically linked into bundled DuckDB (fork PR #23) | Any query using a named timezone (`AT TIME ZONE 'America/New_York'`) fails at runtime, and DuckDB tries to download the extension from the network | silent (query failure) | `crates/runtime/src/dataaccelerator/duckdb.rs::bundled_duckdb_resolves_a_named_time_zone_without_installing_icu` |
| VSS (HNSW) extension statically linked (fork PR #37) | Vector search over a DuckDB accelerator fails, or silently falls back to a full scan | silent (query failure) | `crates/runtime/src/dataaccelerator/duckdb.rs::bundled_duckdb_builds_an_hnsw_index_without_installing_vss` |
...
| Patch | What breaks if it is lost | Loss | Guard |
|---|---|---|---|
| SQLite backend emits a decimal declared type rather than panicking above 16 digits | `CREATE TABLE` for a `Decimal256(40, 4)` column panics; below that the declared type changes, and the SQLite reader keys value decoding off the declared type | silent (panic / wrong decode) | `crates/runtime/src/dataaccelerator/sqlite.rs::test_sqlite_decimal_round_trip` |
...
Reproduced against a four-row table on the `spiceai-54` branch; the constant-array
cast path is the same code at the pin above. The kernel guard passes, so this is not
caught by the row it belongs to and needs a fix on the fork, not a test here.
