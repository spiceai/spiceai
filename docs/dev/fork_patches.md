...
| [datafusion](#datafusion) | `39f73f4e8f631aba98532ae82d26315caf3dce22` | `spiceai-54` |
...
| [datafusion-table-providers](#datafusion-federation-and-datafusion-table-providers) | `ea4fff58fa756f13a00f4aeae189d63ce97a649e` | `spiceai-54` |
...
| Unparser: `Dialect::decimal_type_to_sql`, and the unparameterized `NUMERIC`/`BIGNUMERIC` BigQuery selects through it — by **both** halves of the width, declining a scale it cannot hold (fork PRs #216, #218) | A cast target carries no precision or scale — `CAST(x AS NUMERIC(38, 9))` is refused as a parameterized type — so the width has to come from the type, and the width arrives from arithmetic so no declared column type reveals it. Measured limits: `NUMERIC` holds 29 integer digits and 9 fractional, `BIGNUMERIC` 39 and 38. The two halves fail differently, which is why the rule needs both: an integer overflow is refused outright ("Invalid NUMERIC value"), but a **scale** past the type's is *rounded away silently*, so a scale past 38 must decline rather than render | silent (query failure), and **silent (wrong data)** if the scale rule is lost | `crates/runtime-datafusion/src/dialect/bigquery.rs::the_wrapper_forwards_every_bigquery_specific_rendering` (the decimal, integer-width and rounded-away-scale arms); in the fork, `plan_to_sql.rs::test_bigquery_spells_a_wide_decimal_bignumeric`, `::test_bigquery_widens_a_decimal_whose_integer_part_overflows_numeric`, `::test_bigquery_declines_a_decimal_scale_it_would_round_away` |
