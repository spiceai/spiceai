...
| [datafusion](#datafusion) | `45b2f1091dffa98f92d87bdeaf50cf3905f73b0f` | `claude/substrait-subquery-scan-alias` (PR #226 head on `spiceai-54`) |
...
| Substrait subquery scans of a table the enclosing scope also reads get their own qualifier (fork PR #226) | SQL names such a scan (`lineitem l2`); Substrait cannot, so both scans were `LINEITEM` and decorrelation resolved `LINEITEM.L_ORDERKEY = outer_ref(LINEITEM.L_ORDERKEY)` to the inner scan alone: the semi/anti join lost its condition and `L_SUPPKEY != L_SUPPKEY` stayed behind. TPC-H q21 returned no rows where the SQL returns one | silent (wrong data) | `tools/substrait-compliance/src/mode_a.rs::correlated_subquery_over_the_same_table_keeps_its_rows` (a same-table correlated EXISTS over three rows returns two; empty without the patch); Mode A harness (`spice-substrait-compliance`) on q21 |
