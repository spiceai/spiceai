...
Fork PR #95 landed with its own differential and pruning tests
(`vortex-array/tests/in_list_differential.rs`,
`vortex-layout/tests/list_contains_pruning.rs`). Those are on the fork branch, so
they leave with the patch — the three `IN`-list rows below are the repo-side half.
...
| `IN` list answered by a set probe from four elements up (fork PR #95) | Upstream OR-s one full-length equality array per list element. The fork keys a set on the list once and probes it in a single pass over the rows, for primitive, `Utf8` and `Binary` needles, keeping the equality form below four elements where one comparison still wins. Two forms answering one operation is the hazard here rather than the speed: the probe's equality is the kernel's own `NativePType::is_eq` rather than a comparison kernel, so a re-cut that carries the probe without that equality, or keys the set under a different width, answers the same query differently depending only on how long its `IN` list is | silent (wrong data) | `crates/vortex/src/persistent/mod.rs::an_in_list_answers_the_same_either_side_of_the_probe_threshold`, which asks one membership question twice — at three elements and at four, the fourth repeating an element so the set is unchanged — across `BIGINT`, `VARCHAR`, `DOUBLE`, `TIMESTAMP` and a `DECIMAL` control the probe declines. It cannot see the probe being absent altogether, because both lengths then take the equality form and agree; that loss costs throughput rather than correctness and is left to the benchmark suites |
| A null element sends an `IN` list back to the equality form (fork PR #95) | A null is not a key on any probe path, so the fork establishes that before anything is decoded, allocated or compressed. A probe that keyed such a list anyway is a key short and answers `false` for rows that match a later element, which is rows silently dropped | silent (wrong data) | `crates/vortex/src/persistent/mod.rs::an_in_list_holding_a_null_still_answers_its_other_elements`, over `BIGINT`, `TIMESTAMP` and `VARCHAR`, asserting the count equals the same list without the null. The same test is the regression guard for a converter defect it found on this pin: `Scalar::list` compares each element's dtype against the list's including nullability, and the list was typed from its first element, so a list holding both a null and a value disagreed with its own declared type and panicked the scan |
| `IN` list statistics falsify a scope by interval (fork PR #95) | An `IN` list used to falsify a scope as an `AND` of one term per element, which made deciding whether a file could be skipped grow with the square of the list length. It is now a single top-level `OR`: the scope lies wholly outside the list's range, or wholly inside one of the gaps between two adjacent sorted elements, since nothing orders between an adjacent pair. That reasoning is unobservable when it is wrong in the dangerous direction — a gap taken one element too wide skips a chunk that holds a matching row, and the query returns fewer rows with no error | silent (wrong data) | `crates/vortex/src/persistent/mod.rs::a_clustered_in_list_does_not_skip_a_chunk_that_holds_a_match`, over three files with disjoint ranges: a list whose endpoints are rows of the first file while its middle elements fall in that file's data gap, a list taking one element from each file, a list lying wholly inside every file's gap, and a list past every range — the last two being what says the falsification still rules scopes out at all |
