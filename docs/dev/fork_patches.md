...
| [datafusion](#datafusion) | `3508c9cf5c85dd1d3f329e6628011fdfefec2587` | `ben/filter-distinct-value-selectivity` (TEMPORARY: spiceai/datafusion#228) |
...
| `FilterExec` estimates an `IN` list or an OR-of-equalities from NDV rather than the flat default selectivity (spiceai/datafusion#228) | Neither spelling is analyzable by interval arithmetic — `check_support` rejects `InListExpr`, and `Or` is not in `is_operator_supported` — so both fall back to `FILTER_EXEC_DEFAULT_SELECTIVITY` and a key lookup on a large accelerated table advertises 20% of the table. The join build side is chosen from exactly that number, so the larger input is mistaken for the smaller one and the oversized-join memory gate then rewrites the join into a sort-merge that sorts a whole input to produce a handful of rows. Measured at TPC-H SF100: `orders` (150M) filtered to 128 keys and joined to `customer` (15M) sorted 15.00 M rows / 2.9 GB for 128 output rows. Results stay correct either way — losing this is a plan-quality regression, not a wrong-rows one, which is why nothing fails loudly | silent (perf) | `crates/cayenne/src/optimizer_rules.rs::tests::in_list_row_estimate::{an_in_list_over_a_unique_key_estimates_the_list_length,an_or_chain_estimates_the_same_as_the_in_list_it_rewrites_to,a_non_unique_key_scales_by_rows_per_value}`, which assert the `FilterExec` row estimate over a scan with a known NDV; the OR-chain case is separate because the optimizer rewrites between the two spellings around a list length of three, so a re-cut that carried only one of them would leave the other broken. `::without_an_ndv_the_estimate_stays_at_the_default` is the control — it passes either way and fails only if a re-cut over-applies the estimate. In the fork, the 18 tests of `filter.rs::tests::distinct_value_selectivity` |
