...
2. Client-side adaptive rate control on the per-origin HTTP rate limiter: AIMD and Google SRE throttling.
...
runtime:
  params:
    # adaptive_rate_control: disabled | enabled (=AIMD) | <float k>1> (=SRE with K)
    http_adaptive_rate_control: enabled
    http_requests_per_second_limit: "20"   # sets the AIMD/SRE ceiling for the origin
...
Provide a small matrix of spicepods (one per top-level scenario group) rather than one giant file: `spicepod.caching.yaml` (rate control disabled, tiny windows), `spicepod.ratecontrol.aimd.yaml`, `spicepod.ratecontrol.sre.yaml` (`http_adaptive_rate_control: "2.0"`), `spicepod.composition.yaml` (both features on), and `spicepod.slow.yaml` (real-duration windows). Duration scaling lives here (Section 8).
...
- AIMD shape: effective_limit halves on failures and rises by ~1 per success (sampled from `adaptive_rate_control_effective_limit`), clamped to the ceiling.
- SRE shape: with `http_adaptive_rate_control: "2.0"`, admission ≈ `min(1, (K*accepts+1)/(requests+1))` within tolerance, using accept/request counts reconstructed from the origin log over the 10s decaying window (`SRE_WINDOW_HALF_LIFE`, `adaptive.rs:52`).
...
5. `ratecontrol-aimd` (spicepod.ratecontrol.aimd). Offered QPS above ceiling; t=30 p2 50% 503; t=120 recover. Expect AIMD halving/recovery, throttled_total rises, p2 upstream arrivals capped, p1 clean.
6. `ratecontrol-sre` (spicepod.ratecontrol.sre, K=2.0). Same timeline. Expect admission ≈ SRE formula over the decaying window.
7. `ratecontrol-cooldown` (spicepod.ratecontrol.aimd). p2 returns 429 + `Retry-After: 2`. Expect retry-after metrics move and a ~2s arrival gap.
8. `ratecontrol-ietf-headers` (spicepod.ratecontrol.aimd). p2 sends only `RateLimit`/`RateLimit-Policy`. Expect NO admission change today (assertion flips when #14136 lands).
9. `topology-isolation` (spicepod.composition). Fault p2 only. Expect both p2 datasets throttled/stale, p1 untouched; one shared p2 limiter series.
10. `composition-sie-absorbs-while-backoff` (spicepod.composition). p2 timeout window. Expect: SIE keeps responses `ok=true` STALE while the controller backs off and upstream arrivals collapse — the two features cooperate (cache absorbs user-facing hits while the controller protects the origin).
11. `overshoot-recovery` (spicepod.ratecontrol.aimd). Origin recovers at t=120; load generator ramps a burst. Expect AIMD additive-increase may briefly re-trip the origin (a second, smaller backoff) — assert the re-trip is bounded and converges, not oscillating unbounded.
12. `slow-confirmation` (spicepod.slow, real durations e.g. max-age=60s, swr=120s, sie=1h). One long run mirroring scenario 2/3 to confirm the tiny-window results hold at realistic timescales.
...
3. Phase 2 — metrics scraper + rate-control assertions. Add the scraper and the per-origin adaptive/cooldown checks. Deliverable: `ratecontrol-aimd`, `ratecontrol-sre`, `ratecontrol-cooldown`.
4. Phase 3 — topology + composition + overshoot. Second origin, two p2 datasets, isolation/sharing checks, and the composition/overshoot scenarios.
...
3. Rate control config location — the adaptive knob is a `runtime.params` key (`http_adaptive_rate_control`), applied per origin, NOT a per-dataset param. The task framed it as `adaptive_rate_control`; confirm whether a per-dataset override is also intended on the `14136` branch. If the branch adds a dataset-level param, the spicepods in 4.3 need a second variant. (Branch is uncommitted; re-verify at build time.)
4. Ceiling semantics — the AIMD/SRE ceiling derives from the max of configured rps/rpm/concurrency, else 100. The harness must set an explicit `http_requests_per_second_limit` so the ceiling is known for the AIMD-shape assertion; otherwise assertions must target the default 100.
