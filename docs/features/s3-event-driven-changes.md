...
1. If the accelerator is provably empty, Spice lists the dataset `from:` prefix **once**, applies those objects as creates, and records **that same listing** as the applied-key set so the completeness backfill can skip them. Snapshot and `history_unavailable` rebuild always use the `from:` prefix (the federated table), not a nested `s3_changes_key_prefix`. It does not seed applied keys from a later listing. Objects that arrive after that listing stay eligible for SQS or the next backfill. If the snapshot is interrupted, the next start sees a non-empty accelerator and takes the rebuild path below.
2. If the accelerator is **not** empty (restart after downtime, or `AccelerationContents::Unknown`), Spice records object keys from a `from:` listing taken **before** it emits `history_unavailable` so the accelerator is **replaced from the same listing prefix the federated table uses**. It does **not** append every listed object on top of the existing table, and it does not seed applied keys from a listing taken after the replace. SQS is not a WAL; listing is the completeness floor.
...
8. `ObjectRemoved:*` with `s3_on_object_removed: rebuild`: Spice replaces the in-memory applied-key set with a listing taken **before** it emits `history_unavailable` (it does not clear the set). That is a full prefix replace, not a row-level delete.
...
- **Overwrites**: `ObjectCreated` for a key already in the applied set is acknowledged without appending (covers snapshot/queue overlap and SQS redelivery). A later in-place overwrite of that key is not applied by the event path; use a new object key or `s3_on_object_removed: rebuild`.
