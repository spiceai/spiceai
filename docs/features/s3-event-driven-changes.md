...
- **Dot-segment keys**: an object key with a `.` or `..` path segment cannot be expressed as a listing URL (`Url::set_path` / `Url::parse` collapse it). Those notifications are acknowledged without applying, and listing passes omit the object, so Spice never reads a different key.
