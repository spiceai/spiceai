...
2. If the accelerator is **not** empty (restart after downtime, or `AccelerationContents::Unknown`), Spice records object keys from a `from:` listing taken **before** it emits `history_unavailable` so the accelerator is **replaced from the same listing prefix the federated table uses**. Those keys stay in-flight until the replace envelope commits. It does **not** append every listed object on top of the existing table, and it does not seed applied keys from a listing taken after the replace. SQS is not a WAL; listing is the completeness floor.
