...
1. If the accelerator is provably empty, Spice lists the dataset prefix **once**, applies those objects as creates, and records **that same listing** as the applied-key set so the completeness backfill can skip them. It does not seed applied keys from a later listing (an object that appeared after the snapshot listing would otherwise be marked applied without its rows). Objects that arrive after that listing stay eligible for SQS or the next backfill. If the snapshot is interrupted, the next start sees a non-empty accelerator and takes the rebuild path below.
2. If the accelerator is **not** empty (restart after downtime, or `AccelerationContents::Unknown`), Spice records object keys from a listing taken **before** it emits `history_unavailable` so the accelerator is **replaced from the listing prefix**. It does **not** append every listed object on top of the existing table, and it does not seed applied keys from a listing taken after the replace (that later listing can include objects the replace never saw). SQS is not a WAL; listing is the completeness floor.
