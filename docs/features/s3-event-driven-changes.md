...
4. Each message is parsed as a direct S3→SQS `Records` body, an SNS-wrapped S3 notification, or an EventBridge S3 detail. `s3:TestEvent` is ignored. A missing EventBridge `detail-type` is rejected (not treated as a create). An EventBridge body whose `source` is not `aws.s3` is rejected (not treated as an object event). EventBridge object keys are used verbatim; only direct S3 `Records` keys are URL-decoded.
