...
- Use `runtime.rate_control.state_location` to persist governor request-rate state per origin in object storage, allowing multiple Spice instances or clusters to share the same HTTP request budget. `s3://`, `abfs://`, and `abfss://` locations are supported; `refresh_interval` defaults to `30s`, and `runtime.rate_control.params` accepts the matching object-store parameters such as S3 `s3_region`/`s3_endpoint`/`s3_key`/`s3_secret` or ABFS `account`/`access_key`/`client_id`/`tenant_id`/`endpoint`
