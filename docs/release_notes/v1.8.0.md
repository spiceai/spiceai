...
Spice v1.8.0 unlocks write-enabled datasets with automatic cache invalidation, scales vector search for larger workloads, and delivers another round of reliability improvements across the runtime.

## What's New in v1.8.0

**Partitioned S3-vector indexes**: S3-backed vector indexes now support partitioned layouts and chunked ingestion, enabling faster refreshes and lower latency.

**Write-enabled datasets with smart caching**: Datasets marked `access: read_write` now support `INSERT INTO` statements while automatically invalidating cached results affected by the write. This allows interactive updates without risking stale query responses.

**Vector search durability and scale improvements**: Refined logging, stricter default limits, and safeguards against index-only scans keep large searches fast and predictable while ensuring metadata remains accessible.

**AI UDF hardening**: The `ai` table function now enforces model-only usage and applies security and performance optimizations so LLM-powered workloads remain safe by default.

### Bug Fixes

- Resolved mismatched column length errors and improved logging around the `ai` UDF to keep AI workflows stable.
- Fixed partial data writes and network readiness checks in connectors such as Iceberg, improving end-to-end ingestion reliability.
- Patched vector search edge cases, including duplicate result handling, embedding column removal, and chunked refresh flows.
- Tightened cache behavior for modification queries and disallowed unsupported `INSERT OVERWRITE`, preventing stale or inconsistent results.
...
- Use 8-core runners for Winsows CUDA builds by @sgrebnov in <https://github.com/spiceai/spiceai/pull/7284>
