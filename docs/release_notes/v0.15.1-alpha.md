...
The v0.15.1-alpha minor release focuses on enhancing stability, performance, and usability. Memory usage has been significantly improved for `postgres` and `duckdb` acceleration engines using stream processing. A new Delta Lake Data Connector has been added in addition to the Databricks Data Connector.
