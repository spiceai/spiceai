...
Spice v1.1.0 introduces full support for the Model-Context-Protocol (MCP), expanding how models and tools connect. Spice can now act as both an MCP Server, with the new `/v1/mcp/sse` API, and an MCP Client, supporting stdio and SSE-based servers. This release also introduces advanced evaluation workflows with custom eval scorers, including LLM-as-a-judge, and adds an IMAP Data Connector for federated SQL queries across email servers. Alongside these features, v1.1.0 includes automatic NSQL query retries, expanded task tracing, and request drains for HTTP server shutdowns, delivering improved reliability, flexibility, and observability.
