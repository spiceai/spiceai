...
Spice v2.3.2 is a patch release. Spice Cayenne makes point lookups and repeated queries faster. SQL results-cache improvements apply to every cached query, regardless of its data source or accelerator. Across two benchmark rounds, the time for a small cache hit fell by 50-52% over HTTP and 44-48% over Flight SQL. For small cache hits, CPU time per server request fell by 58-61% over HTTP and 49-51% over Flight SQL. This release also fixes defects that could return stale or missing rows, such as localpod datasets that read replaced source data.
...
The benchmark ran on an Apple M3 Max with 16 cores. The query returned a seven-row `GROUP BY` result. Across two rounds, cache-hit time over HTTP fell by 50-52%. Over Flight SQL, cache-hit time fell by 44-48%. In one HTTP round, cache-hit time fell from 63.8 µs to 30.8 µs. In one round over Flight SQL, it fell from 55.3 µs to 30.9 µs.
