...
- **ODBC data connector**: Use ODBC connections as a data source using the `odbc:` scheme. The ODBC data connector is currently optional and not included in default builds. It can be conditionally compiled using the `odbc` cargo feature when building from the trunk.
