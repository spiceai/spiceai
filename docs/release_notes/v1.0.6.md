...
Spice v1.0.6 improves stability for DuckDB acceleration, Iceberg Data/Catalog connector improvements when using AWS Glue, and fixes an issue with the `on_registration: source` federation fallback when using DuckDB.
...
- **Fixes On-Registration Fallback when using DuckDB**: Previously, when using DuckDB as a data accelerator and the `on_registration: source` configuration, queries made during the initial data refresh did not properly fallback to the federated source. This is now fixed.
