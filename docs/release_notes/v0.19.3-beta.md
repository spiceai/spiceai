...
Spice v0.19.3-beta introduces the new `localpod` Data Connector, enhanced performance for federated queries across multiple sources, and improved stability of data connectors and accelerators.

## Highlights in v0.19.3

**Improved federated queries containing multiple sources**: Faster queries across multiple federated sources by optimizing when filters are applied.
...
**localpod connector**: Implement "tiered" acceleration pattern by using 'localpod' Data Connector pointing to existing Spice dataset. By using `localpod` the data is available for nultiple datasets with different configuration while pulled only once.
