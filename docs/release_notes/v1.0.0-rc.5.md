...
Spice v1.0.0-rc.5 is the fifth release candidate for the first major version of Spice.ai OSS. This release focuses production readiness and critical bug fixes. In addition, a new DynamoDB data connector has been added and automatic detection for GPU acceleration when running Spice using the CLI.
...
```console
sql> SELECT metadata.registration_ip, metadata.user_agent FROM users LIMIT 5;
+---------------------------------+----------------------------------------------------------------------------------------------------------------------------------------+
| users.metadata[registration_ip] | users.metadata[user_agent]                                                                                                             |
+---------------------------------+----------------------------------------------------------------------------------------------------------------------------------------+
| 71.81.101.73                    | Opera/8.78.(X11; Linux x86_64; ss-ZA) Presto/2.9.186 Version/11.00                                                                     |
| 44.97.163.126                   | Mozilla/5.0 (Windows CE) AppleWebKit/536.1 (KHTML, like Gecko) Chrome/35.0.878.0 Safari/536.1                                          |
| 121.32.180.61                   | Mozilla/5.0 (Linux; Android 6.0) AppleWebKit/534.0 (KHTML, like Gecko) Chrome/23.0.866.0 Safari/534.0                                  |
| 129.101.5.240                   | Mozilla/5.0 (compatible; MSIE 5.0; Windows NT 5.0; Trident/5.1)                                                                        |
| 157.148.178.48                  | Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_10_0 rv:3.0; bem-ZM) AppleWebKit/532.48.1 (KHTML, like Gecko) Version/5.0 Safari/532.48.1 |
+---------------------------------+----------------------------------------------------------------------------------------------------------------------------------------+
```
...
- **Databricks Catalog Connector (Delta Lake):** Graduated to [Beta](https://github.com/spiceai/spiceai/blob/trunk/docs/criteria/catalogs/beta.md).
