...
Spice v1.0.0-rc.4 is the fourth release candidate for the first major version of Spice.ai OSS. This release continues the focus on production readiness and includes a new `spice eval` command, support for the Grok xAI model, and support for the `.tsv` file format.

## Highlights in v1.0-rc.4

- **Grok xAI:** Added support for the Grok xAI model.
...
- **New `spice eval` command:** A new `spice eval` command .... # TODO: Add details

- **`.tsv` file format:** Added support for the `.tsv` file format for various object-store based connectors (i.e. `file`, `s3`, `ftp/sftp`).
...
- **Default Spicepod Version:** Spicepod version `v1` is now the default. `v1beta1` will continue to work.
...
- **GitHub Data Connector:** Graduated to [Stable](https://github.com/spiceai/spiceai/blob/trunk/docs/criteria/accelerators/stable.md).

- **Dremio Data Connector:** Enabled support for connecting to Dremio tables that are nested more deeply than 3 levels:

    ```yaml
    datasets:
      - from: dremio:my.deeply.nested.dremio.table
        name: table
    ```

- **Databricks Spark Connector:** Fixed a bug that prevented the connector from being used with tables that contained `-` in the table name.

- **Iceberg Catalog Connector:** Fixed a bug that prevented selecting a subset of columns from a table, i.e. `SELECT col1, col2 FROM my_table`.
