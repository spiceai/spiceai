...
Spice v2.3.1 is a patch release that improves filtered Vortex scans, cache eviction, query planning, and installation reliability. It also improves subquery fallback, numeric conversion, and Cayenne writes. The MCP server and client support the 2026-07-28 revision of the Model Context Protocol.
...
In the reported TPC-H SF1 benchmark, point-lookup p50 latency improved by 16%, and the same query used 37% less CPU. The sort adds work to each full refresh, so only datasets that configure `sort_columns` pay this cost. See [Full Refresh](https://spiceai.org/docs/features/data-acceleration/refresh-modes/full) for full-refresh configuration.
...
`DELETE`, `UPDATE`, and `INSERT` now update the in-memory tier. The append path also enforces each declared `primary_key` according to its `on_conflict` setting. The [SQL DML reference](https://spiceai.org/docs/reference/sql/dml) lists the supported statements.
