...
Spice v1.0.0-rc.2 is the second release candidate for the first major version of Spice.ai OSS. This milestone continues to build on the stability of Spice for production use, including key Connector graduations, bug fixes, and AI features.

## Highlights in v1.0-rc.2

**MS SQL, File Data Connectors:** have graduated from Alpha to Beta.

**GraphQL Data Connector:** has graduated from Beta to Release Candidate.

**Azure AI support:** for both LLMs & embedding models. Use an Azure AI model like:
...
models:
  - name: azure
    ...
    params:
      endpoint: https://your-resource-name.openai.azure.com
      azure_api_version: 2024-08-01-preview
      azure_deployment_name: jeadie-test
      azure_entra_token: ${ secrets:SPICE_AZURE_AZURE_TOKEN }
```

**Accelerate a subset of columns:** Spice now supports accelerating a subset of the columns from a federated source by specifying the columns in the [Refresh SQL](https://docs.spiceai.org/components/data-accelerators/data-refresh#refresh-sql).
...
**Data Connector Delimiters:** now support `/` and `://`, in addition to `:`.
