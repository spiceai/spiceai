...
- **Elasticsearch Data Connector** with native hybrid search (BM25 full-text + kNN vector + RRF)
...
- **Cayenne Native Vector Search** via SIMD similarity UDFs (AVX-512 / AVX2 / NEON)
...
- **MCP Streamable HTTP** transport (rmcp 1.5.0)
...
### PostgreSQL Native CDC via WAL

[Postgres](https://spiceai.org/docs/components/data-connectors/postgres) datasets configured with `refresh_mode: changes` can now stream changes directly from PostgreSQL logical replication (WAL) into any local accelerator — no Debezium or Kafka required.
...
A new `rerank()` table-valued function reorders scored results from `vector_search`, `text_search`, or `rrf` by a reranker model's relevance judgements.
...
### Cayenne Native Vector Search

[Cayenne](https://spiceai.org/docs/components/data-accelerators/cayenne)-accelerated tables now support `vector_search()` natively without requiring an external index (S3 Vectors, Elasticsearch) or an `embed()` fallback.

**Key capabilities**:

- **SIMD Distance UDFs**: `cosine_distance`, `inner_product`, `l2_distance`, `l2_squared_distance`, `l2_norm` backed by `simsimd` with runtime autodispatch (AVX-512 / AVX2 / NEON / fallback).
- **`DistanceMetric::Dot`**: `inner_product` is now supported (previously rejected at parse time).
- **Zero-Copy Arrow Path**: UDFs operate directly over `Float32Array` with null-aware row propagation.
- **Column Statistics and Data Inlining**: Cayenne now computes and persists per-column min/max/null_count/row_count statistics on every write. Small append-only batches (<1024 rows, <1 MB IPC) are inlined directly in the metastore, avoiding Vortex file overhead.
...
The MCP server has been upgraded to [rmcp 1.5.0](https://crates.io/crates/rmcp) and switched to the Streamable HTTP transport (`/v1/mcp`), replacing the previous SSE-based endpoint. The client-side transport is updated to `StreamableHttpClientTransport`.
...
- **Auth Headers**: The OTEL metrics exporter supports authorization headers for authenticated metric endpoints.
...
- **`flatten_json` / `json_tree` / `expand_maps` UDTFs**: New table-valued functions for JSON transformation, map expansion, and schema decomposition in query pipelines.
- **`cosine_distance` Pushdown to DuckDB**: `cosine_distance` is now pushed down to DuckDB accelerators via `array_cosine_distance`.
- **Snowflake Type Support**: Added support for `OBJECT`, `MAP`, `GEOGRAPHY`, `GEOMETRY`, `VECTOR`, and `TIMESTAMP_LTZ` types.
- **MySQL Zero-Date Behavior**: New `mysql_zero_date_behavior` parameter (`null` or `error`) controls how MySQL zero-date values (`0000-00-00`) are handled.
- **Databricks Timeouts**: New `connect_timeout` and `client_timeout` parameters for `sql_warehouse` mode.
