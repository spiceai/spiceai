...
```mermaid
flowchart TD
    Q["Query: SELECT * FROM recent_orders_summary\nWHERE status = 'cancelled'"]
    Q --> VA["View Accelerated Store\n(Spice Cayenne/Arrow/DuckDB/SQLite)"]
    VA -->|rows > 0| R1["Return Results"]
    VA -->|zero rows| VS["Fallback: Re-execute View SQL\nagainst referenced datasets"]
    VS --> DA["orders — Accelerated Store\n(DuckDB/SQLite)"]
    DA -->|rows > 0| R2["Return Results"]
    DA -->|zero rows +\non_zero_results: use_source| FS["Fallback: Query Federated Source\n(Postgres)"]
