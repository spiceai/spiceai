# Spice v0.14-alpha (June 3, 2024)

The v0.14.0-alpha release introduces GraphQL data connector and improves federated query stability and schema compatibility.

## Highlights

- **GraphQL data connector (#1543)**: Added suport for using GraphQL api as data sources.

- **MotherDuck data connector**: Support for [MotherDuck]([url](https://motherduck.com/)) as a data source.

- **Federated Query Push-Down improvements**:

- **Enhanced Telemetry**: Metrics now include last update time for accelerated datasets, count of refresh errors, and new metrics for query duration and failures, including information about the datasets involved.

- **Query History Logging**: Now enabled for Arrow Flight queries as well as for HTTP queries.
