...
- **Delta-Write Encoding Levels**: A new `cayenne_delta_encoding` setting (default `auto`) selects delta-write encoding, and `compression_strategy: zstd` is now fully wired.
...
- **Secret Resolution at Startup**: Secret references are checked and reported at startup, with a full-fidelity reference-iteration and resolution-status API.
...
- improve error messsage for 'params.spiceai_region' by [@Jeadie](https://github.com/Jeadie) in [#10954](https://github.com/spiceai/spiceai/pull/10954)
