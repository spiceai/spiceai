...
See the [Sharepoint Data Connector documentation](https://docs.spiceai.org/components/data-connectors/postgres).
...
**File Data Connector** Use `from: file:` query files stored by locally accessible filesystems.
...
- **Dataset Time Column Format:** The `time_format` value for [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) is now lowercase (`iso8601`) to be consistent with other values. Previously `ISO8601`. To upgrade, change uses of `time_format: ISO8601` to `time_format: iso8601`.
