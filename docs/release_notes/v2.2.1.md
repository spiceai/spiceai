...
Spice v2.2.1 is a patch release focused on the disk footprint and the correctness of accelerated data. Cayenne accelerations now reclaim the space that maintenance leaves behind instead of growing without bound, several query paths that could return a wrong or missing row are fixed, DuckDB and HTTP datasets keep their memory bounded, and new metrics make Cayenne's maintenance decisions and storage growth visible.

## What's New in v2.2.1

### Cayenne Reclaims Disk and Metastore Space

A [Cayenne](https://spiceai.org/docs/components/data-accelerators/cayenne) acceleration under steady writes could grow on disk indefinitely. Superseded state of several kinds — inline tombstones from upserts, retired snapshot directories, the manifest and statistics rows describing compacted-away snapshots, and orphaned deletion vectors — was made unreachable but never removed. Each gap is now closed, and no configuration change is required. The effect was substantial on write-heavy tables: inline tombstones accounted for most metastore growth in measured runs, and a dataset refreshing every 200 ms accumulated hundreds of dead snapshot directories against a live table under a megabyte.
...
Two behavior changes are worth no\ting before upgrading:
