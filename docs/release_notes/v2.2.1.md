...
- **ADBC federation**: The [ADBC Data Connector](https://spiceai.org/docs/components/data-connectors/adbc) again keeps Spice-only functions such as `json_get_str` out of the SQL it sends to a source, and it honours `query_federation: disabled`. The v2.2.0 move of the connector into its own crate dropped both settings. A query with a Spice-only function against BigQuery then failed with an unknown-function error.
...
- **DuckDB moves from v1.5.5 to v1.4.4** to keep the memory of DuckDB-accelerated datasets bounded under upsert refreshes. SQL syntax and functions that DuckDB added in v1.5 are not available to DuckDB-accelerated datasets.
