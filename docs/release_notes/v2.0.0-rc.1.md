...
- **Synchronous**: Queries are distributed across executors and results are streamed back in real-time. Best for interactive queries where low latency is critical.
- **Asynchronous**: Queries are submitted and results are materialized to object storage for later retrieval. Best for long-running analytical workloads and batch processing.
...
- **Distributed Ingestion**: Data ingestion for accelerated tables is now distributed across executor nodes, enabling higher throughput and parallel data loading in cluster mode.
...
- **`TableProvider` supports `DELETE` and `UPDATE`**: New hooks for `DELETE` and `UPDATE` statements in the `TableProvider` trait, enabling Iceberg and Cayenne DDL support
...
**Full Changelog**: <https://github.com/spiceai/spiceai/compare/v1.11.3...v2.0.0-rc.1>
