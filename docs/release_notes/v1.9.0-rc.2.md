...
**S3 Versioning Support**: The S3 Data Connector now supports [S3 Versioning](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Versioning.html), allowing queries to access specific versions of S3 objects. This feature is enabled by default and ensures that queries return consistent results even when S3 objects are updated or deleted during query execution.

**Key benefits**:

- **Consistent reads**: Queries always read the same version of an object throughout execution, even if the object is modified during the query
- **Deleted object handling**: Objects deleted during query execution can still be accessed via their version ID
- **Enabled by default**: No configuration required - versioning is automatically enabled for all S3 data sources
