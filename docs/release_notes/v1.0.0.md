...
- **Default Runtime Version:** The CLI will install the GPU accelerated AI-capable Runtime by default (if supported), when running `spice install` or `spice run`. To force-install the non-GPU version, run `spice install ai --cpu`, useful in case of hardware limitations or runtime issues.
