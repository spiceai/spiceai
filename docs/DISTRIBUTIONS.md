...
- **Globally Persisted HTTP Rate-Control State** - Persist request-rate state and expiring active instance heartbeats per origin in object storage so multiple Spice instances or clusters coordinate a shared request budget. Local open source builds can enable this with `rate-control`.
