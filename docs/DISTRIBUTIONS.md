...
- **Globally Persisted HTTP Rate-Control State** - Persist governor request-rate state per origin in object storage so multiple Spice instances or clusters share a request budget. Local open source builds can enable this with `rate-control`.
