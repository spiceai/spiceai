################################################################################
# Build Performance: Use all available CPU cores for parallel compilation     #
################################################################################
export CARGO_BUILD_JOBS ?= $(shell nproc 2>/dev/null || sysctl -n hw.ncpu 2>/dev/null || echo 8)
