...
RUN if [ "$INSTALL_ORACLE_ODPIC" = "true" ]; then \
    set -euo pipefail; \
    # Debian Trixie renamed libaio1 to libaio1t64 for 64-bit time_t transition
    apt-get update && (apt-get install -y --no-install-recommends libaio1t64 || apt-get install -y --no-install-recommends libaio1) && apt-get install -y --no-install-recommends unzip curl; \
