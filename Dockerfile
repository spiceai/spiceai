...
RUN \
    ...
    if [ "${CARGO_NO_DEFAULT_FEATURES}" = "true" ]; then \
      cargo build --profile "${RUST_PROFILE}" --locked --no-default-features ${CARGO_FEATURES:+--features ${CARGO_FEATURES}}; \
    else \
      cargo build --profile ${RUST_PROFILE} --locked --features ${CARGO_FEATURES:-default}; \
    fi && \
...
RUN if [ "$INSTALL_ORACLE_ODPIC" = "true" ]; then \
    ...
    # debian:trixie-slim where /bin/sh is dash, which does not support pipefail.
    set -eu; \
    apt-get update; \
    if apt-cache show libaio1t64 2>/dev/null | grep -q '^Package: libaio1t64$'; then \
      LIBAIO_PACKAGE=libaio1t64; \
    else \
      LIBAIO_PACKAGE=libaio1; \
    fi; \
    apt-get install -y --no-install-recommends "$LIBAIO_PACKAGE" unzip curl; \
    ...
    LIBAIO_PATH="$(find /usr/lib /lib \( -name 'libaio.so.1' -o -name 'libaio.so.1t64' \) | head -n 1)" && \
    test -n "$LIBAIO_PATH" && \
    LIBAIO_BASENAME="$(basename "$LIBAIO_PATH")" && \
    cp "$LIBAIO_PATH" "/spice_sandbox/usr/lib/$LIBAIO_BASENAME" && \
    if [ "$LIBAIO_BASENAME" != "libaio.so.1" ]; then ln -sf "$LIBAIO_BASENAME" /spice_sandbox/usr/lib/libaio.so.1; fi && \
