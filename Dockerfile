...
RUN \
    ...
    case "${TARGETARCH}" in \
      arm64) export CFLAGS="-O3 -ffunction-sections -fdata-sections -fPIC" && export RUSTFLAGS="-C target-cpu=native" ;; \
      amd64) export CFLAGS="-O3 -ffunction-sections -fdata-sections -fPIC -march=x86-64" && export RUSTFLAGS="-C target-feature=+avx2,+fma,+bmi,+bmi2,+lzcnt,+popcnt" ;; \
