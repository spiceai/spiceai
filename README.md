...
- 💎 **Spice Cayenne**, the acceleration engine that powers Spice (GA) — high-rate CDC ingestion and low-latency queries on the same table, built on Vortex, with secondary indexes and multi-column clustering, in memory or on disk. **1.5x faster than DuckDB with 3x less memory** on TPC-H SF100, **26x faster than Spice 1.x on TPC-DS SF100**, **100x faster random access vs. Parquet**.
...
- **Whole-database CDC** — a PostgreSQL catalog with `refresh_mode: changes` replicates every table its `include` patterns match, with no per-table configuration (Alpha). [Catalog docs →](https://spiceai.org/docs/components/catalogs/postgres)
...
- **A data layout that prunes** — `cayenne_cluster_by` (or `CREATE TABLE … CLUSTER BY`) clusters rows on one or more columns across every storage tier, and full refreshes write key-range files, so filtered queries open only the files that can match.
