...
# The header of a `Cargo.lock` block. Only `[[package]]` describes something
# cargo builds; a `[[patch.unused]]` block has the same name/version/source shape
# but records a `[patch]` entry the resolve did *not* apply, so the revision it
# names is in no crate's dependency graph. Read as a pin it is a fork pinned at
# two revisions at once — a failure the workspace cannot act on, because there is
# only one revision to audit and the other builds nothing.
LOCK_BLOCK_RE = re.compile(r"^\[\[([\w.]+)\]\]\n", re.M)
...
def built_packages(lock_text: str) -> str:
    """`lock_text` reduced to its `[[package]]` blocks — what cargo actually builds."""
    parts = LOCK_BLOCK_RE.split(lock_text)
    # `split` on one capturing group yields [preamble, kind, body, kind, body, …].
    return "\n".join(body for kind, body in zip(parts[1::2], parts[2::2]) if kind == "package")
...
def pinned_forks(lock_text: str) -> dict[str, set[str]]:
    ...
    for match in LOCK_SOURCE_RE.finditer(built_packages(lock_text)):
