...
docker exec scylladb cqlsh -e "COPY ${KEYSPACE}.lineitem (l_orderkey, l_linenumber, l_partkey, l_suppkey, l_quantity, l_extendedprice, l_discount, l_tax, l_returnflag, l_linestatus, l_shipdate, l_commitdate, l_receiptdate, l_shipinstruct, l_shipmode, l_comment) FROM '/tmp/lineitem.csv' WITH DELIMITER=',' AND NULL='';"
