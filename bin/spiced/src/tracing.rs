...
pub(crate) async fn init_tracing(
    ...
    // `fmt::layer()` writes to stdout, so stdout is the stream whose
    // terminal-ness decides this — not stderr. `NO_COLOR` and `FORCE_COLOR`
    // override it, and the answer is cached process-wide, which keeps the two
    // stacks below and the CLI's own painted output in agreement.
