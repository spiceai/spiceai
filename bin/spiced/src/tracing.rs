    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    // `busy_ns` and `idle_ns` cost a clock read and a span-extension write on every
    // poll of every future instrumented with a task span, and the task history
    // table drops both (see `AUTOGENERATED_LABELS`). Only Zipkin shows them, so only
    // a Zipkin export pays for them.
    let track_inactivity = zipkin_exporter.is_some();
    ...
    let layer = tracing_opentelemetry::layer()
        ...
        .with_tracked_inactivity(track_inactivity)
        .with_filter(filter::filter_fn(|metadata| {
            metadata.target() == "task_history"
        }));
