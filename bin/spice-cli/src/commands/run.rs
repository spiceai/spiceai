...
use crate::error::{Result, RuntimeExecutionSnafu, RuntimeNotInstalledSnafu};
use clap::Args;
use snafu::ResultExt;
...
pub fn execute(ctx: &RuntimeContext, args: &RunArgs, verbosity: u8) -> Result<()> {
    if !ctx.is_runtime_installed() {
        tracing::error!("The Spice runtime is not installed. Run 'spice install' first.");
