...
Once this directory is connected, `spice connect` runs it: `spiced` starts in the
foreground exactly as `spice run` would. That includes while the instance is
unattached, because its Cloud Connect stream is how a project created in the
portal reaches it. Naming --project (or --org) manages the connection instead.
