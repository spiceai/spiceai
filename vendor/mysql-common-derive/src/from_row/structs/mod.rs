...
impl ToTokens for GenericStruct<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let crat = match self.item_attrs.crate_name {
            Crate::NotFound => {
                tokens.extend(syn::Error::from(crate::Error::NoCrateNameFound).to_compile_error());
                return;
            }
            Crate::Multiple => {
                tokens
                    .extend(syn::Error::from(crate::Error::MultipleCratesFound).to_compile_error());
                return;
            }
        ...
        let take_field = match take_field {
            Ok(x) => x,
            Err(error) => {
                tokens.extend(syn::Error::from(error).to_compile_error());
                return;
            }
        };
