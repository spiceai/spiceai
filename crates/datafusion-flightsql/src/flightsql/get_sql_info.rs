...
pub(crate) fn do_get(
    ...
    Ok(Response::new(Box::pin(
        flight_data_stream.map_err(to_tonic_err),
    )))
}
...
pub(crate) static SQL_INFO_SUPPORTS_CONVERT: std::sync::LazyLock<HashMap<i32, Vec<i32>>> =
    std::sync::LazyLock::new(|| {
        ...
        for (from_sql, from_arrow) in SQL_DATA_TYPE_TO_ARROW_DATA_TYPE.clone() {
            ...
            for (to_sql, to_arrow) in SQL_DATA_TYPE_TO_ARROW_DATA_TYPE.clone() {
                if can_cast_types(&from_arrow, &to_arrow) {
                    to.push(to_sql as i32);
                }
            }
            if !to.is_empty() {
                convert.insert(from_sql as i32, to);
            }
        }
        convert
    });
...
static INSTANCE: std::sync::LazyLock<SqlInfoData> = std::sync::LazyLock::new(|| {
    ...
    builder.append(SqlInfo::FlightSqlServerName, "datafusion-flightsql");
