...
use runtime_component::dataset::acceleration::{RefreshMode, RefreshOnStartup};
...
impl Refresh {
    ...
    /// `runtime_datafusion::refresh_sql::parse_refresh_sql` before use
...
impl Refresher {
    ...
    /// # Errors
    ///
    /// Returns an error if the refresh task cannot be started — the refresh SQL
    /// does not plan against the dataset's schema, or the changes stream the mode
