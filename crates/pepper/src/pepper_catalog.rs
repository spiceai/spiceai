...
///
/// # Connection Management (Temporary until connection pools!)
///
/// The catalog maintains a single SQLite connection wrapped in a `Mutex` for safe
/// concurrent access. The connection is opened during `init()` and reused across
/// all operations. Database operations acquire the mutex lock, execute synchronously,
/// and release the lock. This prevents the overhead of opening new connections for
/// each operation while maintaining thread safety.
#[derive(Clone)]
pub struct PepperCatalog {
    connection_string: String,
    // Shared SQLite connection protected by a mutex
    // Connection is opened during init() and reused across operations
...
impl PepperCatalog {
    ...
    ///
    /// The SQLite connection is not opened until `init()` is called.
    ...
    /// Execute a database operation with the shared connection.
    ///
    /// This method ensures the connection is available and executes the operation
    /// synchronously while holding the mutex lock. The operation is NOT run in a
    /// blocking thread pool - instead, the caller must ensure this is called from
    /// an appropriate context (typically via spawn_blocking from the async methods).
    fn with_connection_sync<F, T>(&self, operation: F) -> CatalogResult<T>
    where
        F: FnOnce(&rusqlite::Connection) -> CatalogResult<T>,
    {
        // This is a synchronous function that will be called from spawn_blocking
        // We use try_lock with a timeout to avoid deadlocks
        let conn_guard = self.conn.blocking_lock();
        let conn = conn_guard
            .as_ref()
            .ok_or_else(|| CatalogError::InvalidOperation {
                message: "Catalog not initialized. Call init() first.".to_string(),
            })?;
...
impl MetadataCatalog for PepperCatalog {
    async fn init(&self) -> CatalogResult<()> {
        ...
        let conn = tokio::task::spawn_blocking(move || {
            ...
            Ok::<rusqlite::Connection, CatalogError>(conn)
        })
        .await??;
    ...
    async fn create_table(&self, options: CreateTableOptions) -> CatalogResult<i64> {
        ...
        tokio::task::spawn_blocking(move || {
            catalog.with_connection_sync(move |conn| {
                // Start transaction
            ...
            catalog.with_connection_sync(move |conn| {
                ...
                conn.execute(
                    ...
                    INSERT INTO pepper_data_file (
                        data_file_id, table_id, file_order,
                        path, path_is_relative, file_format,
                    ...
                    rusqlite::params![
                        file_id,
                        data_file.table_id,
                        data_file.file_order,
                        data_file.path,
                        data_file.path_is_relative,
                        data_file.file_format,
                        data_file.record_count,
                        data_file.file_size_bytes,
                        data_file.row_id_start,
                    ],
                )?;

                // Update next_file_id
                conn.execute(
                    "UPDATE pepper_metadata SET value = ?1 WHERE key = 'next_file_id'",
                    [next_file_id + 1],
                )?;
    ...
    async fn get_data_files(&self, table_id: i64) -> CatalogResult<Vec<DataFile>> {
        ...
        tokio::task::spawn_blocking(move || {
            catalog.with_connection_sync(move |conn| {
                let mut stmt = conn.prepare(
                    r"
                    SELECT data_file_id, table_id, file_order,
                           path, path_is_relative, file_format,
                           record_count, file_size_bytes, row_id_start
    ...
    async fn delete_data_file(&self, data_file_id: i64) -> CatalogResult<()> {
        let catalog = self.clone();
        tokio::task::spawn_blocking(move || {
            catalog.with_connection_sync(move |conn| {
                // Delete the data file entry from the catalog
                let rows_affected = conn.execute(
                    "DELETE FROM pepper_data_file WHERE data_file_id = ?1",
                    [data_file_id],
                )?;
    ...
    async fn delete_all_data_files(&self, table_id: i64) -> CatalogResult<()> {
        let catalog = self.clone();
        tokio::task::spawn_blocking(move || {
            catalog.with_connection_sync(move |conn| {
                // Delete all data file entries for this table
                conn.execute(
                    "DELETE FROM pepper_data_file WHERE table_id = ?1",
                    [table_id],
                )?;

                // Also delete associated delete files
                conn.execute(
                    "DELETE FROM pepper_delete_file WHERE table_id = ?1",
                    [table_id],
                )?;
    ...
    async fn delete_file_directory(&self, path: &str) -> CatalogResult<()> {
        let path_buf = std::path::PathBuf::from(path);

        if !path_buf.exists() {
            tracing::debug!("Directory {} does not exist, skipping deletion", path);
            return Ok(());
        }
