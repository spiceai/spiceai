...
pub trait MetadataCatalog: Send + Sync {
    ...
    /// Delete a data file from the catalog.
    ///
    /// Marks a virtual file as deleted in the catalog. This does NOT delete the
    /// physical directory - that should be done separately via `delete_file_directory`.
    async fn delete_data_file(&self, data_file_id: i64) -> CatalogResult<()>;

    /// Delete all data files for a table from the catalog.
    ///
    /// Marks all virtual files for a table as deleted. Physical directories must be
    /// deleted separately.
    async fn delete_all_data_files(&self, table_id: i64) -> CatalogResult<()>;

    /// Delete the physical directory for a virtual file.
    ///
    /// This physically removes the directory and all Vortex files within it.
