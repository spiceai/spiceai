...
use pepper::metastore::sqlite::SqliteMetastore;
#[cfg(feature = "turso")]
use pepper::metastore::turso::TursoMetastore;
use pepper::metastore::{
...
async fn get_sqlite_metastore() -> (SqliteMetastore, TempDir) {
...
fn bench_init_schema(c: &mut Criterion) {
    ...
    group.bench_function("sqlite", |b| {
        b.iter(|| {
            rt.block_on(async {
                let (metastore, _temp_dir) = get_sqlite_metastore().await;
...
fn bench_insert_single(c: &mut Criterion) {
    ...
    group.bench_function("sqlite", |b| {
        let setup = rt.block_on(async {
            let (metastore, temp_dir) = get_sqlite_metastore().await;
            metastore
            ...
            b.iter(|| {
                rt.block_on(async {
                    for i in 0..size {
                        ...
                        black_box(setup.0.execute(ExecuteParams {
