...
use data_components::mysql_replication::rows::{TransactionBuffer, build_change_batch};
...
/// Everything the deferred consumer path needs for one table: the value-only
/// decode schema plus a synthesized dataset shape for the Arrow build (types
/// inferred from decoded values; the capture's fixed-point/char columns land
/// as Utf8, which exercises the same `FieldBuilder` dispatch-per-cell path).
struct ArrowRoute {
    schema: FastRowSchema,
...
fn value_data_type(v: &Value) -> Option<DataType> {
    match v {
        Value::Int(_) => Some(DataType::Int64),
        Value::UInt(_) => Some(DataType::UInt64),
        Value::Float(_) | Value::Double(_) => Some(DataType::Float64),
        Value::Bytes(_) | Value::Date(..) | Value::Time(..) => Some(DataType::Utf8),
        _ => None, // NULL — keep scanning later rows
    }
}

/// Build per-table [`ArrowRoute`]s by scanning the slice: the first TableMap
/// per table supplies the decode schema; column Arrow types are inferred from
/// decoded rows (scanning until every column has seen a non-NULL value).
fn build_arrow_routes(slice: &[u8]) -> HashMap<(String, String), ArrowRoute> {
    let mut tmes: HashMap<u64, TableMapEvent<'static>> = HashMap::new();
    let mut schemas: HashMap<u64, FastRowSchema> = HashMap::new();
    let mut types: HashMap<(String, String), Vec<Option<DataType>>> = HashMap::new();
    for event in EventCursor::new(slice) {
        match event.read_data() {
            ...
            Ok(Some(EventData::RowsEvent(rows_data))) => {
                ...
                let slot = types
                    .entry(key)
                    .or_insert_with(|| vec![None; schema.cols.len()]);
                if slot.iter().all(Option::is_some) {
                    continue;
                }
                for image in decode_event_fast(&rows_data, schema) {
                    for (i, v) in image.iter().enumerate() {
                        if slot[i].is_none() {
                            slot[i] = v.as_ref().and_then(value_data_type);
                        }
                    }
                }
            }
    ...
    types
        ...
        .map(|(key, cols)| {
            ...
            let fields: Vec<Field> = cols
                .into_iter()
                .enumerate()
                .map(|(i, dt)| Field::new(format!("c{i}"), dt.unwrap_or(DataType::Utf8), true))
                .collect();
            let schema_by_id = tmes
                .iter()
                .find(|(_, tme)| tme.database_name() == key.0 && tme.table_name() == key.1)
                .map(|(id, _)| *id)
                .expect("table seen in scan");
            let route = ArrowRoute {
                schema: FastRowSchema::new(&tmes[&schema_by_id]),
                dataset_schema: Arc::new(Schema::new(fields)),
                column_map: (0..n).collect(),
...
/// Decode a whole rows event into the production [`TransactionBuffer`] with
/// the op classification `buffer_rows_event` applies — the bench-side twin of
/// `buffer_rows_event_fast`.
fn buffer_event_fast(
    rows_data: &RowsEventData<'_>,
    schema: &FastRowSchema,
    ...
    let op = match rows_data {
        ...
        RowsEventData::PartialUpdateRowsEvent(_) => return,
    };
    let to_values = |image: Vec<Option<Value>>| -> Vec<Value> {
        image
            .into_iter()
            .map(|v| v.expect("value converts"))
            .collect()
    };
    ...
    let mut buf = ParseBuf(rows_data.rows_data());
    while !buf.0.is_empty() {
        let before = before_cols.map(|included| schema.decode_image(&mut buf, included));
        let after = after_cols.map(|included| schema.decode_image(&mut buf, included));
        match op {
            Op::Insert => {
                buffer.push_insert(to_values(after.expect("write has after image")));
            }
            Op::Update => {
                buffer.push_update(
                    pk_source_indexes,
                    to_values(before.expect("update has before image")),
                    to_values(after.expect("update has after image")),
                );
            }
            Op::Delete => {
                buffer.push_delete(to_values(before.expect("delete has before image")));
            }
        }
    }
}
...
fn bench_stages(c: &mut Criterion, slice: &[u8]) {
    ...
    // Full consumer path in production shape: buffer OWNED payloads per
    // transaction (as the pump does), then at each commit fast-decode into the
    // production `TransactionBuffer` and build one Arrow `ChangeBatch` per
    // (table, txn) via the production `build_change_batch`. Delta vs
    // (`parse_decode_own` + the fast-decode stage) is the Arrow-build cost.
    let arrow_routes = build_arrow_routes(slice);
    group.bench_function("fast_decode_arrow", |b| {
        b.iter(|| {
            let mut routes: HashMap<u64, &ArrowRoute> = HashMap::new();
            let mut txn: HashMap<u64, Vec<RowsEventData<'static>>> = HashMap::new();
            for event in EventCursor::new(slice) {
                match event.read_data() {
                    Ok(Some(EventData::TableMapEvent(tme))) => {
                        ...
                        if let Some(route) = arrow_routes.get(&key) {
                            routes.insert(tme.table_id(), route);
                        }
                    }
                    Ok(Some(EventData::RowsEvent(rows_data))) => {
                        if matches!(rows_data, RowsEventData::PartialUpdateRowsEvent(_)) {
                        ...
                        if routes.contains_key(&rows_data.table_id()) {
                            txn.entry(rows_data.table_id())
                                .or_default()
                                .push(rows_data.into_owned());
                        }
                    ...
                    Ok(Some(EventData::XidEvent(_))) => {
                        for (table_id, events) in txn.drain() {
                            let route = routes[&table_id];
                            let mut buffer = TransactionBuffer::new();
                            for ev in &events {
                                buffer_event_fast(
                                    ev,
                                    &route.schema,
                                    &route.pk_source_indexes,
                                    &mut buffer,
                                );
                            }
                            ...
                            .expect("arrow change batch builds");
                            black_box(batch);
                        }
                    }
                    _ => {}
