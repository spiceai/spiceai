...
use crate::s3_single_file_cached::S3SingleFileCached;
...
pub async fn should_skip_refresh_for_table_provider(
    ...
    if let Some(provider) = table_provider.as_any().downcast_ref::<S3SingleFileCached>() {
        return RefreshSkipTableProvider::should_skip_refresh(provider)
            .await
            .map(Some);
    }
