            ...
            DataType::Utf8 => {
                ...
                match value {
                    Some(AttributeValue::L(list)) => {
                        for item in list {
                            match item {
                                AttributeValue::S(s) => values_builder.append_value(s),
                                AttributeValue::N(s) => values_builder.append_value(s),
...
mod tests {
    ...
    fn test_empty_items() {
        ...
        let result = dynamodb_items_to_arrow(&items, schema.clone()).expect("record_batch");
    ...
    fn test_single_item_all_types() {
        ...
        assert!(float_array.value(0) - 6.14 < f64::EPSILON);
    ...
    fn test_list_of_floats() {
        ...
        assert_eq!(float_values.value(0), 9.99);
        assert_eq!(float_values.value(1), 19.99);
        assert_eq!(float_values.value(2), 29.99);
    }
