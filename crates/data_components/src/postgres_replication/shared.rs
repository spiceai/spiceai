...
async fn run_pump(source: Arc<SharedSource>) {
    ...
    // Per-member hold-back queues, so one member's full channel cannot stop the
    // pump reading the connection (and thus stall every other member). Declared
    ...
    'reconnect: loop {
        ...
        // Held-back envelopes belong to the previous connection. The replay
        ...
        'recv: loop {
            ...
            match event {
                ...
                ReplicationEvent::KeepAlive {
                    ...
                    max_wal_end = max_wal_end.max(wal_end.0);
                    should_flush = true;
                    // Keepalives arrive even when no commit is routed, so this is
...
/// The commit boundary being routed: which LSN it ends at, when the source
/// committed it, and the shutdown epoch the pump started under.
#[derive(Clone, Copy)]
...
/// channel had spare capacity *or* the envelope was held back for a lagging
/// member, since neither blocks the pump.
async fn deliver_commit(
    ...
    for (relation_id, raw) in txn {
        ...
        // sent, so `credit_idle` (which only credits a member with
        // `delivered == committed`) can never advance this member's floor past
        // an envelope that is merely held back. Holding therefore pins WAL
        // exactly as an in-flight send does — which is the correct, safe side.
        slot.deliver(end_lsn);
        // Fast path only while this member has nothing held back; otherwise the
        // envelope must queue behind what is already waiting, or it would reach
        // the channel out of LSN order.
        if overflow.held_for(member_key) == 0 {
...
mod tests {
    ...
    #[tokio::test]
    async fn drain_overflow_to_blocks_until_under_target() {
        let epoch = crate::cdc::shutdown_epoch();
        let schema = tiny_schema();
        let (source, mut probes) = test_source_with_members(1);
        let (key, _, mut rx) = probes.remove(0);
        ...
        // Free room only after a delay, so the drain has to wait for it.
        let drainer = tokio::spawn(async move {
