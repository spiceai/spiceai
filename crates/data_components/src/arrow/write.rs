...
impl MemTable {
    ...
    async fn ensure_batches_satisfy_constraints(&self, constraints: &Constraints) -> Result<()> {
        ...
        let mut constraint_keys: Vec<HashSet<String, std::hash::BuildHasherDefault<twox_hash::XxHash3_64>>> =
            ...
            for (i, c) in constraints.iter().enumerate() {
                let valid_ids = match c {
                    Constraint::PrimaryKey(pk) => {
                        ...
                        check_and_filter_non_null_unique_primary_keys::<std::hash::BuildHasherDefault<twox_hash::XxHash3_64>>(
                            &pks,
                    ...
                    Constraint::Unique(u) => {
                        ...
                        check_and_filter_unique_constraint::<std::hash::BuildHasherDefault<twox_hash::XxHash3_64>>(
                            &as_str,
                            constraint_keys.get(i),
                        )?
                    }
...
impl DataSink for MemSink {
    ...
    async fn write_all(
        ...
        let mut new_key_set: HashSet<String, std::hash::BuildHasherDefault<twox_hash::XxHash3_64>> = HashSet::default();
        if let Some(ref pks) = self.primary_key {
            ...
            new_key_set = check_and_filter_non_null_unique_primary_keys::<std::hash::BuildHasherDefault<twox_hash::XxHash3_64>>(
                &new_primary_key_ids,
                None,
            )?;
        }
