...
pub struct CachedQueryVector {
    inner: Arc<dyn ComputeQueryVector + Send + Sync>,
    cached: Arc<OnceCell<Result<Vec<f32>, Arc<Box<dyn std::error::Error + Send + Sync>>>>>,
}

impl CachedQueryVector {
    pub fn new(inner: impl ComputeQueryVector + Send + Sync + 'static, query: String) -> Self {
        Self {
            inner: Arc::new(inner),
...
impl ComputeQueryVector for CachedQueryVector {
    async fn compute_vector(
        ...
        if query != self.query.as_ref() {
            return Err(Box::from(
            "CachedQueryVector called with different query than it was constructed with".to_string()));
        }
        self
            .cached
            .get_or_init(|| async {
                self.inner
                    .compute_vector(&self.query)
                    .await
                    .map_err(Arc::new)
            })
            .await.boxed().cloned()
