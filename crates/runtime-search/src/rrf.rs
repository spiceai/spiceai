...
mod tests {
    ...
    fn df_as_embedding_table(
        embedding_models: Arc<tokio::sync::RwLock<crate::embeddings::EmbeddingModelStore>>,
        df: DataFrame,
    ...
    async fn make_test_session() -> Result<(
        Arc<SessionContext>,
        Arc<tokio::sync::RwLock<crate::embeddings::EmbeddingModelStore>>,
    )> {
        ...
        // Register RRF
        ctx.register_udf(super::ReciprocalRankFusion::from_ctx(&ctx).into());
        ctx.register_udtf(
            super::RRF_UDF_NAME,
            Arc::new(super::ReciprocalRankFusion::from_ctx(&ctx)),
        );

        Ok((ctx, embedding_models))
    }

    #[cfg(feature = "models")]
    async fn make_fruit_dataframe(ctx: &SessionContext) -> Result<DataFrame> {
        let df = ctx
            .sql(
                "SELECT
              unnest([
    ...
    async fn test_recency_scoring() -> Result<ExitCode> {
        let (ctx, embedding_models) = make_test_session().await?;

        let fruit_df = make_fruit_dataframe(&ctx)
            .await?
    ...
    async fn test_recency_unboosting_disjoint() -> Result<ExitCode> {
        ...
        ctx.register_table("left_fruit", left_fruit)?;

        ctx.register_table("right_fruit", right_fruit)?;
    ...
    async fn test_fuse_with_case_sensitive_columns() -> Result<ExitCode> {
        ...
        let fruit_df = make_fruit_dataframe(&ctx).await?.select(vec![
            col("id").alias("Id"),
            col("content"),
    ...
    async fn test_score_computation() -> Result<ExitCode> {
        ...
        ctx.register_table("foo", fruit_table)?;
