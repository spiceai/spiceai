...
use runtime_proto::{
    RrfArgs as SerializableRrfArgs, RrfNestedQuery, RrfTextSearchQuery, RrfVectorSearchQuery,
    TextSearchArgs, VectorSearchArgs, rrf_nested_query::Query,
};
...
mod tests {
    ...
    fn named_arg(name: &str, value: impl Into<ScalarValue>) -> Expr {
        let meta = FieldMetadata::new(BTreeMap::from([(
            "spice.parameter_name".to_string(),
            name.to_string(),
        )]));
        Expr::Literal(value.into(), Some(meta))
    }

    fn vector_search_expr(table: &str, query: &str) -> Expr {
        ...
        Expr::ScalarFunction(ScalarFunction::new_udf(
            Arc::new(stub_udf),
            vec![col_expr(table), lit_str(query)],
        ))
    }

    fn text_search_expr(table: &str, query: &str) -> Expr {
        let stub_udf = create_udf(
            "text_search",
            vec![],
            DataType::Utf8,
    ...
    fn test_parse_argument_exprs() {
        ...
        let result =
            ReciprocalRankFusionArgs::from_udtf_exprs(&[vector_search_expr("foo", "query")]);
        assert!(result.is_err(), "Expected error for single search query");

        // Two search queries - should succeed
        let result = ReciprocalRankFusionArgs::from_udtf_exprs(&[
            vector_search_expr("foo", "query1"),
            vector_search_expr("foo", "query2"),
        ]);
        assert!(result.is_ok(), "Expected success for two search queries");

        // Two queries with named args - should succeed
        let result = ReciprocalRankFusionArgs::from_udtf_exprs(&[
            vector_search_expr("foo", "query1"),
            vector_search_expr("foo", "query2"),
    ...
    #[test]
    fn test_parse_rank_weight() {
        // rank_weight is a sub-query arg, not a top-level named arg
        let stub_udf = create_udf(
            "vector_search",
            vec![],
            DataType::Utf8,
        ...
        let vs_with_weight = Expr::ScalarFunction(ScalarFunction::new_udf(
            Arc::new(stub_udf),
            vec![
                col_expr("foo"),
                lit_str("query"),
                spice_named_lit!("rank_weight", 2.5f64),
            ],
        ));
