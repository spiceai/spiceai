...
//! Config-only **component specifications**.
//!
//! This crate holds the pure-configuration cores of Spice's runtime components —
//! `DatasetSpec`, `CatalogSpec`, `ViewSpec` and their supporting config types
//! (acceleration, columns, access mode, declared types, replication, …). It names
//! nothing from the `runtime` crate: no `Arc<Runtime>`, no `App`, no orchestrator
//! internals. `runtime` composes these specs into its `Arc<Runtime>`-bound
//! wrappers (`runtime::component::{dataset::Dataset, catalog::Catalog, view::View}`),
//! which `Deref` to the specs here.
//!
//! Sitting below `runtime` in the crate graph, this crate lets connectors (and
//! anything else that only needs a component's declared configuration) name that
...
/// # Errors
///
/// Returns [`Error::InvalidIdentifier`] if `identifier` is not a valid SQL identifier.
...
#[must_use]
pub fn find_first_delimiter(from: &str) -> Option<(usize, usize)> {
...
///
/// [`ComponentInitialization::OnStartup`] indicates that the component should be
/// initialized when runtime started.
/// [`ComponentInitialization::OnTrigger`] indicates that the component should be
/// initialized when a specific trigger event occurs.
