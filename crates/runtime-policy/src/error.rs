...
#[derive(Debug, Snafu)]
pub enum Error {
    #[snafu(display("Failed to parse Cedar policy: {reason}"))]
    PolicyParse { reason: String },

    #[snafu(display("Failed to build Cedar schema: {reason}"))]
    SchemaBuild { reason: String },
    ...
    #[snafu(display("Policy provider error: {reason}"))]
