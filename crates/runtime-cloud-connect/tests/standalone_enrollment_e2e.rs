...
impl axum::serve::Listener for TlsListener {
    ...
    async fn accept(&mut self) -> (Self::Io, Self::Addr) {
        loop {
            let (stream, address) = self.tcp.accept().await.expect("accept cloud connection");
...
/// Serve the cloud mock over HTTPS on an ephemeral port; returns its address.
async fn spawn_cloud_mock(mock: CloudMock, ca: &TestCa) -> SocketAddr {
    let app = Router::new()
    ...
    let tls = rustls::ServerConfig::builder()
        ...
        .expect("build HTTPS cloud mock configuration");
...
/// A fully-wired split control plane: the cloud mock (HTTPS) and the
...
impl Harness {
    ...
    async fn new(leaf_validity_secs: i64) -> Self {
        ...
        let cloud_addr = spawn_cloud_mock(cloud.clone(), &ca).await;
        Self {
            ca,
            cloud,
            cloud_addr,
    ...
    fn config(
        ...
        CloudConnectConfig {
            enroll_endpoint: format!("https://{}", self.cloud_addr),
...
/// Existing identity precedence applies only to an identity the runtime can
/// actually use. In particular, a stored endpoint must not let bootstrap
/// discard the supplied one-shot key and then fail later during startup.
#[tokio::test]
async fn an_existing_identity_with_an_unsafe_control_plane_endpoint_fails_closed() {
    let harness = Harness::new(24 * 60 * 60).await;
    let dir = tempfile::tempdir().unwrap();
    let config = harness.config(
        dir.path().join("identity.json"),
    ...
    let mut identity = IdentityStore::load_optional(&config.identity_path)
        .expect("load enrolled identity")
        .expect("enrollment persisted an identity");
    identity.control_plane_endpoint = Some("http://control.example".to_string());
    IdentityStore::store(&config.identity_path, &identity).expect("store edited identity");

    let error = enroll_now(
        &config,
        &token_authority(SECOND_ENROLLMENT_KEY),
        test_retry(),
    )
    .await
    .expect_err("bootstrap must reject the unusable stored endpoint");

    assert!(
        matches!(
            error,
            runtime_cloud_connect::enroll::EnrollNowError::IdentityUnusable { .. }
        ),
        "{error}"
    );
    assert_eq!(
        harness.cloud.enroll_requests.lock().await.len(),
        requests_after_first,
        "rejecting the stored identity must not redeem the supplied key"
    );
}
