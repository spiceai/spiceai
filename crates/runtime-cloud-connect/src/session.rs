...
pub struct SessionAck {
    ...
    notify: Notify,
    withdrawn: tokio_util::sync::CancellationToken,
}
...
impl SessionAck {
    ...
    /// Permanently withdraw this process's Cloud session.
    ///
    /// A dispatched `Remove` deliberately leaves the runtime serving locally,
    /// so process/server shutdown cannot represent this transition. Consumers
    /// waiting to announce a connection use this independent signal to avoid
    /// reporting a session after its identity has been released.
    pub(crate) fn withdraw(&self) {
        self.withdrawn.cancel();
    }

    /// A waiter-owned view of permanent session withdrawal.
    #[must_use]
    pub fn withdrawal(&self) -> tokio_util::sync::CancellationToken {
...
mod tests {
    ...
    async fn a_report_names_an_attachment_that_landed_after_the_acknowledgement() {
        ...
        IdentityStore::set_attachment(
            path.parent().expect("identity has a parent directory"),
            &path,
    ...
    #[test]
    fn permanent_session_withdrawal_wakes_every_observer() {
        let ack = SessionAck::new();
        let observer = ack.withdrawal();
        assert!(!observer.is_cancelled());
        ack.withdraw();
        assert!(observer.is_cancelled());
        assert!(ack.withdrawal().is_cancelled());
    }
}
