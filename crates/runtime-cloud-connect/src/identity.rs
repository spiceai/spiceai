...
pub struct Identity {
    ...
    ///
    /// `None` means no deploy has ever named an app for this instance — either
    /// it is attached to none, or every deploy it has seen predates the field.
    /// Defaulted so identity files written before this field existed still load.
    #[serde(default)]
