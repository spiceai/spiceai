        ...
        CAYENNE_AT_FRESHNESS_SECS
            .get_or_init(|| {
                cayenne_operational_meter()
                    ...
                    .with_description("Measured freshness — windowed-peak per-apply row freshness (worst PG-commit→queryable lag) over the rolling goal-convergence window. Peak, not instantaneous: captures transient stalls and does not ramp on an idle table. Falls back to the instantaneous now−last_apply age on sources without a commit timestamp, or before the first timestamped apply.")
                    .with_unit("s")
                    .build()
            })
