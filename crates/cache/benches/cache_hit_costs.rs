...
/// Legacy SQL serve: `Arc<Vec<RecordBatch>>`, `RecordBatch::clone` on each
/// poll. Bench-local so construct+drain and construct+drain+touch isolate
/// the new SQL path (`QueryResult::from_cached_raw` → `CachedRawStream`,
/// prefetch + one `Arc` clone per poll).
...
fn bench_encoded_stream_serve(c: &mut Criterion) {
    ...
    for (batches, rows, text_columns) in cases {
        ...
        group.bench_function(BenchmarkId::new("encoded_records_serve", &id), {
            let bytes = Bytes::from(zstd_payload.bytes.clone());
            let decoded_len = zstd_payload.decoded_len;
            ...
            move |b| {
                let runtime = tokio::runtime::Builder::new_current_thread()
                    .build()
                    .expect("a tokio runtime");
                // A fresh entry per iteration: `records()` memoizes its decode
                // in the entry's `OnceCell`, so reusing one entry would time an
                // `Arc` clone instead of the decode-and-serve this names.
                b.iter_batched(
                    || {
                        let now = Instant::now();
