...
//! so the following poll's `RecordBatch` clone sees warm `ArrayRef` targets.
...
/// first few `dyn Array` allocations — the lines `RecordBatch::clone` and a
/// later poll touch before any cell values.
