...
use connector_traits::{
    ...
    DataConnector, DataConnectorError, DataConnectorResult, ExposedParamLookup, Parameters,
};
use data_components::object::{metadata::ObjectStoreMetadataTable, text::ObjectStoreTextTable};

use crate::infer::{infer_partitions_with_types_from_files, infer_partitions_with_types_prefix};
use crate::{DelimitedFormat, get_df_default_config};
use connector_traits::DataConnectorError::SchemaMismatch;
...
#[derive(Debug)]
pub struct ListingConnector<T> {
    inner: T,
}

impl<T> ListingConnector<T> {
    #[must_use]
...
impl<T: Display> Display for ListingConnector<T> {
...
#[async_trait]
impl<T: ListingTableConnector + Display + Send + Sync> ListingTableConnector
    for ListingConnector<T>
{
    fn object_versioning_type(&self) -> Option<ObjectVersionType> {
        self.inner.object_versioning_type()
    }

    fn supports_single_file_etag_cache(&self) -> bool {
        self.inner.supports_single_file_etag_cache()
    }

    fn as_any(&self) -> &dyn Any {
        ListingTableConnector::as_any(&self.inner)
    }
    ...
    fn get_object_store_url(
        ...
        self.inner.get_object_store_url(dataset, url)
    }
    ...
    fn get_tokio_io_runtime(&self) -> tokio::runtime::Handle {
        self.inner.get_tokio_io_runtime()
    }
    ...
    async fn on_accelerated_table_registration(
        ...
        ListingTableConnector::on_accelerated_table_registration(
            &self.inner,
            dataset,
            accelerated_table,
        )
...
#[async_trait]
pub trait ListingTableConnector {
    fn object_versioning_type(&self) -> Option<ObjectVersionType> {
        None
    }
    ...
    /// # Errors
    ///
    /// Returns an error if the URL cannot be constructed or parsed.
    ...
    /// # Errors
    ///
    /// Returns an error if the object store cannot be resolved.
    #[expect(clippy::result_large_err)]
    fn get_object_store(
        &self,
        dataset: &dyn ConnectorDataset,
        ...
        Self: Display,
    {
        ...
        let listing_store_url = ListingTableUrl::parse(store_url).boxed().context(
            connector_traits::UnableToConnectInternalSnafu {
        ...
        self.get_session_context()
            ...
            .boxed()
            .context(connector_traits::UnableToConnectInternalSnafu {
                dataconnector: format!("{self}"),
                connector_component: ConnectorComponent::from(dataset),
            })
    }
        ...
        Self: Display,
    {
        ...
        let table = ObjectStoreMetadataTable::try_new(store, &store_url, Some(extension.clone()))
            .context(connector_traits::InvalidConfigurationSnafu {
            dataconnector: format!("{self}"),
            message: format!(
    ...
    /// # Errors
    ///
    /// Returns an error if any JSONL parameter value is invalid.
    #[expect(clippy::result_large_err)]
    fn get_jsonl_format(
        &self,
        dataset: &dyn ConnectorDataset,
        ...
        Self: Display,
    {
        ...
        {
            let schema_infer_max_rec = usize::from_str(infer_max_rec_str)
                .boxed()
                .context(connector_traits::InvalidConfigurationSnafu {
                    dataconnector: format!("{self}"),
                    message: format!(
    ...
    /// # Errors
    ///
    /// Returns an error if any JSON parameter value is invalid.
    #[expect(clippy::result_large_err)]
    fn get_json_format(
        &self,
        dataset: &dyn ConnectorDataset,
        ...
        Self: Display,
    {
        ...
        if let ExposedParamLookup::Present(json_format_str) = params.get("json_format").expose() {
            let json_format = json_format_str
                .parse::<Format>()
                .boxed()
                .context(connector_traits::InvalidConfigurationSnafu {
                    dataconnector: format!("{self}"),
    ...
    /// # Errors
    ///
    /// Returns an error if any delimiter-separated parameter is invalid.
    #[expect(clippy::result_large_err)]
                ...
                .with_file_compression_type(
                    FileCompressionType::from_str(compression_type)
                        ...
                        .context(connector_traits::InvalidConfigurationSnafu {
                            dataconnector: format!("{self}"),
                            message: format!(
                                "Invalid {} compression_type: {compression_type}, supported types are: GZIP, BZIP2, XZ, ZSTD, UNCOMPRESSED",
                                delimiter.to_string().to_uppercase()
                            ),
                            connector_component: ConnectorComponent::from(dataset),
                        })?,
                ),
        ...
        Self: Display,
    {
        ...
        let table_path = ListingTableUrl::parse(url.clone()).boxed().context(
            connector_traits::InternalSnafu {
                dataconnector: format!("{self}"),
                connector_component: ConnectorComponent::from(dataset),
        ...
        let (schema_infer_url, schema_infer_meta) =
            if let Some(url) = dataset.params().get("schema_source_path") {
                ...
                let schema_infer_url = ListingTableUrl::parse(&url).boxed().context(
                    connector_traits::UnableToGetSchemaInternalSnafu {
                        dataconnector: format!("{self}"),
                        connector_component: ConnectorComponent::from(dataset),
                    },
                )?;
        ...
        let table =
            ListingTable::try_new(config)
                .boxed()
                .context(connector_traits::InternalSnafu {
                    dataconnector: format!("{self}"),
                    connector_component: ConnectorComponent::from(dataset),
        ...
        if self.supports_single_file_etag_cache()
    ...
    /// # Errors
    ///
    /// Returns an error if partition columns cannot be deduplicated safely.
    #[expect(clippy::result_large_err)]
...
#[async_trait]
impl<T: ListingTableConnector + Display + Send + Sync + std::fmt::Debug + 'static> DataConnector
    for ListingConnector<T>
{
    fn as_any(&self) -> &dyn Any {
        ListingTableConnector::as_any(&self.inner)
...
fn get_url_prefix(table_url: &Url) -> Arc<str> {
    Arc::<str>::from(format!("{}/", &table_url[..url::Position::BeforePath]))
}
...
/// # Errors
///
/// Returns an error if the listing table URL cannot be parsed.
#[expect(clippy::result_large_err)]
