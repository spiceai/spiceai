...
pub(crate) fn get_user_agent() -> String {
    ...
    format!(
        "repl/{} ({os_type}/{os_release} {os_arch})",
...
mod test {
    ...
    fn test_get_user_agent() {
        let matching_regex = regex::Regex::new(
            r"^repl/\d+\.\d+\.\d+(-beta|-stable|-rc|-unstable|-enterprise|-enterprise-beta|-enterprise-unstable)?(\.\d+)?(-enterprise|-enterprise-beta|-enterprise-unstable)? \((Linux|Windows|Darwin)/[\d\w\.\-\_]+ (x86_64|aarch64|i386)\)$",
        )
