...
/// Returns `true` if the input string contains at least one secret reference
/// pattern (e.g. `${store:key}`).
#[must_use]
pub fn contains_secret_ref(input: &str) -> bool {
    SecretReplacementMatcher::new(input).next().is_some()
}
...
mod tests {
    ...
    #[test]
    fn test_contains_secret_ref() {
        assert!(contains_secret_ref("${env:MY_KEY}"));
        assert!(contains_secret_ref("prefix_${secret:key}_suffix"));
        assert!(!contains_secret_ref("no secrets here"));
        assert!(!contains_secret_ref("${invalid}"));
        assert!(!contains_secret_ref(""));
    }
}
