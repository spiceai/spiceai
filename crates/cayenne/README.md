...
- **`mode: memory`** (fully in-RAM, ephemeral): all data lives in the RAM mem-tier, backed by an in-memory `memdb` `SQLite` metastore — **nothing is written to disk**. Checkpointing, sealing, compaction, and the cold tier are disabled, and the source replication slot is never advanced. The dataset is ephemeral and reloads from its source on restart (like the in-memory Arrow accelerator); it works for all refresh modes (`full`/`append`/`changes`) and for both keyed and no-PK datasets, and a full refresh atomically replaces the in-RAM tier. Since nothing spills to disk, a per-table hard RAM bound (`cayenne_cdc_mem_tier_max_bytes`; default unbounded) returns a structured error on breach rather than growing without limit.
