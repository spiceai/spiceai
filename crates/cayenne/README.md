...
| Parameter                                     | Description                                                                                                                                      | Default          |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------- |
| `cayenne_metadata_dir`                        | Directory holding the metastore (`cayenne.db`). One metastore serves every Cayenne-accelerated dataset in the instance, so this is runtime-global; `acceleration.params.cayenne_metadata_dir` is ignored, with a warning naming the directory the dataset's rows actually go to. | `{cayenne_file_path}/metadata` when that path is local, else `{spice_data}/metadata` |
