...
/// Decode the marked keys as `(id, op)` pairs, sorted by id.
async fn marked_keys(table: &CayenneTableProvider) -> Vec<(i64, WriteBackOp)> {
    let markers = table
    ...
    let pk_bytes: Vec<Vec<u8>> = markers.iter().map(|m| m.pk_bytes.clone()).collect();
    let decoded = table
        .decode_pk_keys(&pk_bytes)
        .expect("marker keys decode back to the primary-key columns");
    let ids = decoded
        .first()
        .expect("a single-column primary key decodes to one array")
        .as_any()
        .downcast_ref::<Int64Array>()
        .expect("the decoded key column is Int64")
        .values()
        .to_vec();

    let mut pairs: Vec<(i64, WriteBackOp)> = ids
        .into_iter()
        .zip(markers.iter().map(|m| m.op))
        .collect();
...
async fn a_user_delete_marks_its_keys_for_write_back(
    ...
    let before = marked_keys(&table).await;
    assert!(
    ...
    let marked = marked_keys(&table).await;
...
async fn a_delete_on_a_table_without_write_back_marks_nothing(
    ...
    assert!(
        marked_keys(&table).await.is_empty(),
