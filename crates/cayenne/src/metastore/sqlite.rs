    ...
    async fn init_schema(&self) -> CatalogResult<()> {
        ...
        guard
            .call(|conn| {
                ...
                // column only upsert commits marked keys. Forward-only: a binary
                // predating the column rejects the migrated table outright,
                // because `validate_existing_schema` compares the full ordered
                // column list and fails on an extra one. Appended last to match
                // CREATE TABLE and EXPECTED_TABLES.
                let _ = conn.execute(
