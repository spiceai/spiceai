...
pub struct CayenneAccelerationExec {
    ...
    /// Dataset label carried into the query-pool reservation name.
...
impl CayenneAccelerationExec {
    ...
    /// Records the dataset this scan reads for actionable memory-pool errors.
    #[must_use]
    ...
    fn execute(
            ...
            && let Some(fan_out) = self.decode_fan_out(partition)
        {
            let accounted = MemoryAccountedScanStream::new(
                ...
                schema,
                self.table_name.as_deref().map_or_else(
                    || format!("cayenne_scan[partition={partition}]"),
                    |table| format!("cayenne_scan[{table}, partition={partition}]"),
                ),
                &context,
                fan_out,
            );
