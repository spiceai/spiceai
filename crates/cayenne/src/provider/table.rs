...
impl CayenneTableProvider {
    ...
    ///
    /// File size management is handled internally by the Vortex `ListingTable` format writer.
    ...
    /// - Only one insert runs at a time per table
    /// - Writes complete before the listing table is refreshed
    ...
    pub(crate) async fn insert(&self, stream: SendableRecordBatchStream) -> CatalogResult<u64> {
        ...
        // This ensures listing table refresh happens after writes complete
        ...
        // Write the stream directly to the listing table.
        // File splitting is handled internally by the Vortex format writer.
        let total_rows = self.write_stream(prepared_stream).await?;

        tracing::debug!("Insert completed, wrote {} rows to Vortex", total_rows,);
        ...
        // 1. Writes complete before retention filters are evaluated
        ...
        // that must scan all data. Applying them atomically after the write completes ensures
        // consistency without write amplification.
        ...
        if self.context.has_sort_columns() {
            self.sort_and_rewrite_data().await?;
        }
        ...
        // happens after writes are complete and no other insert can interfere.
    ...
    async fn insert_to_new_snapshot_with_sequence(
        ...
        // File splitting is handled internally by the Vortex format writer.
        let total_rows = self
            .write_stream_to_snapshot(stream, &new_snapshot_id)
            .await?;

        tracing::debug!(
            "Insert to new snapshot {} completed, wrote {} rows to Vortex",
            new_snapshot_id,
            total_rows,
        );
    ...
    /// Write a stream of record batches to the current listing table.
    ///
    /// This method passes the stream directly to the underlying Vortex `ListingTable`'s
    /// `insert_into`, which handles file size management internally. File splitting is
    /// delegated entirely to the Vortex format writer.
    ///
    /// # Returns
    ///
    ...
    async fn write_stream(&self, stream: SendableRecordBatchStream) -> CatalogResult<u64> {
        ...
        let listing_table = {
            let guard = self
                ...
                .read()
                .map_err(|_| CatalogError::LockPoisoned {
                    operation: "write_stream (read listing table)".to_string(),
                })?;
            Arc::clone(&guard)
        };
        ...
        let insert_plan = listing_table
            ...
            .await
            .map_err(|e| CatalogError::InvalidOperation {
                message: "Failed to create insert plan.".to_string(),
                source: Box::new(e),
            })?;

        // Execute the insert plan and count rows from the result batches
        let result_batches = collect(insert_plan, state.task_ctx()).await.map_err(|e| {
            CatalogError::InvalidOperation {
                message: "Failed to execute insert.".to_string(),
                source: Box::new(e),
            }
        })?;

        // The insert plan returns a single batch with a "count" column containing the row count
        ...
        tracing::debug!("Wrote {} rows to Vortex", total_rows);
    ...
    ///
    /// This is similar to `write_stream` but writes to a specified snapshot
    ...
    ///
    /// The stream is passed directly to the underlying Vortex `ListingTable`'s
    /// `insert_into`, which handles file size management internally.
    ///
    /// # Arguments
    ///
    ...
    pub(crate) async fn write_stream_to_snapshot(
        &self,
        stream: SendableRecordBatchStream,
        snapshot_id: &str,
    ) -> CatalogResult<u64> {
        ...
        let insert_plan = snapshot_listing_table
            ...
            .await
            .map_err(|e| CatalogError::InvalidOperation {
                message: format!("Failed to create insert plan for snapshot {snapshot_id}."),
                source: Box::new(e),
            })?;
        ...
        let result_batches = collect(insert_plan, state.task_ctx()).await.map_err(|e| {
            CatalogError::InvalidOperation {
                message: format!("Failed to execute insert for snapshot {snapshot_id}."),
                source: Box::new(e),
            }
        })?;
    ...
    /// Create a clone of necessary fields for write tasks.
    ///
    ...
    /// the write lock and applies retention atomically after writes complete.
    ///
    /// This design provides ACID semantics:
    /// - Retention filters are table-wide predicates (e.g., "delete rows older than 30 days")
    /// - They must scan all table data, not just the newly written data
    /// - The write lock ensures atomicity: writes + retention happen as one operation
    ...
    /// 4. Writes the sorted data back (Vortex handles file splitting internally)
    ///
    ...
    async fn sort_and_rewrite_data(&self) -> CatalogResult<()> {
        tracing::info!(
        ...
        // File splitting is handled internally by the Vortex format writer.
        let total_rows = self.write_stream(sorted_stream).await?;

        tracing::info!(
            "Rewrote {} sorted rows for table {}",
            total_rows,
            self.table_metadata.table_name
        );
...
impl TableProvider for CayenneTableProvider {
    ...
    async fn insert_into(
        ...
        // - Writing via write_stream_to_snapshot (Vortex handles file splitting)
        ...
        // Use CayenneDataSink with DataSinkExec for writes:
        // - Streaming writes via write_stream (Vortex handles file splitting)
