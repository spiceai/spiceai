...
impl CayenneTableProvider {
    ...
    pub async fn insert(&self, stream: SendableRecordBatchStream) -> CatalogResult<u64> {
        ...
        self.apply_on_conflict_deletions(delete_specs, deleted_pk_i64, deleted_row_keys).await?;
    ...
    async fn load_existing_keyset(
        ...
        for batch in batches {
            ...
            let int64_pk_array: Option<&Int64Array> =
                if self.pk_deletion_strategy == PkDeletionStrategy::Int64Pk && pk_indices.len() == 1
                {
...
impl TableProvider for CayenneTableProvider {
    ...
    async fn insert_into(
        ...
        let final_input = if let Some(pk_indices) = self.primary_key_indices().map_err(|e| {
            ...
            let mut existing_keys = self.load_existing_keyset(&pk_indices, &converter).await.map_err(|e| {
                datafusion_common::DataFusionError::Execution(format!(
            ...
            eprintln!("DEBUG: Validated batches: {}, delete_specs: {:?}", validation_result.filtered_batches.len(), validation_result.delete_specs);
            ...
            if has_on_conflict_deletions {
                eprintln!("DEBUG: On-conflict deletions applied, writing to new protected snapshot");
                ...
                let batch_stream = futures::stream::iter(validation_result.filtered_batches.into_iter().map(Ok));
                ...
                eprintln!("DEBUG: Wrote {} rows to new snapshot with seq={}", rows_written, insert_sequence);
            ...
            Arc::new(StreamingExec::new(schema, Box::pin(validated_stream))) as Arc<dyn ExecutionPlan>
        } else {
        ...
        let result = listing_table
            .insert_into(state, final_input, InsertOp::Append)
            .await?;
