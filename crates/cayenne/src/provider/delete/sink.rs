...
//!   Optimized path that extracts PK values directly without `RowConverter`
//!   overhead (with a no-scan fast path when the filter is a recognizable PK
//!   `IN`-list, see [`pk_filter_extract`]). Each PK is recorded with its
//!   `delete_sequence` and published into the table's
//!   `DeletionIndex` snapshot via `extend_max_deletes`.
//!
//! - **Key-based**: For tables with composite or non-integer primary keys.
//!   Uses Arrow's `RowConverter` to create deterministic byte keys, recorded
//!   with their `delete_sequence` and published into the `KeyDeletionIndex`
//!   snapshot.
//!
//! # Workflow
//!
