...
/// Level chosen by `auto` for every delta write. `Sparse + Dict` (with the
/// cascade's built-in constant detection as of Vortex 0.79) keeps the big
/// repetitive-CDC wins (often 3-5×) while skipping the per-file `BtrBlocks`
/// strategy search and FSST symbol-table training that dominate encode cost.
/// Every delta is transient — compaction re-encodes it at [`FULL_LEVEL`] — so
/// the light scheme set trades a larger transient file (compaction folds it)
/// for materially cheaper CDC-hot-path encode; the full cascade is reserved
/// for the durable [`WriteClass::Maintenance`] artifacts whose encoding
/// quality pays for scan throughput.
