...
impl CayenneDataSink {
    ...
    async fn write_all_append(
        ...
        let mut on_conflict_deletions_applied = false;
        ...
        if can_inline {
            ...
            if !exceeded && total_rows > 0 {
                self.table
                    .apply_on_conflict_deletions(
                        delete_specs.clone(),
                        deleted_pk_i64.clone(),
                        deleted_row_keys.clone(),
                        deleted_inlined_pk_i64.clone(),
                        deleted_inlined_row_keys.clone(),
                    )
                    .await?;
                on_conflict_deletions_applied = true;
            }
            ...
            if !on_conflict_deletions_applied {
                self.table
                    .apply_on_conflict_deletions(
                        delete_specs,
                        deleted_pk_i64,
