...
//! Wall-clock benchmark for the write-side metadata publish path.
//!
//! Historically each CDC checkpoint publish called `commit_on_conflict_deletions`
//! and wrote one `cayenne_insert_record` row per upserted key through the single
//! `BEGIN IMMEDIATE` SQLite writer. Metadata-only publish now writes delete-file
//! metadata with a per-file `reinsert_sequence` instead. This bench keeps the
//! original shapes but measures only the catalog metadata commit; it uses
//! synthetic delete-file rows and does not perform deletion-vector file I/O.
//!
//! - `per_commit/<keys>` — cost of ONE metadata-only publish representing N keys
//!   against a FRESH (empty) catalog. The per-publish floor, isolated from
//!   catalog growth.
//! - `grown_catalog/<preloaded>` — cost of one 10K-key publish against a
//!   catalog already holding metadata-only publish rows representing `preloaded`
//!   keys. This is intended to stay flat now that the durable per-key rows are
//!   gone.
//!
...
use cayenne::metadata::{CreateTableOptions, DeleteFile, DeletionType};
...
fn synthetic_delete_file(
    ...
    DeleteFile {
        ...
        path: format!("metadata_only_delete_{path_suffix}.arrow"),
...
/// Commit one metadata-only publish with synthetic delete-file metadata at a
...
async fn commit_keys(catalog: &Arc<dyn MetadataCatalog>, table_id: &str, start: i64, keys: usize) {
    ...
    let delete_file = synthetic_delete_file(table_id, &format!("preload_{start}"), keys, seq);
    catalog
            ...
            |bencher, &keys| {
                bencher.to_async(&rt).iter_custom(|iters| async move {
                    ...
                    for _ in 0..iters {
                        ...
                        let delete_file = synthetic_delete_file(&table_id, "per_commit", keys, seq);
            ...
            |bencher, &preload| {
                bencher.to_async(&rt).iter_custom(|iters| async move {
                    ...
                    for _ in 0..iters {
                        ...
                        let delete_file = synthetic_delete_file(
                            &table_id,
                            &format!("grown_{preload}"),
                            GROWN_COMMIT_KEYS,
                            seq,
                        );
                        let snapshot_id = uuid::Uuid::now_v7().to_string();
