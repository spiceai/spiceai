...
pub struct Running {
    schedules: Vec<Arc<Schedule>>,
    request_handles: Arc<RwLock<HashMap<Arc<str>, Vec<JoinHandle<Result<()>>>>>>,
    request_channels: Arc<
        RwLock<HashMap<Arc<str>, Arc<RwLock<tokio::sync::mpsc::Receiver<Arc<NewTaskRequest>>>>>>,
    >,
    cancellation_token: Arc<CancellationToken>,
    notification_channels: Arc<NotificationChannels>,
    scheduler_handles: Arc<RwLock<HashMap<Arc<str>, Vec<JoinHandle<Result<()>>>>>>,
}
...
impl Scheduler<NotStarted> {
    ...
    pub async fn start(self) -> Result<Scheduler<Running>> {
        ...
        for schedule in &schedules {
            ...
            let (tx, rx) = tokio::sync::mpsc::channel::<Arc<NewTaskRequest>>(5);
            let tx = Arc::new(tx);
            let schedule_id = schedule.id();

            for channel_lock in schedule.channels() {
                let mut channel = channel_lock.write().await;
                if channel.needs_task_completion_notification() {
                            ...
                            maybe_task = rx.recv() => {
                                match maybe_task {
                                    Some(task_request) => {
                                        ...
                                        if task_request.clear_queue == true {
                                            for _ in 0..rx.len() {
