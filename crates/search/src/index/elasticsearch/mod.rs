...
/// Default candidate pool (kNN `k`) for a vector query when no `LIMIT` is pushed
/// down to the Elasticsearch scan.
///
/// The search pipeline places `Filter`/`Sort` above the index scan, so DataFusion
/// cannot push the requested top-K below them and the scan receives `limit = None`
/// (see `ElasticsearchKnnTable::scan`, which falls back to this `k`). A small
/// fallback silently capped every such query to a handful of hits; this matches
/// the sibling DuckDB vector backend's `DEFAULT_DUCKDB_VECTOR_SEARCH_LIMIT` (1000)
/// so enough candidates leave Elasticsearch before the pipeline applies the user's
/// filter, sort, and limit.
///
/// Kept at 1000 (not higher): the kNN exec requests `num_candidates = k * 2`, and
/// Elasticsearch rejects `num_candidates` above 10,000, so a larger default would
/// fail the search request outright.
const DEFAULT_ELASTICSEARCH_VECTOR_SEARCH_LIMIT: usize = 1000;

/// Guards against regressing the default back below the old hardcoded cap of 10
/// (the bug in #11264) while staying within Elasticsearch's `num_candidates` ceiling.
const _: () = assert!(
    DEFAULT_ELASTICSEARCH_VECTOR_SEARCH_LIMIT > 10
        && DEFAULT_ELASTICSEARCH_VECTOR_SEARCH_LIMIT * 2 <= 10_000
);
...
#[cfg(test)]
mod query_table_provider_tests {
    // Explicit imports (no `use super::*`): the parent module pulls the `Index`
    // trait and other `as_any` providers into scope, which makes a bare
    // `.as_any()` downcast ambiguous. Inspecting the built exec via the public
    // `source_as_provider` + `displayable` helpers sidesteps downcasting entirely.
    use std::sync::Arc;
    ...
    use super::{
        DEFAULT_ELASTICSEARCH_VECTOR_SEARCH_LIMIT, ElasticsearchIndex,
        ElasticsearchIndexWriteMaintenance,
    };
    use crate::index::SearchIndex;
    use crate::metadata::MetadataColumns;

    /// Client stub: `query_table_provider` only clones the client into the table
    /// provider and never calls it while building the plan, so every method panics.
    #[derive(Debug)]
    struct UnusedClient;

    #[async_trait]
    impl ElasticsearchTrait for UnusedClient {
        async fn get_mapping(&self, _: &str) -> elasticsearch::Result<MappingResponse> {
            unimplemented!("client is not invoked while building the query plan")
        }
        async fn search(
            &self,
            _: &str,
        ...
        async fn refresh_index(&self, _: &str) -> elasticsearch::Result<serde_json::Value> {
            unimplemented!("client is not invoked while building the query plan")
        }
        async fn force_merge(&self, _: &str, _: u32) -> elasticsearch::Result<serde_json::Value> {
            unimplemented!("client is not invoked while building the query plan")
        }
        async fn index_document(
            &self,
    ...
    /// Embed stub: the lazy embedder is only invoked at execution time, never
    /// while building the plan under test.
    #[derive(Debug)]
    struct UnusedEmbed;

    #[async_trait]
    impl Embed for UnusedEmbed {
        async fn embed(
            &self,
            _input: llms::embeddings::EmbeddingInput,
        ) -> llms::embeddings::Result<Vec<Vec<f32>>> {
            unimplemented!("query_table_provider defers embedding to execution time")
        }
        fn size(&self) -> i32 {
            8
        }
    ...
    fn test_index() -> ElasticsearchIndex {
        ElasticsearchIndex {
            client: Arc::new(UnusedClient),
            es_index: "test-index".to_string(),
            embedded_column: "body".to_string(),
            vector_field: "body_vector".to_string(),
            text_fields: vec![],
            primary_key: vec![Field::new("id", DataType::Int64, false)],
            compute_query: Arc::new(UnusedEmbed),
            dims: 8,
    ...
    /// Regression test for #11264: when no `LIMIT` is pushed down to the scan
    /// (the pipeline's `Filter`/`Sort` block pushdown), the kNN table must fall
    /// back to a large candidate pool rather than the previous hardcoded `10`,
    /// which silently capped every filtered/sorted vector search at 10 hits.
    ///
    /// Drives the real `query_table_provider` plan and inspects the kNN exec the
    /// scan builds when `limit = None` (building the exec does not call the
    /// Elasticsearch client — only `execute()` would). The exec's display reports
    /// the effective `k`, which equals the table's fallback candidate pool.
    #[tokio::test]
    async fn query_table_provider_uses_large_default_candidate_pool() {
        let index = test_index();

        let plan = index
            .query_table_provider("a search query")
            .expect("build query table provider");

        let LogicalPlan::TableScan(scan) = plan.as_ref() else {
            panic!("expected a TableScan plan, got: {plan:?}");
        };

        let provider =
            source_as_provider(&scan.source).expect("table scan should expose a TableProvider");

        // `limit = None` models the pipeline blocking limit pushdown with a
        // `Filter`/`Sort`, so the scan must use the table's fallback `k`.
        let ctx = SessionContext::new();
        let exec = provider
            .scan(&ctx.state(), None, &[], None)
            .await
            .expect("build kNN execution plan");

        let display = displayable(exec.as_ref()).one_line().to_string();
        assert!(
            display.contains(&format!("k={DEFAULT_ELASTICSEARCH_VECTOR_SEARCH_LIMIT}")),
            "kNN scan must request the shared default candidate pool \
             (k={DEFAULT_ELASTICSEARCH_VECTOR_SEARCH_LIMIT}) when no limit is pushed down, \
             not the old hardcoded 10; got: {display}"
        );
    }
}
