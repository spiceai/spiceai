...
use super::{
    CompoundReadMode, Error, compound_lifecycle_both, compound_on_write_start,
...
impl CompoundVectorIndex {
    ...
    fn primary_search(&self) -> Arc<dyn SearchIndex> {
        Arc::clone(&self.primary) as Arc<dyn SearchIndex>
    }

    fn secondary_search(&self) -> Arc<dyn SearchIndex> {
        Arc::clone(&self.secondary) as Arc<dyn SearchIndex>
    }
}
...
impl Index for CompoundVectorIndex {
    ...
    fn required_columns(&self) -> Vec<String> {
        compound_required_columns(&self.primary_search(), &self.secondary_search())
    }
    ...
    async fn on_write_start(&self) -> Result<(), DataFusionError> {
        compound_on_write_start(&self.primary_search(), &self.secondary_search()).await
    }
...
impl SearchIndex for CompoundVectorIndex {
    ...
    async fn write(
        ...
        compound_write(&self.primary_search(), &self.secondary_search(), record).await
    }
