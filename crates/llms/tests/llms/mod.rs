...
/// Serialises local model loading: building two multi-gigabyte models at once exhausts the
/// runner. Mirrors `LOCAL_LLM_INIT_MUTEX` in `crates/runtime/tests/models/hf.rs`.
static LOCAL_LLM_INIT_MUTEX: LazyLock<AsyncMutex<()>> = LazyLock::new(|| AsyncMutex::new(()));
...
async fn get_or_create_model(model_name: &str) -> Result<Arc<dyn Chat>, anyhow::Error> {
    ...
    let _init_lock = if is_local_model(model_name) {
        Some(LOCAL_LLM_INIT_MUTEX.lock().await)
    } else {
        None
    };
