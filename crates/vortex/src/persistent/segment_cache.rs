...
impl SharedSegmentCache {
    pub(crate) fn new(
        ...
        Self::new_registered_with_retirement(
            max_capacity_bytes,
            dataset,
            track_retirement,
    ...
    fn new_registered(
        ...
        Self::new_registered_with_retirement(max_capacity_bytes, dataset, false, metrics)
    }

    fn new_registered_with_retirement(
        max_capacity_bytes: u64,
        dataset: Option<Arc<str>>,
        track_retirement: bool,
        metrics: &SegmentCacheMetrics,
...
impl SegmentCache for PathSegmentCache {
    async fn get(&self, id: SegmentId) -> VortexResult<Option<ByteBuffer>> {
        let result = self.shared.state.cache.get(&(self.path.clone(), id)).await;

        // Observable collection reads these atomics directly, so the hot path
        // never allocates labels or synchronously records metrics.
        self.shared
            .dataset_metrics
            .counters
            .accesses
...
mod tests {
    ...
    async fn invalidation_physically_evicts_entries_without_later_cache_activity() {
        ...
        assert_eq!(shared.state.cache.entry_count(), 258);

        shared.invalidate_paths(HashSet::from([retired_path])).await;
        assert_eq!(shared.state.cache.entry_count(), 257);
    }
