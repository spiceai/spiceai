...
use data_components::flightsql::{query_to_stream, FlightSqlClient, FlightSqlExec};
...
fn try_rewrite(
    ...
    // when that is cheaper than shipping the fact to the coordinator
    // (`dim_rows × N < fact_rows`) — a scale-invariant broadcast-vs-shuffle cost
    // test that fires for small dims at any scale factor — and cap the dim size
    // to bound per-executor memory. Requires row-count stats on both sides
    // (else we cannot tell which side is small, so leave the plan alone).
    let dim_is_left = match (left_rows, right_rows) {
        (Some(l), Some(r)) => {
            let (dim_rows, fact_rows, is_left) =
                if l <= r { (l, r, true) } else { (r, l, false) };
            if dim_rows > max_broadcast_rows
                || dim_rows.saturating_mul(num_executors) >= fact_rows
            {
                return Ok(Transformed::no(plan));
            }
            is_left
    ...
    let sql = match build_join_sql(join, fact, dim, fact_is_left, addresses) {
        Some(sql) => sql,
...
fn build_join_sql(
    ...
    for (l, r) in join.on() {
        ...
        let (fact_col, dim_col) = if fact_is_left { (l_name, r_name) } else { (r_name, l_name) };
        on_terms.push(format!("f.{} = d.{}", quote_ident(fact_col), quote_ident(dim_col)));
    }
    ...
    let fact_names: std::collections::HashSet<&str> =
        fact.schema.fields().iter().map(|f| f.name().as_str()).collect();
    let dim_names: std::collections::HashSet<&str> =
        dim.schema.fields().iter().map(|f| f.name().as_str()).collect();
    ...
    let columns = target
        ...
        .map(|(i, field)| {
            ...
            if col.data_type() == field.data_type() {
                ...
                cast(col, field.data_type()).map_err(|e| DataFusionError::ArrowError(Box::new(e), None))
            }
        })
