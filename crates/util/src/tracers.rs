...
//! Log-deduplication helpers: tracers that remember what they have already
//! logged so a repeated condition does not flood the log.
//!
//! The macros are `#[macro_export]`ed, so they are named from this crate's root
//! (`util::warn_once!`) while the tracer structs they operate on live here.
