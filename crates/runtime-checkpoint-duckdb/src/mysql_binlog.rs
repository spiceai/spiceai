...
/// `MySQL` binlog position store backed by a `DuckDB` accelerator.
pub struct DuckDbMySqlBinlogStore {
    pool: Arc<DuckDbConnectionPool>,
    dataset_name: String,
}

impl DuckDbMySqlBinlogStore {
    #[must_use]
    ...
    fn upsert_duckdb(
        ...
        for migration in migrate_columns() {
            duckdb_conn.execute(&migration, []).map_err(store_error)?;
        }
        ...
        duckdb_conn
            .execute(
                ...
                duckdb::params![
                    ...
                    position_to_i64(checkpoint.binlog_pos),
...
#[async_trait]
impl MySqlBinlogStore for DuckDbMySqlBinlogStore {
    ...
    async fn upsert(&self, checkpoint: &MySqlBinlogCheckpoint) -> Result<(), CheckpointError> {
        ...
        spawn_checkpoint_blocking(move || Self::upsert_duckdb(&dataset_name, &pool, &checkpoint))
            .await
    }
...
mod tests {
    ...
    async fn test_duckdb_roundtrip() {
        let (pool, _tmp) = temp_pool("test_mysql_binlog_duckdb_roundtrip");
        let dataset_name = "test_mysql_binlog_duckdb_roundtrip".to_string();
        let sys = DuckDbMySqlBinlogStore::new(Arc::clone(&pool), dataset_name.clone());
    ...
    async fn test_duckdb_empty_gtid_set_stays_gtid() {
        let (pool, _tmp) = temp_pool("test_mysql_binlog_duckdb_empty_gtid");
        let dataset_name = "test_mysql_binlog_duckdb_empty_gtid".to_string();
        let sys = DuckDbMySqlBinlogStore::new(Arc::clone(&pool), dataset_name.clone());
    ...
    async fn test_duckdb_checkpoint_overwrite() {
        let (pool, _tmp) = temp_pool("test_mysql_binlog_duckdb_overwrite");
        let dataset_name = "test_mysql_binlog_duckdb_overwrite".to_string();
        let sys = DuckDbMySqlBinlogStore::new(Arc::clone(&pool), dataset_name.clone());
    ...
    async fn test_duckdb_get_nonexistent() {
        let (pool, _tmp) = temp_pool("test_mysql_binlog_duckdb_get_nonexistent");
        let dataset_name = "test_mysql_binlog_duckdb_get_nonexistent".to_string();
        let sys = DuckDbMySqlBinlogStore::new(Arc::clone(&pool), dataset_name.clone());
    ...
    async fn test_duckdb_delete() {
        let (pool, _tmp) = temp_pool("test_mysql_binlog_duckdb_delete");
        let dataset_name = "test_mysql_binlog_duckdb_delete".to_string();
        let sys = DuckDbMySqlBinlogStore::new(Arc::clone(&pool), dataset_name.clone());
