/*
Copyright 2024-2026 The Spice.ai OSS Authors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

     https://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

//! The caching engine's per-dataset `fetched_at` marker.
//!
//! `DuckDB`-only: it is the only engine Spice serves cached results from.

use std::sync::Arc;

use datafusion_table_providers::duckdb::DuckDB;
use datafusion_table_providers::sql::db_connection_pool::duckdbpool::DuckDbConnectionPool;
use runtime_checkpoint_api::CheckpointError;

use crate::{spawn_checkpoint_blocking, store_error};

/// Records when the caching engine last fetched a dataset held in a `DuckDB`
/// accelerator.
pub struct DuckDbCachingEngine {
    pool: Arc<DuckDbConnectionPool>,
    dataset_name: String,
}

impl DuckDbCachingEngine {
    #[must_use]
    pub fn new(pool: Arc<DuckDbConnectionPool>, dataset_name: String) -> Self {
        Self { pool, dataset_name }
    }

    /// Stamps this dataset as fetched now.
    ///
    /// # Errors
    ///
    /// Returns [`CheckpointError::Store`] when the marker cannot be written.
    pub async fn update_fetched_at(&self) -> Result<(), CheckpointError> {
        let pool = Arc::clone(&self.pool);
        let dataset_name = self.dataset_name.clone();
        spawn_checkpoint_blocking(move || Self::update_fetched_at_duckdb(&dataset_name, &pool))
            .await
    }

    /// Blocking: takes the pool's write gate. Callers must reach this through
    /// `spawn_duckdb_blocking`, never directly from an async worker.
    fn update_fetched_at_duckdb(
        dataset_name: &str,
        pool: &Arc<DuckDbConnectionPool>,
    ) -> Result<(), CheckpointError> {
        let write_gate = pool.write_gate();
        let _write_guard = write_gate
            .read()
            .unwrap_or_else(std::sync::PoisonError::into_inner);

        let mut db_conn = Arc::clone(pool).connect_sync().map_err(store_error)?;
        let duckdb_conn = DuckDB::duckdb_conn(&mut db_conn)
            .map_err(store_error)?
            .get_underlying_conn_mut();

        let tx = duckdb_conn.transaction().map_err(store_error)?;

        let has_table = table_exists(&tx, dataset_name)?;
        let mut internal_tables = list_internal_tables(&tx, dataset_name)?;

        // Determine the actual table name (internal or direct)
        let table_name = match (internal_tables.pop(), has_table) {
            (Some((internal_name, _)), _) => internal_name,
            (None, true) => dataset_name.to_string(),
            (None, false) => {
                // No table exists yet
                tracing::warn!("No table found for dataset: {dataset_name}");
                return Ok(());
            }
        };

        // Update fetched_at for the table. Escape embedded double-quotes by doubling them so the
        // quoted identifier stays well-formed (and can't break out of the identifier) even if the
        // dataset/table name contains a `"`.
        let escaped_table_name = table_name.replace('"', "\"\"");
        let update_query = format!(
            "UPDATE \"{escaped_table_name}\" SET _fetched_at = (now() AT TIME ZONE 'UTC')::TIMESTAMP_NS"
        );
        tx.execute(&update_query, []).map_err(store_error)?;

        tx.commit().map_err(store_error)?;
        Ok(())
    }
}

fn table_exists(tx: &duckdb::Transaction<'_>, table_name: &str) -> Result<bool, CheckpointError> {
    let mut stmt = tx
        .prepare("SELECT 1 FROM duckdb_tables() WHERE table_name = ?")
        .map_err(store_error)?;
    let mut rows = stmt.query([table_name]).map_err(store_error)?;
    Ok(rows.next().map_err(store_error)?.is_some())
}

fn list_internal_tables(
    tx: &duckdb::Transaction<'_>,
    table_name: &str,
) -> Result<Vec<(String, u64)>, CheckpointError> {
    // Escape LIKE metacharacters (`%`, `_`, and the escape char itself) in the dataset name so
    // they are matched literally. Dataset names frequently contain `_`, which is a single-char
    // wildcard in LIKE and would otherwise match unrelated internal tables.
    let escaped = table_name
        .replace('\\', "\\\\")
        .replace('%', "\\%")
        .replace('_', "\\_");
    let pattern = format!("__data_{escaped}%");
    let mut stmt = tx
        .prepare("SELECT table_name FROM duckdb_tables() WHERE table_name LIKE ? ESCAPE '\\'")
        .map_err(store_error)?;
    let mut rows = stmt.query([pattern]).map_err(store_error)?;

    let mut table_names = Vec::new();
    while let Some(row) = rows.next().map_err(store_error)? {
        let internal_table_name: String = row.get(0).map_err(store_error)?;
        let Some(inner_name) = internal_table_name.strip_prefix("__data_") else {
            continue;
        };
        let Some((inner_table_name, timestamp)) = inner_name.rsplit_once('_') else {
            continue;
        };
        if inner_table_name != table_name {
            continue;
        }
        let timestamp = timestamp.parse::<u64>().map_err(store_error)?;
        table_names.push((internal_table_name, timestamp));
    }

    table_names.sort_by_key(|left| left.1);
    Ok(table_names)
}
