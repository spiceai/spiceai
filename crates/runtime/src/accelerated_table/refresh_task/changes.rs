...
mod tests {
    ...
    // ---------------------------------------------------------------------
    // Tests for `start_changes_stream` (the CDC source-stream to apply
    ...
    /// Used to assert the apply-to-commit ordering invariant.
    #[derive(Default)]
    struct CommitLog {
        // (envelope_id, commit_outcome)
        events: StdMutex<Vec<(i32, Result<(), String>)>>,
    }
    ...
    impl CommitLog {
        ...
        fn ids(&self) -> Vec<i32> {
            self.events
                .lock()
                .expect("commit log should not be poisoned")
                .iter()
                .map(|(id, _)| *id)
                .collect()
        }
    ...
    struct WriteOrderRecordingProvider {
        ...
        write_log: Arc<StdMutex<Vec<&'static str>>>,
    }
    ...
    impl CommitChange for SequencedCommitter {
        fn commit(&self) -> Result<(), CommitError> {
            self.log
                .lock()
                .expect("write/commit log should not be poisoned")
                .push("commit");
            Ok(())
        }
    }
    ...
    async fn test_start_changes_stream_commits_after_write() {
        let write_log: Arc<StdMutex<Vec<&'static str>>> = Arc::new(StdMutex::new(Vec::new()));
        ...
        let combined: Arc<StdMutex<Vec<&'static str>>> = Arc::clone(&write_log);
    ...
    async fn test_start_changes_stream_pipelines_reads_with_writes() {
        ...
        assert_eq!(log.ids(), vec![1, 2, 3, 4, 5, 6]);
    }
