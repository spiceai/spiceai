...
impl CacheRefreshHelper {
    ...
    async fn fetch_from_source_on_miss(
        ...
        tracing::debug!(
            "Cache: Fetching from source on cache miss for dataset {}",
            dataset_name
        );
    ...
    fn execute(
        ...
        let cache_miss_or_stale_stream = futures::stream::once(async move {
            ...
            if let Some(first_batch) = accelerator_stream.next().await {
                ...
                match CacheRefreshHelper::fetch_from_source_on_miss(federated, &dataset_name, &filters, limit).await {
