...
fn validate_refresh_lookback_window(refresh: &Refresh, dataset: &str, schema: &SchemaRef) {
    if refresh.period.is_some() {
        if let Some(time_column) = &refresh.time_column {
            if schema.column_with_name(time_column).is_none() {
                tracing::warn!(
                    "No matching column found for {time_column} in the source table, refresh_lookback_window will be ignored for {dataset}"
                );
            }
        } else {
            tracing::warn!(
                "No time_column is provided, refresh_lookback_window will be ignored for {dataset}"
            );
        }
    }
}
...
impl Builder {
    ...
    pub async fn build(self) -> (AcceleratedTable, oneshot::Receiver<()>) {
        ...
        validate_refresh_lookback_window(&self.refresh, &self.dataset_name, &self.federated.schema());
