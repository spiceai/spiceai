...
///
/// Returns the decoded [`Statistics`] (the reporter decides how rich they are
/// and handles encoding/caching — see [`StatsRichness`]).
///
...
fn effective_max_for_ndv(
    ...
    let add = |m: i128| -> i128 { m.saturating_add(ndv as i128) };
    // Synthetic max = min + ndv, in the column's own integer type.
    let synthetic = match min_sv {
        SV::Int8(Some(m)) => i8::try_from(add(i128::from(*m))).ok().map(|v| SV::Int8(Some(v))),
        SV::Int16(Some(m)) => i16::try_from(add(i128::from(*m))).ok().map(|v| SV::Int16(Some(v))),
        SV::Int32(Some(m)) => i32::try_from(add(i128::from(*m))).ok().map(|v| SV::Int32(Some(v))),
        SV::Int64(Some(m)) => i64::try_from(add(i128::from(*m))).ok().map(|v| SV::Int64(Some(v))),
        SV::UInt8(Some(m)) => u8::try_from(add(i128::from(*m))).ok().map(|v| SV::UInt8(Some(v))),
        SV::UInt16(Some(m)) => u16::try_from(add(i128::from(*m))).ok().map(|v| SV::UInt16(Some(v))),
        SV::UInt32(Some(m)) => u32::try_from(add(i128::from(*m))).ok().map(|v| SV::UInt32(Some(v))),
        SV::UInt64(Some(m)) => u64::try_from(add(i128::from(*m))).ok().map(|v| SV::UInt64(Some(v))),
        _ => None,
    };
