...
pub(crate) async fn local_executor_table_statistics(
    ...
    let Some(provider) = df.get_table(table).await else {
        tracing::info!(table = %table, "q18-stats: executor get_table returned None");
        return None;
    };
    ...
    let stats = match scan_stats {
        Some(s) if s.num_rows.get_value().is_some() => {
            let cols_with_stats = s
                .column_statistics
                .iter()
            ...
            tracing::info!(
                table = %table,
                num_rows = ?s.num_rows.get_value(),
                cols = s.column_statistics.len(),
                cols_with_minmax = cols_with_stats,
                "q18-stats: executor computed scan-plan statistics"
            );
            s
        }
        _ => {
            ...
            match n {
                Some(n) => {
                    tracing::info!(table = %table, num_rows = n, "q18-stats: executor computed row count via COUNT(*)");
                    datafusion::common::Statistics::new_unknown(&schema)
                        .with_num_rows(Precision::Inexact(n))
                }
                None => {
                    tracing::info!(table = %table, "q18-stats: executor could NOT compute statistics");
                    return None;
                }
            }
        }
