...
///
/// This is the same logic used by [`PartitionManagementTask::update_dataset_statuses`],
...
pub(crate) async fn update_dataset_statuses_from_partitions(
    ...
    for table_name in &tables {
        ...
        let all_assigned = metadata.partitions.iter().all(|p| p.is_assigned());
...
mod tests {
    ...
    async fn setup_partition_manager(
        ...
        for (table_name, partitions) in tables {
            ...
            // Now write metadata with the actual partitions
        ...
        // Refresh cache so get_cached_table_metadata works
    ...
    async fn test_all_partitions_assigned_marks_dataset_ready() {
        ...
        // Register dataset as Initializing (like the scheduler does)
    ...
    async fn test_empty_partitions_marks_dataset_ready() {
        // A table with no partitions (edge case) should be considered ready
    ...
    async fn test_no_tables_in_partition_manager() {
        // Empty partition manager - no tables tracked
        let store = Arc::new(InMemory::new());
        ...
        // Status should remain unchanged since no tables are tracked
        assert_eq!(
    ...
    #[tokio::test]
    async fn test_runtime_not_ready_until_all_datasets_assigned() {
        // Simulates the end-to-end flow:
        // 1. Dataset registered as Initializing (by accelerated_table on scheduler)
        // 2. partition_metadata component registered as Initializing
        // 3. After management cycle assigns all partitions -> dataset becomes Ready
        // 4. partition_metadata already marked Ready
        // 5. Now is_ready() returns true
        ...
        status.update_dataset(&table_ref, ComponentStatus::Initializing);
        // Step 2: partition_metadata also registered
        ...
        // Simulate: management cycle discovers and assigns partitions
        // (In reality, run_management_cycle would do this)
        // For test, we write updated metadata with all partitions assigned
        let metadata = super::super::metadata::TablePartitionMetadata {
        ...
        // Step 3: partition_metadata marked Ready (by management cycle success)
        status.update_component_status("partition_metadata", ComponentStatus::Ready);

        // Step 4: update_dataset_statuses promotes dataset to Ready
        update_dataset_statuses_from_partitions(&partition_manager, &status).await;
        ...
        // Step 5: Now runtime should be ready
        assert!(
            status.is_ready(),
            "Runtime should be ready when all components (dataset + partition_metadata) are Ready"
        );
    }
}
