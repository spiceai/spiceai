...
impl Chat for ChatWrapper {
    ...
    async fn chat_stream(
        ...
        match self.chat.chat_stream(req).instrument(span.clone()).await {
            Ok(resp) => {
                let logged_stream = resp.instrument(span.clone()).inspect(move |item| {
                    if let Ok(item) = item {
                        ...
                        if let Some(usage) = item.usage.clone() {
                            tracing::info!(target: "task_history", parent: &sspan, completion_tokens = %usage.completion_tokens, total_tokens = %usage.total_tokens, prompt_tokens = %usage.prompt_tokens, "labels");
                        }
                    }
                });
            ...
            Err(e) => {
                tracing::error!(target: "task_history", parent: &span2, "Failed to run chat model: {}", e);
                Err(e)
            }
        }
    }
