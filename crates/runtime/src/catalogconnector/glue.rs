...
pub(super) static VALIDATORS: LazyLock<
...
/// Table providers are created via `runtime::dataconnector::glue::{create_s3_provider,
/// create_iceberg_provider}` — the same helpers used by `connector-glue` — so this
/// catalog connector never goes through the `DataConnector` trait or the factory registry.
#[derive(Clone)]
