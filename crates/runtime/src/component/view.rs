...
impl AccelerationSource for View {
    ...
    fn definition_fingerprint(
        ...
        // own SQL plus every view, dataset, and catalog it transitively reads — so that is
        // what a snapshot of them is valid against. The closure also carries the
        // configuration that shapes the stored VALUES rather than the row set:
        // `prepare_view` builds embedding and full-text columns from `columns` and reads
        // `file_format` from `params`, so an embedding model can be swapped for another of
