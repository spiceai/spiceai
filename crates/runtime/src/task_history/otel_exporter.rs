...
impl TaskHistoryExporter {
    ...
    fn redact_payload(task: &str, value: Arc<str>) -> Arc<str> {
        if value.is_empty() || !Self::should_redact_task_payload(task) {
            value
        } else {
    ...
    fn span_to_task_span(&self, span: SpanData) -> TaskSpan {
        ...
        let captured_output: Option<Arc<str>> = extract_attr!(span, "captured_output")
            ...
            .map(|output| Self::redact_payload(task.as_ref(), output));
