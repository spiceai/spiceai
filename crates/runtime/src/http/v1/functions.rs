...
// Needed by the `utoipa::path` attribute macro below when the `openapi`
// feature is enabled. The proc-macro layer hides the usage from the
// compiler's name resolution, so the import looks unused at lint time —
// expect the warning so stale allows don't linger.
#[expect(unused_imports, reason = "consumed by utoipa::path macro expansion")]
use serde_json::json;
