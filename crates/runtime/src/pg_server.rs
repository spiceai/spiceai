...
use arrow::{array::{Array, ArrayRef, BooleanBuilder, Int32Array, Int32Builder, Int64Array, Int64Builder, ListBuilder, NullArray, RecordBatch, StringArray, StringBuilder, UInt32Builder}, datatypes::{DataType, Field, Schema, SchemaRef}};
use datafusion::{catalog::schema::{MemorySchemaProvider, SchemaProvider}, common::{cast::as_list_array, not_impl_err, plan_datafusion_err, plan_err}, datasource::{function::{TableFunction, TableFunctionImpl}, MemTable, TableProvider, TableType}, error::DataFusionError, execution::{context::{ExecutionProps, SessionContext, SessionState}, TaskContext}, logical_expr::{create_udf, simplify::SimplifyContext, ColumnarValue, Expr, LogicalPlanBuilder, ScalarUDF, ScalarUDFImpl, Signature, Volatility}, optimizer::simplify_expressions::ExprSimplifier, physical_plan::{functions::make_scalar_function, memory::MemoryExec, udf, ExecutionPlan}, scalar::ScalarValue, sql::TableReference};
...
pub async fn start(bind_address: std::net::SocketAddr, df: Arc<RwLock<DataFusion>>) -> Result<()> {
    ...
    df.read().await
        ...
        .unwrap()
        .register_schema("pg_catalog", schema)
        .unwrap();
    ...
    df.read().await.ctx
        .register_table(pg_namespace_table_reference, Arc::new(PgCatalogNamespaceProvider::new())).unwrap();

    df.read().await.ctx.register_udtf(
        "generate_series",
        Arc::new(GenerateSeriesUDTF {}),
    );
    df.read().await.ctx.register_udf(array_upper::create_udf());
    df.read().await.ctx.register_udf(current_schema::create_udf());
    df.read().await.ctx.register_udf(current_schemas::create_udf());
    df.read().await.ctx.register_udf(version::create_udf());
