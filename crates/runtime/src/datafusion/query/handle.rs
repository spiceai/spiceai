...
impl QueryHandle {
    ...
    /// Requests cancellation for this query.
    ...
    fn spawn_finalize(
        ...
        handle.spawn(
            async move {
                ...
                if request_cancel {
                    // Drop-guard path: the caller dropped the handle without
                    // going through `finish_tracker_*`. That can happen
                    // either because the client genuinely disconnected, or
                    // because they `poll_status()`-ed until success and then
                    // dropped — `poll_status` is read-only and does not
                    // finalize the tracker. Peek the scheduler's terminal
                    // status before assuming disconnect so we don't
                    // mis-record an already-finished job as failed and
                    // don't request cancellation for a job that already
                    // completed.
                ...
                if let Some(graph) = graph_for_record {
                    let graph = graph.execution_graph.read().await;
