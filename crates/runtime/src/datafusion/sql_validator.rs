...
mod tests {
    ...
    fn test_is_safe_set_variable_allowlist() {
        ...
        assert!(is_safe_set_variable("datafusion.optimizer.enable_round_robin_repartition"));
        assert!(is_safe_set_variable("datafusion.execution.target_partitions"));
        ...
        assert!(!is_safe_set_variable("datafusion.execution.parquet.pushdown_filters"));
    ...
    async fn test_read_only_validator_rejects_unsafe_set() {
        ...
        validate_sql_query_read_only(&plan)
            .expect_err("SET of a pinned parquet config must be rejected even under read-only auth");
    }
}
