...
impl CommentLookup {
    ...
    async fn table_schema(
        &self,
        table_key: &TableKey,
        cache: &mut HashMap<TableKey, Option<SchemaRef>>,
    ) -> Result<Option<SchemaRef>, DataFusionError> {
        if let Some(schema) = cache.get(table_key) {
            return Ok(schema.clone());
        }

        let schema = self.load_table_schema(table_key).await?;
        cache.insert(table_key.clone(), schema.clone());
        Ok(schema)
    }
...
impl ObjDescription {
    ...
    async fn table_comment_for_row(
        ...
        let Some(table_key) = table_key_from_obj_description_args(&self.lookup, args, row)? else {
            return Ok(None);
        };
...
impl ColDescription {
    ...
    async fn column_comment_for_row(
        ...
        let Some((table_key, selector)) = col_description_args(&self.lookup, args, row)? else {
            return Ok(None);
        };
        let Some(schema) = self.lookup.table_schema(&table_key, cache).await? else {
            return Ok(None);
...
fn table_key_from_obj_description_args(
    ...
    match args.len() {
        1 => Ok(string_arg(args, 0, row)?.map(|table_name| lookup.resolve_table_name(&table_name))),
        2 => {
            let Some(table_name) = string_arg(args, 0, row)? else {
                return Ok(None);
            };
            ...
            Ok(Some(lookup.resolve_table_name(&table_name)))
        }
...
fn col_description_args(
    ...
    match args.len() {
        2 => {
            ...
            Ok(Some((lookup.resolve_table_name(&table_name), selector)))
        }
        4 => {
