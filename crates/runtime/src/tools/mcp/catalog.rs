...
impl McpToolCatalog {
    pub async fn try_new(cfg: MCPConfig, name: &str, description: Option<&str>) -> Result<Self> {
        let client = match cfg {
            MCPConfig::Stdio { command, args } => {
                Self::stdio_transport(command.as_str(), args).await
            }
            MCPConfig::Https { url } => Self::https_client(url).await,
        }
        .map_err(|e| Error::UnderlyingTransportError { source: e })?;
    ...
    async fn stdio_transport(
    ...
    ) -> std::result::Result<RwLock<dyn McpClientTrait>, TransportError> {
        ...
        Ok(RwLock::new(McpClient::new(service)))
    }
    ...
    async fn list_tools(&self) -> std::result::Result<Vec<McpTool>, McpError> {
        ...
        loop {
            let response = self.client.list_tools(cursor.clone()).await?;
