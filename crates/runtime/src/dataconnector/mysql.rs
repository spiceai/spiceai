...
pub struct MySQL {
    pool: Arc<dyn DbConnectionPool<mysql_async::Conn, &'static (dyn ToValue + Sync)> + Send + Sync>,
}
    ...
    fn get_all_data(
        ...
        Box::pin(async move {
            ...
            let Some(async_conn) = conn.as_async() else {
                tracing::error!("Failed to convert mysql conn to async connection",);
                return vec![];
            };
            ...
            let recs: Vec<RecordBatch> =
                match record_batch_stream.try_collect::<Vec<RecordBatch>>().await {
                    Ok(recs) => recs,
                    Err(e) => {
                        tracing::error!("Failed to collect record batches from MySQL: {e}");
                        return vec![];
                    }
                };
