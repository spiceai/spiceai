...
impl CayenneAccelerator {
    ...
    fn get_vortex_config(
        ...
        if let Some(acceleration) = source.acceleration() {
            ...
            tracing::debug!(
                "Cayenne Vortex config: footer_cache={}MB, segment_cache={}MB, target_file_size={}MB, sort_columns={:?}, sort_on_refresh={:?}, compression_strategy={:?}",
                config.footer_cache_mb,
                config.segment_cache_mb,
                config.target_vortex_file_size_mb,
                config.sort_columns,
                config.sort_on_refresh,
                config.compression_strategy
            );
        }
...
const PARAMETERS: &[ParameterSpec] = &[
    ...
    ParameterSpec::component("sort_on_refresh")
        .description("Whether to sort data on full refresh operations. Options: 'enabled' (default), 'disabled'")
        .default("enabled"),
