...
>(
    ...
    [
        ...
        ParameterSpec::component("compaction_max_concurrent_merges")
            .description("Maximum number of disjoint protected-snapshot size tiers a single key-delete compaction pass merges concurrently, instead of merging only the lowest qualifying tier. Each concurrent merge draws its own cayenne_write_concurrency encode permits from the process-global budget, so raising this lets a table use more of the encode pool per pass when write_concurrency is much smaller than the pool. Ignored for position-delete tables, which always merge one tier at a time. Default: 1 (0 is clamped to 1; today's single-tier-per-pass behavior). Still experimental: force-disabled fleet-wide regardless of this setting by the SPICE_CAYENNE_DISABLE_PIPELINED_COMPACTION environment variable.")
            .default("1"),
