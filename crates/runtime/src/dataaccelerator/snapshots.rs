...
pub(crate) async fn snapshot_before_recreate(
    ...
    // fails at `infer_existing_partitions` with `TableNotFound` once the drop cascade
    // has removed the live child rows. Publishing would make that unrestorable archive
    // the store's `current-snapshot-id`, so skip until the slice covers child tables.
    // `build_snapshot_creation_config` applies the same gate to the periodic
    // publish path.
