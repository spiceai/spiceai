    ...
    async fn create_external_table(
        ...
        match cmd.options.remove("on_full_refresh").as_deref() {
            ...
            Some(other) => super::InvalidConfigurationSnafu {
                msg: format!(
                    "Failed to register dataset {name} (duckdb accelerator): Invalid 'on_full_refresh' value '{other}'. Expected 'reuse_file' or 'replace_file'. See: {DUCKDB_ACCELERATOR_DOCS}",
                    name = dataset_display_name,
                ),
            }
            .fail()?,
