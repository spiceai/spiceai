...
pub(crate) fn init_tracing(trace_config: Option<&str>) {
    let filter = match (trace_config, std::env::var("SPICED_LOG").ok()) {
        ...
        _ => EnvFilter::new(
            "datafusion-federation=DEBUG,bench=DEBUG,runtime=DEBUG,secrets=INFO,data_components=INFO,cache=INFO,extensions=INFO,spice_cloud=INFO,llms=INFO,reqwest_retry::middleware=off,task_history=off,WARN",
        ),
    };
