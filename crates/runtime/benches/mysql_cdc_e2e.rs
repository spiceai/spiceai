...
/// timed region.
///
/// `SPICE_BENCH_EAGER_REPLAY=1` ships materialized batches (the pump's eager
/// delivery shape). Decode then happens here in setup, so the timed region
/// measures pure apply — the consumer's ceiling when pump-side decode runs
/// concurrently on its own task, as in production.
fn replay_envelopes(binlog: &[u8], cap: usize) -> Vec<ChangeEnvelope> {
    let table = ReplayTable {
        ...
        eager: std::env::var_os("SPICE_BENCH_EAGER_REPLAY").is_some(),
    };
