...
fn statement_query(sql: &str) -> CommandStatementQuery {
...
fn get_flight_info_request(sql: &str) -> Request<FlightDescriptor> {
    Request::new(FlightDescriptor::new_cmd(
        statement_query(sql).as_any().encode_to_vec(),
    ))
}
...
///
/// The second query is what a pooled client does: an id that repeated per
/// connection would correlate a whole pool's traffic to one row.
#[tokio::test]
...
-> Result<(), anyhow::Error> {
    ...
    test_request_context()
        .scope(async {
            ...
            let info = client
                .inner_mut()
                .get_flight_info(get_flight_info_request("SELECT 1 AS n"))
                .await?;

            let from_metadata = trace_id_metadata(info.metadata());
            assert!(
                normalize_trace_id(&from_metadata).is_some(),
                "not a usable trace id: `{from_metadata}`"
            );
            ...
            let next = client
                ...
                .await?;
            assert_ne!(
                trace_id_metadata(next.metadata()),
                from_metadata,
...
async fn a_pinned_trace_id_is_the_one_returned() -> Result<(), anyhow::Error> {
    ...
    test_request_context()
        .scope(async {
            ...
            let info = client.inner_mut().get_flight_info(request).await?;
...
async fn an_unwrapped_ticket_still_runs() -> Result<(), anyhow::Error> {
    ...
    test_request_context()
        .scope(async {
            ...
            let results = create_flight_client(channel, None)?
                .do_get(Ticket::new(
                    statement_query("SELECT 1 AS n").as_any().encode_to_vec(),
                ))
                .await?;
