...
async fn shared_slot_multiplexes_multiple_datasets() -> Result<(), anyhow::Error> {
    ...
    assert_eq!(boot_a.change_batch().expect("built change batch").record.num_rows(), 2, "bootstrap a rows");
    ...
    assert_eq!(boot_b.change_batch().expect("built change batch").record.num_rows(), 3, "bootstrap b rows");
    ...
    assert_eq!(boot_c.change_batch().expect("built change batch").record.num_rows(), 1, "bootstrap c rows");
    ...
    assert_eq!(boot_d.change_batch().expect("built change batch").record.num_rows(), 1, "bootstrap d rows");
    boot_d.commit().await?;
    ...
    assert_eq!(
        ready.change_batch().expect("built change batch").record.num_rows(),
        0,
    ...
    assert_eq!(
        boot_a3.change_batch().expect("built change batch").record.num_rows(),
        6,
...
async fn shared_slot_partitioned_source_table_streams_changes() -> Result<(), anyhow::Error> {
    ...
    assert_eq!(
        boot.change_batch().expect("built change batch").record.num_rows(),
        2,
...
async fn shared_and_independent_slots_coexist() -> Result<(), anyhow::Error> {
    ...
    assert_eq!(boot_a.change_batch().expect("built change batch").record.num_rows(), 1);
    boot_a.commit().await?;
    ...
    assert_eq!(boot_b.change_batch().expect("built change batch").record.num_rows(), 1);
    boot_b.commit().await?;
    ...
    assert_eq!(boot_i.change_batch().expect("built change batch").record.num_rows(), 1);
    boot_i.commit().await?;
