...
async fn start(keys: &[&str]) -> Result<TestRuntime, anyhow::Error> {
    ...
    let rt = Arc::new(
        Runtime::builder()
            .with_app(app::AppBuilder::new("sessions_test").build())
            .build()
            .await,
    );
...
-> Result<(), anyhow::Error> {
    ...
    test_request_context()
        .scope(async {
            ...
            rt.sql_ok("k", None, "PREPARE p AS SELECT 1 AS result").await?;
...
async fn naming_an_unknown_session_is_reported_as_such() -> Result<(), anyhow::Error> {
    ...
    test_request_context()
        .scope(async {
            ...
            let (status, body) = rt
                .sql("k", Some("00000000-0000-4000-8000-000000000000"), "SELECT 1")
                .await?;
...
async fn a_session_cannot_be_used_by_another_principal() -> Result<(), anyhow::Error> {
    ...
    test_request_context()
        .scope(async {
            ...
            rt.sql_ok("a", Some(&session), "PREPARE victim AS SELECT 'a data' AS v")
                .await?;
...
async fn two_principals_naming_the_same_id_do_not_share_a_session() -> Result<(), anyhow::Error> {
    ...
    test_request_context()
        .scope(async {
            ...
            let (status, body) = rt
                .sql("a", Some(GUESSABLE), "PREPARE squat AS SELECT 'a private' AS v")
                .await?;
            assert_eq!(
                status,
    ...
    test_request_context()
        .scope(async {
            ...
            rt.flight_sql("k", &session, "PREPARE from_flight AS SELECT 8 AS n")
                .await?;
            let body = rt.sql_ok("k", Some(&session), "EXECUTE from_flight").await?;
            assert_eq!(
                serde_json::from_str::<Value>(&body)?,
                serde_json::json!([{ "n": 8 }]),
            ...
            let body = rt.sql_ok(&session, Some(&session), "EXECUTE shared").await?;
            assert_eq!(
                serde_json::from_str::<Value>(&body)?,
                serde_json::json!([{ "n": 7 }])
            );
