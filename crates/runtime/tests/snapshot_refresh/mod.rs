...
fn build_writer_dataset(config: &WorkerConfig) -> Dataset {
    ...
    dataset.params = Some(Params::from_string_map(
        SnapshotRefreshFixture::dataset_params(),
    ));
    dataset.acceleration = Some(Acceleration {
        enabled: true,
        mode: Mode::File,
...
pub(crate) async fn run_writer_phase(config: &WorkerConfig) -> Result<()> {
    ...
    let app = AppBuilder::new(format!("snapshot_writer_{}", config.engine.engine_name()))
    ...
    let first_id = wait_for_snapshot_id_at_prefix(&config.s3_prefix, 0, Duration::from_secs(120))
        .await
        .context("waiting for writer to produce initial snapshot")?;
    ...
    wait_for_snapshot_id_at_prefix(&config.s3_prefix, first_id + 1, Duration::from_secs(120))
        .await
...
pub(crate) async fn run_reader_phase(config: &WorkerConfig) -> Result<()> {
    let app = AppBuilder::new(format!("snapshot_reader_{}", config.engine.engine_name()))
        .with_snapshots(worker_snapshots_config(&config.s3_prefix))
