...
async fn test_cayenne_primary_key_delete() -> Result<(), anyhow::Error> {
    ...
    no_cache_context
        .scope(async {
            ...
            let delete_plan = DeletionTableProvider::delete_from(deletion_provider.as_ref(), &ctx, &[filter]).await?;
...
async fn test_cayenne_delete_then_insert_new() -> Result<(), anyhow::Error> {
    ...
    test_request_context()
        .scope(async {
            ...
            let delete_plan = DeletionTableProvider::delete_from(table.as_ref(), &ctx.state(), &[filter]).await?;
