...
/// canonical name, which `DuckDB` has no function for — before the dialect
/// rewrite the whole query failed with `Catalog Error: Scalar Function with
/// name btrim does not exist!` (issue #13794).
///
/// The unit tests in `runtime_datafusion::dialect::duckdb` pin the rendering;
/// this one is here because they cannot see the wired path: the handler has to
/// be registered in `duckdb_scalar_overrides`, that list has to reach the
/// accelerator's `DuckDBTableFactory`, and the deny-list has to leave `btrim`
/// federatable for any of it to matter. So it asserts both halves — that the
/// call is still pushed down, and that pushing it down agrees with evaluating
/// it locally.
#[tokio::test]
async fn duckdb_btrim_pushdown() -> Result<(), String> {
    let _tracing = init_tracing(Some("integration=debug,info"));
    register_test_connectors().await;
    ...
    test_request_context()
        .scope(async {
            ...
            let mut arrow_dataset =
                make_duckdb_acceleration_dataset("trim_test_arrow", "csv", &path);
            arrow_dataset.acceleration = Some(Acceleration {
                enabled: true,
                ..Default::default()
            });

            let app = AppBuilder::new("duckdb_btrim_test")
                .with_dataset(make_duckdb_acceleration_dataset("trim_test", "csv", &path))
                .with_dataset(arrow_dataset)
                .build();
            ...
            // Every shape the DuckDB rewrite has to cover: both arities, the
            // canonical spelling, a NULL and an empty character set, and the
            // call wrapped by another function.
            let projections = [
                "trim(name)",
                "trim(name, 'x')",
                "btrim(name)",
            ...
            for projection in projections {
                let diff = run(format!(
                    "WITH pushed AS (SELECT {projection} AS v FROM trim_test),
                          local AS (SELECT {projection} AS v FROM trim_test_arrow),
                          missing_locally AS (SELECT v FROM pushed EXCEPT ALL SELECT v FROM local),
                          missing_pushed AS (SELECT v FROM local EXCEPT ALL SELECT v FROM pushed)
                ...
                assert_eq!(
                    ...
                    0,
                    "`{projection}` disagrees between the DuckDB accelerator and local evaluation"
                );
            }

            // A filtered form too: the deny-list and the dialect both see filter
            // expressions, and a pushed-down `WHERE btrim(...)` was the other
            // half of the reported failure.
            let filtered =
                run("SELECT name FROM trim_test WHERE trim(name) = 'alpha'".to_string()).await?;
            assert_batches_eq!(
                [
                    "+-----------+",
                    "| name      |",
            ...
            // The call must still be *pushed down*, not merely correct: if it
            // stopped federating, every assertion above would pass while the
            // rewrite sat dead. The DuckDB SQL is the physical plan's
            // `base_sql=`; assert on that segment alone, because the logical
            // plan beside it names the expression `btrim` — which is correct
            // there, that being DataFusion's own name for it.
            let plan = run("EXPLAIN SELECT trim(name) AS v FROM trim_test".to_string()).await?;
            let plan = arrow::util::pretty::pretty_format_batches(&plan)
                .map_err(|e| format!("formatting the plan failed: {e}"))?
                .to_string();
            let base_sql = plan
                .split_once("VirtualExecutionPlan name=duckdb")
                .and_then(|(_, rest)| rest.split_once("base_sql="))
                .map(|(_, sql)| sql)
                .ok_or_else(|| {
                    format!("trim(name) is no longer pushed down to DuckDB; plan was:\n{plan}")
                })?;
            assert!(
                base_sql.contains("trim(") && !base_sql.contains("btrim("),
                "the pushed-down SQL must call DuckDB's `trim`, never `btrim`; it was:\n{base_sql}"
            );
