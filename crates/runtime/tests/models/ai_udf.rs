    ...
    test_request_context()
        .scope(async {
            ...
            let app = AppBuilder::new("ai_udf_test")
                ...
                .with_model(get_openai_model("gpt-4o-mini", "gpt-4o-mini"))
                .with_model(get_xai_model("grok-2-1212", "grok-2"))
                .with_model(get_anthropic_model("claude-3-5-haiku-20241022", "claude-haiku"))
                .build();
            ...
            let query = r#"SELECT LocationID, Zone, 
                ...
                ai(concat_ws(' ', 'Categorize the zone', Zone, 'in a single word. Only return the word.'), 'grok-2') as "xai",
            ...
            assert!(!results[3].is_empty(), "xAI (grok-2) response should not be empty");
    ...
    test_request_context()
        .scope(async {
            ...
            let query = r#"SELECT 
                ...
                left(ai('What datasets do you have access to?', 'grok-2'), 25) as "xai",
            ...
            let provider_names = ["OpenAI (gpt-4o-mini)", "xAI (grok-2)", "Anthropic (claude-haiku)"];
...
async fn test_ai_udf_with_local_model() -> Result<(), anyhow::Error> {
    ...
    test_request_context()
        .scope(async {
            // Local model test - uses Phi-3.5 mini (3.8B parameters, fast inference)
            ...
            let query = "SELECT ai('Say hello in one word', 'phi-local')";
            ...
            let query = "SELECT ai('hi', 'phi-local'), ai('hello', 'phi-local')";
            tracing::info!("Testing: Multiple calls to local model");
            let results = run_ai_query_multiple(&rt, query).await?;
            assert_eq!(results.len(), 2, "Query should return 2 columns");
            assert!(
                !results[0].is_empty(),
                "First call to phi-local should return a response"
            );
            assert!(
                !results[1].is_empty(),
                "Second call to phi-local should return a response"
            );
