...
use runtime::dataconnector::{
    DataConnector, DataConnectorFactory, DataConnectorResult, NewDataConnectorResult,
...
impl ODBCFactory {
    #[must_use]
    pub fn new_arc(handle: Handle) -> Arc<dyn DataConnectorFactory> {
        Arc::new(Self { handle }) as Arc<dyn DataConnectorFactory>
    }
}
    ...
    fn create(
        ...
        Box::pin(async move {
            ...
            let pool: Arc<ODBCDbConnectionPool> = Arc::new(
                ODBCPool::new(params.to_secret_map(), handle)
