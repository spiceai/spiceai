...
impl OtelToArrowConverter {
    ...
    ) -> Result<RecordBatch, OTelSdkError> {
        ...
        match metric.data() {
            AggregatedMetrics::F64(data) => match data {
                ...
                MetricData::ExponentialHistogram(_) => {
                    return Err(OTelSdkError::InternalFailure(
                        "ExponentialHistogram is not supported".into(),
                    ));
                }
            },
    ...
    fn process_histogram<T: AppendFloat64 + Copy>(
        ...
        for data_point in histogram.data_points() {
            ...
            self.data_histogram_builder
                ...
                .append_value(data_point.bounds().map(Some).collect::<Vec<Option<f64>>>());
