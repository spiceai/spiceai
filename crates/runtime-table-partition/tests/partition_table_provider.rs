...
struct SchemaTransformingCreator {
    ...
    engine_schema: SchemaRef,
    partitions: Arc<RwLock<HashMap<String, Arc<PartitionMemTable>>>>,
}

impl SchemaTransformingCreator {
    fn new(source_schema: SchemaRef, engine_schema: SchemaRef) -> Self {
        Self {
            source_schema,
            engine_schema,
            partitions: Arc::new(RwLock::new(HashMap::new())),
...
#[async_trait]
impl PartitionCreator for SchemaTransformingCreator {
    async fn create_partition(
        &self,
        partition_values: Vec<ScalarValue>,
    ) -> Result<Partition, creator::Error> {
        ...
        let empty_batch = RecordBatch::try_new(Arc::clone(&self.engine_schema), empty_columns)
            .map_err(|e| creator::Error::CreatePartition { source: e.into() })?;

        let mem_table = Arc::new(
            MemTable::try_new(Arc::clone(&self.engine_schema), vec![vec![empty_batch]])
                .map_err(|e| creator::Error::CreatePartition { source: e.into() })?,
        );
        let partition_mem_table = Arc::new(PartitionMemTable {
            mem_table,
    ...
    async fn infer_existing_partitions(&self) -> Result<Vec<Partition>, creator::Error> {
        // Simulate existing partitions (restart scenario) with engine schema
        let partition_values = vec![ScalarValue::Int32(Some(0))];
...
async fn test_partition_schema_uses_engine_schema() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Engine schema with Timestamp(µs) - what DuckDB actually stores
    ...
    let creator = Arc::new(SchemaTransformingCreator::new(
        Arc::clone(&source_schema),
        Arc::clone(&engine_schema),
    ));
    ...
    let table_provider = PartitionTableProvider::new(
        creator as Arc<dyn PartitionCreator>,
        partition_by,
        Arc::clone(&source_schema),
    )
    .await?;

    let advertised_schema = table_provider.schema();

    // The advertised schema should match the engine schema, not the source schema
    assert_eq!(
        advertised_schema
            .field_with_name("ts")
            .expect("ts field")
            .data_type(),
        &DataType::Timestamp(TimeUnit::Microsecond, Some("UTC".into())),
        "PartitionTableProvider should advertise the engine schema (Timestamp µs), not the source schema (Timestamp ns)"
    );
    ...
    assert_eq!(
        advertised_schema
            ...
            .data_type(),
        &DataType::Utf8,
        "PartitionTableProvider should advertise the engine schema (Utf8), not the source schema (LargeUtf8)"
    );
