...
/// Stamps the identity keys onto one unnested row.
pub(crate) fn insert_identity(row: &mut Map<String, Value>, owner: &str, repo: Option<&str>) {
    ...
    if let Some(repo) = repo {
        row.insert(REPO_COLUMN.to_string(), Value::String(repo.to_string()));
    }
}
...
pub(crate) fn push_identity_columns(
    ...
    columns.push(Arc::new(StringArray::from(vec![owner; num_rows])) as ArrayRef);
    if let Some(repo) = repo {
        columns.push(Arc::new(StringArray::from(vec![repo; num_rows])) as ArrayRef);
    }
}
...
mod tests {
    use super::{
        OWNER_COLUMN, REPO_COLUMN, identity_unnest, push_identity_columns, push_identity_fields,
    };
