...
async fn create_s3_provider(
    ...
    let Some(storage_descriptor) = table.storage_descriptor() else {
        ...
        return Err(runtime::dataconnector::DataConnectorError::InvalidConfiguration {
            dataconnector: PREFIX.to_string(),
            connector_component: (&dataset).into(),
            message: e.to_string(),
            source: Box::new(e),
    ...
    let s3 = S3::new(
        params,
        Some(Arc::unwrap_or_clone(dataset.runtime())),
        tokio_io_runtime,
    );
...
/// Returns a new instance of the `GlueDataConnector` factory.
#[must_use]
