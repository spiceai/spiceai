...
pub struct Scheduler {
    ...
    /// `poll_work` calls to the scheduler). Defaults keep a silently-dropped
    /// connection detected and reconnected well under `executor_timeout`. Without
    /// this, a stale connection hangs each `poll_work` for the request timeout,
    /// which gaps the `poll_work`-carried heartbeat and flaps the executor.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub grpc_client: Option<ClusterGrpcClient>,
}

/// gRPC client tuning for internal cluster communication.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(deny_unknown_fields)]
#[cfg_attr(feature = "schemars", derive(JsonSchema))]
pub struct ClusterGrpcClient {
    /// HTTP/2 keep-alive ping interval, in seconds. Detects a silently-dropped
    /// connection so it is torn down and reconnected; keep well under
    /// `executor_timeout`.
    #[serde(default = "default_cluster_grpc_http2_keep_alive_interval_seconds")]
    pub http2_keep_alive_interval_seconds: u64,

    /// Per-request timeout, in seconds, for cluster gRPC calls.
    #[serde(default = "default_cluster_grpc_timeout_seconds")]
    pub timeout_seconds: u64,

    /// TCP keep-alive interval, in seconds.
    #[serde(default = "default_cluster_grpc_tcp_keep_alive_seconds")]
    pub tcp_keep_alive_seconds: u64,

    /// Connection-establishment timeout, in seconds.
    #[serde(default = "default_cluster_grpc_connect_timeout_seconds")]
    pub connect_timeout_seconds: u64,

    /// HTTP/2 keep-alive ping timeout, in seconds: how long to wait for a ping
    /// ack before declaring the connection dead. Dead-connection detection is
    /// bounded by (ping interval + this), so keep the sum well under
    /// `executor_timeout` for a dropped connection to recover before the
    /// executor is reaped.
    #[serde(default = "default_cluster_grpc_keep_alive_timeout_seconds")]
    pub keep_alive_timeout_seconds: u64,
}

impl Default for ClusterGrpcClient {
    fn default() -> Self {
        Self {
            http2_keep_alive_interval_seconds:
                default_cluster_grpc_http2_keep_alive_interval_seconds(),
            timeout_seconds: default_cluster_grpc_timeout_seconds(),
            tcp_keep_alive_seconds: default_cluster_grpc_tcp_keep_alive_seconds(),
            connect_timeout_seconds: default_cluster_grpc_connect_timeout_seconds(),
            keep_alive_timeout_seconds: default_cluster_grpc_keep_alive_timeout_seconds(),
        }
    }
}

#[must_use]
pub fn default_cluster_grpc_http2_keep_alive_interval_seconds() -> u64 {
    5
}

#[must_use]
pub fn default_cluster_grpc_timeout_seconds() -> u64 {
    10
}

#[must_use]
pub fn default_cluster_grpc_tcp_keep_alive_seconds() -> u64 {
    60
}

#[must_use]
pub fn default_cluster_grpc_connect_timeout_seconds() -> u64 {
    20
}

#[must_use]
pub fn default_cluster_grpc_keep_alive_timeout_seconds() -> u64 {
    5
}
