...
- **New crates must opt into the workspace lints**: clippy lint levels live in `[workspace.lints.clippy]` in the root `Cargo.toml` (pedantic + `unwrap_used`/`expect_used`/`clone_on_ref_ptr`/…). Every member crate inherits them via `[lints]\nworkspace = true` in its `Cargo.toml` — add this to any new crate, or scoped `cargo clippy -p <crate>` and rust-analyzer will silently under-lint it. A crate needing its own lint config still uses `workspace = true` and puts shared entries in `[workspace.lints.*]` (you can't mix `workspace = true` with manual overrides in one `[lints]` table). `make lint-rust` re-applies the flags over `--workspace` as a backstop, so a forgotten opt-in fails CI, not silently ships.
