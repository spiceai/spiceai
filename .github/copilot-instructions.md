...
- **Sign off after pushing** (the script attests your pushed HEAD and refuses an unpushed branch): `make signoff` (= `make lint-rust` + `make build-cli nextest`) records the attestation that gates merge-queue entry — a single `Attestation` PR check; the full suite then runs once in the merge queue (see `docs/dev/ci_signoff.md`). These steps span profiles (lint/dev vs release), so expect separate compiles; keep the *same* `--features`/profile as the `make lint-rust` gate across your other local `cargo` commands to avoid re-fingerprinting and full rebuilds (see the fixed-feature-set bullet above).
