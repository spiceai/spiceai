...
Spice's product *is* its surface: everything a user configures, calls, reads, or scripts against. Each piece is a contract someone already depends on, so a change to it is a product decision, not an implementation detail — it is designed and reviewed in an Enhancement issue, never improvised inside a PR.

**The bar is absolute industry-leading UX/DX.** Every surface is kept as simple, easy, flexible, powerful, and understandable as it can be — not parity with the best peer, but the simplest experience that could exist. Concretely: the shortest golden path in the category (fewest steps and required params from zero to first result); a zero-config common case, with defaults that are right for most users; one obvious way to do each thing; consensus naming, so a user of Snowflake, ClickHouse, Databricks, or Kubernetes already knows the term; flexibility from a few composable concepts rather than a long list of knobs; power that costs the simple case nothing; errors that name the exact fix; and no implementation detail leaking into a name (`storage_size_gb`, not `storage_claim_size_gb`). Simplicity is measured in concepts a user has to learn, not in conveniences: before adding one, question the requirement, delete scope, and try a smarter default or a generalized field on existing surface (the Enhancement template's *The Algorithm*) — a field on an existing resource beats a new resource, endpoint, or param, and a new concept is a *no* until the proposal shows what the user cannot do without it. Best for the user beats easiest to build: when the superior experience costs more engineering, propose it anyway and name the cost.

**The surface is all of it** — anything a user interacts with, reads, or depends on:
...
- **CLI (`spice`)**: commands and subcommands, flags and defaults, prompts, table/JSON output (columns, formatting, success and failure text), exit codes.
...
- **Everything else a user reads or depends on**: on-disk layout and file names (`.spice/`, acceleration and data files), Docker images and entrypoints, and the docs and cookbook recipes the change would invalidate.
...
**The gate**: before the surface is built there is an Enhancement issue (`.github/ISSUE_TEMPLATE/enhancement.md`) whose *Specification* shows the exact surface — the YAML, the request and response, the CLI transcript, the metric names, the log lines — and whose Done-Done `PM/Design Review` and `DX/UX Review` items are ticked. The review judges the intended final experience against the bar above, not the implementation, and says where the proposal settles for parity with a peer when it could lead. Agents with the `enhancement-pm-ux-review` skill can run that review; otherwise do it by hand and record the PM and UX/DX calls in the issue.

Rules:

- **No signed-off Enhancement, no surface.** Don't invent the surface in the PR: draft the Enhancement with the proposed surface in its Specification, run the review, and stop for the sign-off before building the user-visible part — build everything that doesn't depend on it meanwhile, so the reviewers get a design question, not a working implementation to rubber-stamp.
- **The PR ships exactly the reviewed surface.** Link the Enhancement; a rename, an extra param, or a changed default discovered during implementation goes back through the issue, not into the diff. Fill in the PR template's *Breaking Changes* (plus the `breaking change` label) and *Docs* sections when they apply. `.schema/spicepod.schema.json` and `.schema/openapi.json` are regenerated by CI from the Rust types, `ParameterSpec`s, and OpenAPI annotations — that is where the reviewed surface is encoded, so check the regenerated diff matches what was signed off.
...
- **Share by `Arc` rather than deep-copying**: a value cloned on a hot path that owns heap data (a `Vec`, a decoded buffer, a nested struct) belongs behind an `Arc`, and a collection re-cloned in order to be extended holds `Arc<T>` *per element* (`Vec<Arc<T>>`) so extending it costs a refcount bump per entry instead of an allocation per entry. Caches, snapshot/overlay views, and anything rebuilt per scan are where this pays — the per-entry cost is invisible at one entry and is the whole cost at ten thousand. Only where nothing mutates in place; `Arc::make_mut` gives copy-on-write where something must. Pin the sharing with a `ptr_eq` assertion (`append_shares_tombstone_map_no_deep_copy` in `crates/cayenne/src/provider/mem_tier.rs`), since a wholesale clone that creeps back still returns correct rows and no correctness test will notice.
...
**Every `error!`/`warn!`/`info!` a user can see is a user-facing message** and follows the same guidelines as the errors above (`docs/dev/error_handling.md`): name the affected dataset/model/catalog (and schema/table), give an actionable fix with a docs link, and use no internal vocabulary ("table provider", "read provider", "connection pool", "metadata"). Put the resource *in the message text*, not only in a `tracing` field. Log lines are user-facing surface — see *User-facing surface* for which changes to them need an Enhancement.
...
- **Config is user-facing surface** (see *User-facing surface*): the name, accepted values, and default of a new or changed field, `params` key, flag, or env var are decided in the Enhancement review, not in the PR.
...
1. Find every wrapper impl: `rg -n "impl\s+(\w+\s+for\s+)?<TraitName>\b" crates/`. Known wrapped traits:
   ...
   - `TableProvider` — `AcceleratedTable`, `SpiceTable`, `SwappableTableProvider`, `PartitionTableProvider`, `IndexedMemTable`, `EnsureSchema`, `PolyTableProvider`, `MetadataEnrichedTableProvider`, `UpsertDedupTableProvider`, view/sink/partition providers — **including `statistics()`**
   - `Index` (`crates/spice-table`) — `CompoundSearchIndex`, `CompoundVectorIndex`, `ChunkedSearchIndex`, `ChunkedVectorIndex` (two of its methods carry an explicit "wrappers MUST forward this" note)
...
3. Put `#[deny(clippy::missing_trait_methods)]` on every wrapper impl (38 already carry it, e.g. `EmbeddingConnector`, `FullTextConnector`, `PartitionTableProvider`). It turns "forgot to forward" into a compile error, which is the only check that catches this class mechanically — a wrapper without it is one defaulted method away from a silent regression.
...
- **A green unit test is not proof a bug existed or is gone** — see *Evidence — no claim without a reproduction*.
