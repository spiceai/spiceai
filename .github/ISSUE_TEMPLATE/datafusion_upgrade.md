...
- [ ] **[datafusion-federation](https://github.com/spiceai/datafusion-federation)**: Update the fork to be compatible with the new DataFusion version.
  - We keep a fork because our changes are incompatible with upstream, and we maintain this fork separately. Create a branch from the previous `spiceai-Y` version into `spiceai-X`, and perform the upgrades there.
