...
pub(crate) fn build_postgres_setup_response(
    connection_string: Option<&str>,
    state: &RunState,
) -> SetupResponse {
    let mut kwargs: HashMap<String, serde_json::Value> = HashMap::new();
    if let Some(conn_str) = connection_string {
        kwargs.insert("uri".into(), serde_json::Value::String(conn_str.to_string()));
    }
...
/// Generate a spicepod that reads from PostgreSQL via a temporary replication slot,
/// using the `PG_CONNECTION_STRING` environment variable. Unlike `generate_postgres_wal_spicepod`,
/// this does not require pre-created publications — Spice manages the slot automatically.
pub(crate) fn generate_postgres_spicepod(
    run_id: &Uuid,
    datasets: &HashMap<String, DatasetConfig>,
) -> SpicepodDefinition {
    let run_id_str = run_id.to_string();
    ...
    for dataset_name in datasets.keys() {
        let param_map = HashMap::from([
            (
                "pg_connection_string".to_string(),
                "${env:PG_CONNECTION_STRING}".to_string(),
            ),
            // Create a temporary slot so it is dropped automatically when spiced exits.
            ("pg_replication_temporary_slot".to_string(), "true".to_string()),
        ]);
