...
use test_framework::{
    anyhow, arrow::util::pretty::print_batches, flight::query_to_batches, spiced::SpicedInstance,
};
...
pub(crate) async fn run(args: &SchemaTestArgs) -> anyhow::Result<()> {
    ...
    if total_table_rows == 0 {
        return Err(anyhow::anyhow!(
            "Schema test failed: no tables found in information_schema.tables. \
             Catalog connector may not have registered any tables."
        ));
    }
    ...
    if total_column_rows == 0 {
        return Err(anyhow::anyhow!(
            "Schema test failed: no columns found in information_schema.columns. \
             Tables were discovered but their schemas were not registered."
        ));
    }
