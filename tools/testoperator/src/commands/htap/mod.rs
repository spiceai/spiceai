...
pub(crate) async fn run(args: &HtapArgs) -> anyhow::Result<()> {
    ...
    let debezium_harness = match args.cdc_mode {
        HtapCdcMode::Native => None,
        HtapCdcMode::Debezium => {
            let source = chbench_source_from_env()?;
            let harness = debezium::DebeziumHarness::start(&source).await?;
            harness.rewrite_app(&mut app);
            Some(harness)
        }
        HtapCdcMode::DebeziumPreloaded => {
            let harness = debezium::DebeziumHarness::preloaded_from_env()?;
            harness.rewrite_app(&mut app);
            Some(harness)
        }
    };
