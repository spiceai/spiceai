...
fn start_local_containers() -> anyhow::Result<()> {
    ...
    run_docker(&[
        ...
        "host",
        "-e",
        &format!("BOOTSTRAP_SERVERS={DEFAULT_KAFKA_BOOTSTRAP}"),
        "-e",
    ...
    println!(
        "Started local CH-benCH Debezium stack: {REDPANDA_CONTAINER} ({DEFAULT_KAFKA_BOOTSTRAP}), {DEBEZIUM_CONTAINER} ({DEFAULT_CONNECT_URL})"
    );
    Ok(())
}
...
async fn register_connector(
    ...
    let publication_name = pg_identifier(&format!("{topic_prefix}_pub_{table}"));
    let table_include = format!("public.{table}");
    ...
    println!(
        "Debezium: registered connector '{connector_name}' for public.{table} -> {topic_prefix}.public.{table}"
    );
    Ok(())
}
