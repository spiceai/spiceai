...
impl DebeziumHarness {
    ...
    pub(super) fn preloaded_from_env() -> anyhow::Result<Self> {
        let kafka_bootstrap_servers = std::env::var("CHBENCH_KAFKA_BOOTSTRAP_SERVERS")
            .context("CHBENCH_KAFKA_BOOTSTRAP_SERVERS must point to the preloaded Kafka broker")?;
        let topic_prefix = std::env::var("CHBENCH_DEBEZIUM_TOPIC_PREFIX")
            .context("CHBENCH_DEBEZIUM_TOPIC_PREFIX must name the preloaded Debezium topic prefix")?;

        println!(
            "CH-benCH Debezium CDC: using preloaded topic prefix '{topic_prefix}' (Kafka {kafka_bootstrap_servers})"
        );
...
async fn register_connector(
    ...
    let body = serde_json::json!({
        ...
        "config": {
            ...
            // Full initial snapshot: v1.11 consumes the whole baseline through
            // Debezium/Kafka, matching how this compatibility path would have
            // worked without the native Postgres CDC connector.
