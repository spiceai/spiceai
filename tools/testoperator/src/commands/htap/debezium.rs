...
/// Temporary directory that holds absolute Cayenne file paths shared by the
/// Postgres prewarm spiced process and the final v1.11 Debezium spiced process.
pub(super) struct CayennePrewarmDir {
    path: PathBuf,
}

impl CayennePrewarmDir {
    #[must_use]
...
impl Drop for CayennePrewarmDir {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Rewrites every Cayenne file path in the app to an absolute path under a
/// per-run temp dir. This lets a first spiced process bulk-load from Postgres,
/// then a second spiced process open the same files and apply Debezium changes.
pub(super) fn rewrite_cayenne_file_paths(app: &mut App) -> anyhow::Result<CayennePrewarmDir> {
    let root = std::env::temp_dir().join(format!(
        "chbench-cayenne-prewarm-{}-{}",
        std::process::id(),
        uuid::Uuid::new_v4()
    ));
    std::fs::create_dir_all(&root).with_context(|| {
        format!(
            "create CH-benCH Cayenne prewarm directory {}",
            root.display()
        )
    })?;
    ...
    for dataset in &mut app.datasets {
        ...
        if acceleration.engine.as_deref() != Some("cayenne") {
            continue;
        }
        ...
        params.data.insert(
            ...
            ParamValue::String(table_dir.to_string_lossy().to_string()),
        );
        rewritten += 1;
    }

    println!(
        "CH-benCH Debezium CDC: rewrote {rewritten} Cayenne file path(s) under {} for Postgres prewarm reuse",
        root.display()
    );

    Ok(CayennePrewarmDir { path: root })
}

/// Build the app used for the prewarm phase: keep Postgres sources, but turn
/// changes-mode CH-benCH tables into ordinary full refreshes so initial SF100
/// data loads via the fast Postgres path instead of through Debezium snapshots.
#[must_use]
pub(super) fn postgres_prewarm_app(app: &App) -> App {
    let mut prewarm = app.clone();
    for dataset in &mut prewarm.datasets {
        let table = dataset
            .name
            .rsplit_once('.')
            .map_or(dataset.name.as_str(), |(_, table)| table);
...
/// Declare schemas captured from the Postgres prewarm on the final Debezium
/// datasets. With `snapshot.mode=no_data`, v1.11 must not block registration on
/// a schema-bearing Kafka message. Declared columns let it open the existing
/// acceleration immediately and start streaming future changes when topics are
/// created by OLTP writes.
pub(super) fn declare_dataset_schemas(
    app: &mut App,
        ...
        dataset.columns = schema
            ...
            .map(|field| {
                Column::new(field.name())
                    .with_type(declared_type_for_arrow(field.data_type()))
    ...
    println!("CH-benCH Debezium CDC: declared schemas for {declared} Debezium dataset(s)");
    declared
}

fn declared_type_for_arrow(data_type: &DataType) -> String {
    match data_type {
...
pub(super) async fn wait_for_source_spice_catchup(
    driver: Arc<dyn ChBenchDriver>,
    spice: &SpiceClients,
    tables: &[String],
    max_wait: Duration,
    ...
    println!(
        "\nWaiting up to {}s for {phase_name} to catch up before OLTP...",
        max_wait.as_secs()
    );
    ...
    loop {
        ...
        if pending.is_empty() {
            println!(
                "{phase_name} caught up in {}ms for {} table(s)",
        ...
        if start.elapsed() >= max_wait {
            anyhow::bail!(
                "{phase_name} did not catch up within {}s; pending: {}",
                max_wait.as_secs(),
                pending.join("; ")
            );
        }
        ...
        if last_log.elapsed() >= Duration::from_secs(15) {
            println!(
                "{log_prefix} still catching up ({}ms elapsed): {}",
                start.elapsed().as_millis(),
                pending.join("; ")
            );
...
async fn register_connector(
    ...
    let body = serde_json::json!({
        ...
        "config": {
            ...
            // The initial SF100 data is prewarmed directly from Postgres into
            // Cayenne files. Debezium should only stream OLTP changes after the
            // connector starts. The final Spice datasets declare their schema so
            // v1.11 does not need a schema-bearing snapshot event.
