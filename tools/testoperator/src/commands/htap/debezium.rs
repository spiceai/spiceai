...
async fn register_connectors(
    ...
    // Use one PostgreSQL connector for all changes-mode CH-benCH tables. This
    // keeps Debezium on a single replication slot/publication and avoids the
    // runner/PostgreSQL limits and Connect group rebalances we saw when creating
    // one connector per table. Topics remain {topic_prefix}.public.{table}, so
    // the Spice Debezium datasets do not need to change.
    let table_include_list = STALENESS_PROBE_TABLES
        .iter()
    ...
    wait_for_connector_running(
        connect_url,
        &client,
        &connector_name(topic_prefix, "chbench"),
    )
    .await?;
...
async fn register_connector(
    ...
    let connector_name = connector_name(topic_prefix, "chbench");
    let slot_name = pg_identifier(&format!("{topic_prefix}_slot"));
    let publication_name = pg_identifier(&format!("{topic_prefix}_pub"));
