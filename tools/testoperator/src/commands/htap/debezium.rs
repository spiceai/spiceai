...
async fn register_connector(
    ...
    let snapshot_override_key = format!("snapshot.select.statement.overrides.public.{table}");
    let mut body = serde_json::json!({
        ...
        "config": {
            ...
            // Cayenne files. Debezium only needs to seed each Kafka topic with a
            // schema-bearing event before switching to streaming changes. A full
            // Debezium snapshot takes too long for order_line at SF100.
            "snapshot.mode": "initial",
            "snapshot.select.statement.overrides": table_include_list,
    ...
    body["config"][snapshot_override_key.as_str()] =
        serde_json::Value::String(format!("SELECT * FROM public.{table} LIMIT 1"));
