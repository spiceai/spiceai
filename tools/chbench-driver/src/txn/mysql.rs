...
pub async fn execute(
    ...
    match txn_type {
        super::TxnType::NewOrder => run_new_order(conn, rng, assignment).await,
        super::TxnType::Payment => run_payment(conn, rng, assignment).await,
        super::TxnType::Delivery => run_delivery(conn, rng, assignment.num_warehouses).await,
...
async fn run_new_order(
    ...
    let mut insert_sql = String::with_capacity(writes.len() * 40 + 128);
    insert_sql.push_str(
        "INSERT INTO order_line \
         (ol_o_id, ol_d_id, ol_w_id, ol_number, ol_i_id, ol_supply_w_id, \
          ol_quantity, ol_amount, ol_dist_info) VALUES ",
    );
    let mut params: Vec<Value> = Vec::with_capacity(writes.len() * 9);
    for (i, w) in writes.iter().enumerate() {
        if i > 0 {
            insert_sql.push(',');
        }
        // Placeholder count per row: 9. Overall placeholder count: 9 * writes.len().
