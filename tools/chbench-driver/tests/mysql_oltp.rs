...
/// `mysql_async` enables `CLIENT_MULTI_STATEMENTS` unconditionally
/// (`opts/mod.rs` `get_capabilities`), so several `;`-separated statements can
/// share one round-trip via `query_iter` — the `MySQL` analogue of the Postgres
/// path's `batch_execute`. The batched transaction rewrites depend on this
/// working *inside* a transaction and on DML, so prove both rather than
/// inferring it from the capability flag.
#[tokio::test(flavor = "multi_thread")]
async fn multi_statement_dml_shares_one_round_trip() {
    if !enabled() {
        eprintln!("skipping: set CHBENCH_MYSQL_E2E=1 to run against a live MySQL source");
        return;
    }
    ...
    conn.query_drop("DROP TABLE IF EXISTS _multi_stmt_probe")
        .await
        .expect("drop probe table");
    conn.query_drop("CREATE TABLE _multi_stmt_probe (k INT PRIMARY KEY, v INT NOT NULL)")
        .await
    ...
    let mut tx = conn
        ...
        .await
        .expect("begin");
    // Three DML statements plus a SELECT, in one call.
    let result = tx
        .query_iter(
            "INSERT INTO _multi_stmt_probe (k, v) VALUES (1, 10), (2, 20); \
             UPDATE _multi_stmt_probe SET v = v + 1 WHERE k = 1; \
             DELETE FROM _multi_stmt_probe WHERE k = 2; \
             SELECT k, v FROM _multi_stmt_probe",
        )
        .await
        .expect("multi-statement DML in a transaction");
    // Every result set must be drained before the connection is reusable;
    // `drop_result` is the drain that handles all of them.
    result.drop_result().await.expect("drain all result sets");
    tx.commit().await.expect("commit");

    let rows: Vec<(i32, i32)> = conn
        .query("SELECT k, v FROM _multi_stmt_probe ORDER BY k")
        .await
        .expect("read back");
    assert_eq!(
        rows,
        vec![(1, 11)],
        "all four statements must have applied in order"
    );

    conn.query_drop("DROP TABLE _multi_stmt_probe")
        .await
        .expect("drop probe table");
}

/// Time the individual statement shapes `run_new_order` uses, to attribute
/// transaction cost to a specific round-trip rather than inferring it.
#[tokio::test(flavor = "multi_thread")]
async fn statement_timing_probe() {
    if !enabled() {
        eprintln!("skipping: set CHBENCH_MYSQL_E2E=1 to run against a live MySQL source");
        return;
    }
    ...
    // Bare round-trip floor.
    let t = Instant::now();
    for _ in 0..n {
        let _: Option<i32> = conn.query_first("SELECT 1").await.expect("select 1");
    }
    println!(
        "SELECT 1 (text):            {:>7.0}us",
        t.elapsed().as_secs_f64() * 1_000_000.0 / f64::from(n)
    );

    let t = Instant::now();
    for _ in 0..n {
        let _: Option<i32> = conn.exec_first("SELECT ?", (1,)).await.expect("exec 1");
    }
    println!(
        "SELECT ? (prepared):        {:>7.0}us",
        t.elapsed().as_secs_f64() * 1_000_000.0 / f64::from(n)
    );

    // Spread-out stock read, prepared, FOR UPDATE inside a transaction.
    let t = Instant::now();
    for _ in 0..n {
        let mut tx = conn
            .start_transaction(mysql_async::TxOpts::default())
            .await
            .expect("begin");
        let _: Vec<(i32, i32, i32, String)> = tx.exec(
            "SELECT s_w_id, s_i_id, s_quantity, s_dist_01 FROM stock \
             WHERE (s_w_id, s_i_id) IN ((?,?),(?,?),(?,?),(?,?),(?,?),(?,?),(?,?),(?,?),(?,?),(?,?)) \
             ORDER BY s_w_id, s_i_id FOR UPDATE",
            [37, 4021, 19887, 54012, 98311, 271, 33333, 61111, 7777, 90001]
                .iter()
                .flat_map(|&i| [mysql_async::Value::from(1), mysql_async::Value::from(i)])
                .collect::<Vec<_>>(),
        ).await.expect("stock batch read");
        tx.rollback().await.expect("rollback");
    }
    println!(
        "txn + stock IN x10 FOR UPD: {:>7.0}us",
        t.elapsed().as_secs_f64() * 1_000_000.0 / f64::from(n)
    );

    // The multi-statement write batch shape, rolled back so state is untouched.
    let mut batch = String::new();
    batch.push_str("UPDATE district SET d_next_o_id = d_next_o_id WHERE d_id = 1 AND d_w_id = 1;");
    for i in 0..10 {
        use std::fmt::Write as _;
        write!(
            &mut batch,
            "UPDATE stock SET s_quantity = s_quantity, s_ytd = s_ytd + 0, s_order_cnt = s_order_cnt + 0, \
             s_remote_cnt = s_remote_cnt + 0 WHERE s_i_id = {} AND s_w_id = 1;", 37 + i * 5000
        )
    ...
    for _ in 0..n {
        let mut tx = conn
            ...
            .await
            .expect("begin");
        tx.query_drop(&batch).await.expect("batch");
        tx.rollback().await.expect("rollback");
    }
    println!(
        "txn + 11-stmt write batch:  {:>7.0}us",
        t.elapsed().as_secs_f64() * 1_000_000.0 / f64::from(n)
    );

    // Same 11 writes as separate prepared statements, for comparison.
    let t = Instant::now();
    for _ in 0..n {
        let mut tx = conn
            .start_transaction(mysql_async::TxOpts::default())
            ...
            .expect("begin");
        tx.exec_drop(
            "UPDATE district SET d_next_o_id = d_next_o_id WHERE d_id = ? AND d_w_id = ?",
            (1, 1),
        )
        .await
        .expect("d");
        for i in 0..10 {
            tx.exec_drop(
                "UPDATE stock SET s_quantity = s_quantity, s_ytd = s_ytd + 0, s_order_cnt = s_order_cnt + 0, \
                 s_remote_cnt = s_remote_cnt + 0 WHERE s_i_id = ? AND s_w_id = ?",
                (37 + i * 5000, 1),
            ).await.expect("s");
        }
        tx.rollback().await.expect("rollback");
    ...
    println!(
        "txn + 11 separate prepared: {:>7.0}us",
        t.elapsed().as_secs_f64() * 1_000_000.0 / f64::from(n)
    );
}
